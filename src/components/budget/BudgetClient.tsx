...
export default function BudgetClient() {
  ...
  const load = useCallback(async () => {
    setLoad(true);
    try {
      const r = await fetch("/api/portfolio/financial-plan-summary", {
        cache: "no-store",
      });
  ...
  const totalVar =
    port?.totalVariance ?? ((port?.totalForecast ?? 0) - (port?.totalBudget ?? 0));

  const overCount = projects.filter(
    (p) => p.hasFinancialPlan && varRag(p.totals.variance, p.totals.budget) === "R"
  ).length;
  const watchCount = projects.filter(
    (p) => p.hasFinancialPlan && varRag(p.totals.variance, p.totals.budget) === "A"
  ).length;
  const okCount = projects.filter(
    (p) => p.hasFinancialPlan && varRag(p.totals.variance, p.totals.budget) === "G"
  ).length;
  ...
  const filtered = useMemo(() => {
    ...
    if (q) {
      list = list.filter(
        (p) =>
          ...
          (p.projectCodeLabel ?? "").toLowerCase().includes(q)
      );
    }
    ...
    if (filter === "over") {
      ...
      list = list.filter((p) => !p.hasFinancialPlan);
    }
          ...
          >
            ...
            >
              <div>
                ...
                >
                  Financial health across all active projects -- forecast variance,
                  burn rate and delivery exposure.
                </p>
              </div>
          ...
          {!loading && port && (
            ...
            >
              ...
              <KpiCell
                ...
                value={signedFmt(totalVar)}
                color={totalVar > 0 ? RC.R.fg : RC.G.fg}
                note="Forecast minus budget"
              />
              <KpiCell
                label="Over Budget"
                value={String(overCount)}
                color={overCount > 0 ? RC.R.fg : T.ink}
              />
            </div>
          )}
              ...
              {filtered.length === 0 ? (
                ...
                <div style={{ overflowX: "auto" }}>
                  ...
                  >
                    ...
                    <tbody>
                      {filtered.map((p) => {
                        const rag = p.hasFinancialPlan
                          ? varRag(p.totals.variance, p.totals.budget)
                          : "N";
                        ...
                        return (
                          ...
                          >
                            ...
                            <td style={TD}>
                              <Mono size={12} color={T.ink3} weight={500}>
                                {p.hasFinancialPlan ? fmt(p.totals.budget) : "--"}
                              </Mono>
                            </td>

                            <td style={TD}>
                              <Mono size={12} color={T.ink3} weight={500}>
                                {p.hasFinancialPlan ? fmt(p.totals.forecast) : "--"}
                              </Mono>
                            </td>

                            <td style={TD}>
                              <Mono size={12} color={T.ink3} weight={500}>
                                {p.hasFinancialPlan ? fmt(p.totals.actual) : "--"}
                              </Mono>
                            </td>
                            ...
                            <td style={TD}>
                              {p.hasFinancialPlan ? (
                                <Mono
                                  size={12}
                                  color={p.totals.variance > 0 ? RC.R.fg : RC.G.fg}
                                  weight={600}
                                >
                                  {signedFmt(p.totals.variance)}
                                </Mono>
                              ) : (
                                <Mono size={11} color={T.ink5}>
                                  No plan
                              ...
                              {p.hasFinancialPlan ? (
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    <Mono size={10} color={RC[brag].fg} weight={600}>
                                      {p.totals.burnPct}%
                                    </Mono>
                                  </div>
                            ...
                            <td style={TD}>
                              {p.hasFinancialPlan ? (
                                p.artifactId ? (
                                  <Link
                                    href={`/projects/${p.projectId}/artifacts/${p.artifactId}?panel=intelligence`}
                                    style={{
                                      fontFamily: T.mono,
                                      fontSize: 10,
                                      fontWeight: 600,
                                ...
                                <Link
                                  href={`/projects/${p.projectId}/artifacts/new?type=FINANCIAL_PLAN`}
                                  style={{
                                    fontFamily: T.mono,
                                    fontSize: 10,
                                    color: "#1d4ed8",
