...
type ProjectSummary = {
  projectId: string; projectCode: string | null; projectCodeLabel: string | null;
  title: string; colour: string; hasFinancialPlan: boolean; artifactId: string | null;
  totals: { budget: number; forecast: number; actual: number; variance: number; burnPct: number };
};
type Portfolio = { totalBudget: number; totalForecast: number; totalActual: number; projectCount: number; withPlanCount: number };
type ApiResponse = { ok: boolean; portfolio: Portfolio; projects: ProjectSummary[]; error?: string };
type SortKey = "variance" | "burn" | "budget" | "title";
type Filter  = "all" | "over" | "watch" | "ok" | "no-plan";

const T = {
  bg: "#f9f7f4", surface: "#ffffff", hr: "#e7e5e4", ink: "#1c1917",
  ink2: "#44403c", ink3: "#78716c", ink4: "#a8a29e", ink5: "#d6d3d1",
...
function varRag(v: number, b: number): "G"|"A"|"R"|"N" {
  if (b === 0) return "N";
  const p = (v/b)*100;
  if (p > 5) return "R"; if (p > 0) return "A"; return "G";
}
                            ...
                            <td style={TD}>
                              {p.hasFinancialPlan ? (
                                p.artifactId ? (
                                  ...
                                  <span style={{ fontFamily:T.mono, fontSize:10, fontWeight:600, letterSpacing:"0.07em", textTransform:"uppercase", padding:"3px 10px", borderRadius:2, background:rc.bg, color:rc.fg }}>
                                    {rc.label}
                                  </span>
                                )
                              ) : (
