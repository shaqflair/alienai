"use client";
// src/components/raid/RaidClient.tsx
...
function Pill({ label, active, onClick, dotColor }: {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      {dotColor && active && <span style={{ width: 5, height: 5, borderRadius: "50%", background: dotColor }} />}
      {label}
    </button>
  );
}

function KpiCell({ label, value, sub, alert }: { label: string; value: string | number; sub?: string; alert?: boolean }) {
  return (
    <div style={{ padding: "22px 28px", borderRight: `1px solid ${T.hr}` }}>
      <Cap>{label}</Cap>
      <div style={{ fontFamily: T.serif, fontSize: 40, fontWeight: 700, lineHeight: 1, marginTop: 10, marginBottom: 6, color: alert ? RAG_CONFIG.R.fg : T.ink }}>
        {value}
      </div>
      {sub && <Cap>{sub}</Cap>}
    </div>
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <div style={{ width: 52, height: 3, background: T.ink5, borderRadius: 3, overflow: "hidden", flexShrink: 0 }}>
        <div style={{ width: `${score}%`, height: "100%", borderRadius: 3, background: color, transition: "width 0.9s cubic-bezier(0.16,1,0.3,1)" }} />
      </div>
...
function StatusChip({ status }: { status: string }) {
  ...
  return (
    <span style={{ fontFamily: T.mono, fontSize: 9, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", padding: "2px 7px", borderRadius: 2, background: bg, color: fg, border: `1px solid ${bd}`, whiteSpace: "nowrap" }}>
      {status}
    </span>
  );
}
...
function DetailDrawer({ item, onUpdated }: { item: RaidItem; onUpdated?: (updates: Partial<RaidItem>) => void }) {
  ...
  const merged = { ...item, ...localItem } as RaidItem;
  ...
  const rag      = itemRag(merged);
  const exposure = totalExposure(merged);
  const rc       = RAG_CONFIG[rag];

  return (
    <div style={{ background: rc.bg, borderTop: `1px solid ${rc.border}`, padding: "26px 36px 30px 36px", animation: "drawerOpen 0.18s ease-out both" }}>
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: 48 }}>

        {/* Summary */}
        <div>
          <SectionRule label="Intelligence Summary" />
          <p style={{ fontFamily: T.body, fontSize: 13.5, color: T.ink2, lineHeight: 1.75, margin: 0, fontWeight: 300 }}>
            {merged.ai_rollup || merged.description || "No summary available for this item."}
          </p>
          {merged.ai_status && (
            <div style={{ marginTop: 10 }}>
              <Mono size={10} color={T.ink4}>AI assessment: </Mono>
              <Mono size={10} color={T.ink3}>{merged.ai_status}</Mono>
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
            {([
              ["Probability",    merged.probability           != null ? `${merged.probability}%`           : null],
              ["Severity",       merged.severity              != null ? `${merged.severity}%`              : null],
              ["SLA Breach",     merged.sla_breach_probability != null ? `${merged.sla_breach_probability}%` : null],
              ["Days to Breach", merged.sla_days_to_breach    != null ? `~${merged.sla_days_to_breach}d`   : null],
              ["AI Confidence",  merged.sla_confidence        != null ? `${merged.sla_confidence}%`        : null],
              ["Scoring Model",  merged.score_source === "ai"  ? "AI (trained)" : "Formula"],
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
            {([
              ["Cost Impact",     merged.est_cost_impact],
              ["Revenue at Risk", merged.est_revenue_at_risk],
              ["Penalties",       merged.est_penalties],
            ] as [string, number | null][]).map(([k, v]) => (
              <div key={k} style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 12 }}>
                <Mono size={11} color={T.ink4}>{k}</Mono>
                <Mono size={11} color={T.ink2} weight={500}>{fmtMoney(merged.currency_symbol, v)}</Mono>
              </div>
            ))}
            <div style={{ borderTop: `1px solid ${T.hr}`, paddingTop: 9, display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
              ...
              <Mono size={14} color={exposure > 0 ? RAG_CONFIG.A.fg : T.ink5} weight={600}>
                {fmtMoney(merged.currency_symbol, exposure || null)}
              </Mono>
            </div>
          </div>
          ...
          <div style={{ marginTop: 16, display: "flex", flexDirection: "column", gap: 8 }}>
            <button
              ...
              disabled={assessing}
              style={{ padding: "7px 12px", fontFamily: T.mono, fontSize: 10, fontWeight: 600, letterSpacing: "0.07em", textTransform: "uppercase", background: assessing ? T.ink4 : T.ink, color: "#fff", border: "none", borderRadius: 2, cursor: assessing ? "default" : "pointer", width: "100%" }}
            >
              {assessing ? "Assessing..." : "AI Assess Financial Impact"}
            </button>
            {aiFinErr && (
              <div style={{ fontFamily: T.mono, fontSize: 10, color: RAG_CONFIG.R.fg, padding: "6px 8px", background: RAG_CONFIG.R.bg, borderRadius: 2 }}>{aiFinErr}</div>
            )}
            {aiFinData && (
              <div style={{ background: "rgba(255,255,255,0.6)", borderRadius: 2, padding: "10px 12px", border: `1px solid ${T.hr}`, display: "flex", flexDirection: "column", gap: 6 }}>
                {([
                  ...
                  <div key={k} style={{ display: "flex", justifyContent: "space-between" }}>
                    ...
                    <Mono size={10} color={T.ink2} weight={600}>{fmtMoney(merged.currency_symbol || "GBP", v)}</Mono>
                  </div>
                ))}
                ...
                <div style={{ borderTop: `1px solid ${T.hr}`, paddingTop: 6, marginTop: 2 }}>
                  ...
                  <div style={{ marginTop: 4, fontFamily: T.body, fontSize: 11, color: T.ink4, lineHeight: 1.5 }}>{aiFinData.reasoning}</div>
                </div>
              </div>
            )}
            <div style={{ display: "flex", gap: 10, alignItems: "center", marginTop: 4 }}>
              <button
                ...
                style={{ padding: "6px 14px", fontFamily: T.mono, fontSize: 10, fontWeight: 600, letterSpacing: "0.07em", textTransform: "uppercase", background: T.ink, color: "#fff", border: "none", borderRadius: 2, cursor: "pointer" }}
              >
                Edit Item
              </button>
              <Link
                href={`/projects/${merged.project_id}/raid`}
                onClick={(e) => e.stopPropagation()}
                style={{ fontFamily: T.mono, fontSize: 10, fontWeight: 600, letterSpacing: "0.1em", color: "#1d4ed8", textDecoration: "none", borderBottom: "1px solid #bfdbfe", paddingBottom: 1 }}
              >
...
/* - Edit Item Modal -- includes Financial Exposure section - */
...
function EditItemModal({ item, onClose, onSaved }: {
  ...
  const [costImpact,    setCostImpact]    = React.useState<string>(item.est_cost_impact != null ? String(item.est_cost_impact) : "");
  const [revenueAtRisk, setRevenueAtRisk] = React.useState<string>(item.est_revenue_at_risk != null ? String(item.est_revenue_at_risk) : "");
  const [penalties,     setPenalties]     = React.useState<string>(item.est_penalties != null ? String(item.est_penalties) : "");
  ...
  function parseMoneyField(s: string): number | null {
    const cleaned = s.replace(/[GBP,\s]/g, "");
  ...
  async function handleSave() {
    ...
    const costVal    = parseMoneyField(costImpact);
    const revenueVal = parseMoneyField(revenueAtRisk);
    const penaltyVal = parseMoneyField(penalties);
    try {
      const res = await fetch(`/api/raid/${item.id}`, {
        ...
        body: JSON.stringify({
          ...
          est_cost_impact:    costVal,
          est_revenue_at_risk: revenueVal,
          est_penalties:      penaltyVal,
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}
      ...
      <div style={{ background: T.surface, borderRadius: 4, border: "1px solid " + T.hr, boxShadow: "0 24px 80px rgba(0,0,0,0.2)", width: "100%", maxWidth: 580, maxHeight: "90vh", overflowY: "auto" }}>

        <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid " + T.hr, display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, background: T.surface, zIndex: 1 }}>
          <div>
            <div style={{ fontFamily: T.serif, fontSize: 20, fontWeight: 700, color: T.ink }}>Edit Item</div>
            <Cap>{item.project_title}</Cap>
          </div>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", fontFamily: T.mono, fontSize: 18, color: T.ink4, padding: "4px 8px" }}>x</button>
        </div>
        ...
        <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div>
            ...
            <textarea value={description} onChange={e => setDescription(e.target.value)}
              rows={3} style={{ ...INP, resize: "vertical", fontFamily: T.body, fontSize: 13, color: T.ink, background: "#fff" }} />
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div>
              ...
              <input type="range" min={0} max={100} step={5} value={probability}
                onChange={e => setProbability(Number(e.target.value))} style={{ width: "100%", accentColor: T.ink }} />
            </div>
            <div>
              <label style={LBL}>Severity: {severity}%</label>
              <input type="range" min={0} max={100} step={5} value={severity}
                onChange={e => setSeverity(Number(e.target.value))} style={{ width: "100%", accentColor: T.ink }} />
            </div>
          </div>
          <div style={{ background: "#f5f3f0", padding: "8px 12px", borderRadius: 2 }}>
            <span style={{ fontFamily: T.mono, fontSize: 10, color: T.ink3 }}>
              ...
              <span style={{ color: T.ink4 }}> = {probability}% x {severity}% / 100</span>
            </span>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <div>
              ...
              <input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} style={{ ...INP, colorScheme: "light" }} />
            </div>
          </div>

          {/* Financial Exposure */}
          <div style={{ borderTop: "1px solid " + T.hr, paddingTop: 14 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
              <Cap>Financial Exposure</Cap>
              <span style={{ fontFamily: T.mono, fontSize: 9, color: T.ink5 }}>Enter values in GBP (e.g. 50000)</span>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
              <div>
              ...
              <div>
                <label style={LBL}>Revenue at Risk</label>
                <input
                  value={revenueAtRisk}
              ...
              <div>
                <label style={LBL}>Penalties</label>
                <input
                  value={penalties}
            ...
            {/* Live total */}
            {(parseMoneyField(costImpact) || parseMoneyField(revenueAtRisk) || parseMoneyField(penalties)) ? (
              <div style={{ marginTop: 8, padding: "6px 10px", background: "#fffbeb", border: "1px solid #fcd34d", borderRadius: 2, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <Mono size={10} color={T.ink4}>Total Exposure</Mono>
                <Mono size={12} color={RAG_CONFIG.A.fg} weight={600}>
                  {fmtMoney("GBP ", (parseMoneyField(costImpact) ?? 0) + (parseMoneyField(revenueAtRisk) ?? 0) + (parseMoneyField(penalties) ?? 0))}
                </Mono>
              </div>
            ) : null}
          </div>
          ...
          {error && (
            <div style={{ padding: "10px 14px", borderRadius: 2, background: RAG_CONFIG.R.bg, border: "1px solid " + RAG_CONFIG.R.border, fontFamily: T.mono, fontSize: 11, color: RAG_CONFIG.R.fg }}>{error}</div>
          )}
        </div>

        <div style={{ padding: "14px 24px 20px", borderTop: "1px solid " + T.hr, display: "flex", justifyContent: "flex-end", gap: 10, position: "sticky", bottom: 0, background: T.surface }}>
          <button onClick={onClose} style={{ padding: "9px 20px", fontFamily: T.mono, fontSize: 10, fontWeight: 600, letterSpacing: "0.07em", textTransform: "uppercase", background: "transparent", color: T.ink3, border: "1px solid " + T.hr, borderRadius: 2, cursor: "pointer" }}>
            Cancel
          </button>
          <button onClick={handleSave} disabled={saving} style={{ padding: "9px 20px", fontFamily: T.mono, fontSize: 10, fontWeight: 600, letterSpacing: "0.07em", textTransform: "uppercase", background: saving ? T.ink3 : T.ink, color: "#fff", border: "none", borderRadius: 2, cursor: saving ? "default" : "pointer" }}>
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch("/api/raid", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          project_id: projectId, type, priority, status: "Open",
          title: title.trim(), description: description.trim(),
          due_date: dueDate || null, owner_label: owner.trim(),
      ...
      <div style={{ background: T.surface, borderRadius: 4, border: "1px solid " + T.hr, boxShadow: "0 24px 80px rgba(0,0,0,0.2)", width: "100%", maxWidth: 560, animation: "fadeUp 0.2s ease both", overflow: "hidden", maxHeight: "92vh", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 16, overflowY: "auto", flex: 1 }}>
          ...
          <div style={{ background: "linear-gradient(135deg,#0f172a 0%,#1e293b 100%)", borderRadius: 4, padding: "14px 16px", border: "1px solid #334155" }}>
            ...
            <textarea value={aiPrompt} onChange={e => { setAiPrompt(e.target.value); setAiDraftDone(false); }}
              placeholder="e.g. Our key supplier may not deliver the integration module by go-live due to their financial difficulties..."
              rows={3}
              style={{ width: "100%", boxSizing: "border-box", padding: "10px 12px", fontFamily: T.body, fontSize: 13, color: "#f1f5f9", background: "rgba(255,255,255,0.07)", border: "1px solid #334155", borderRadius: 2, resize: "vertical", outline: "none" }}
            />
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 8 }}>
              <button onClick={handleAiDraft} disabled={aiDrafting || !aiPrompt.trim()}
                style={{ padding: "8px 18px", fontFamily: T.mono, fontSize: 10, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", background: aiDrafting ? "#334155" : "#6366f1", color: "#fff", border: "none", borderRadius: 2, cursor: aiDrafting || !aiPrompt.trim() ? "default" : "pointer", opacity: !aiPrompt.trim() ? 0.5 : 1 }}>
                {aiDrafting ? "Drafting..." : "AI Draft"}
              </button>
              {aiDraftDone && <span style={{ fontFamily: T.mono, fontSize: 10, color: "#4ade80" }}>Form filled - review below</span>}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
            ...
            <div><label style={LBL}>Impact</label><select value={impact} onChange={e => setImpact(e.target.value as any)} style={INP}>{["low","medium","high","critical"].map(v => <option key={v} value={v}>{v.charAt(0).toUpperCase()+v.slice(1)}</option>)}</select></div>
          </div>
          ...
          <div><label style={LBL}>Description *</label><textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Describe the risk/issue, its impact and context..." rows={3} style={{ ...INP, resize: "vertical", fontFamily: T.body, fontSize: 13, color: T.ink, background: "#fff" }} /></div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div>
              <label style={LBL}>Probability: {probability}%</label>
          ...
          <div style={{ background: "#f5f3f0", padding: "8px 12px", borderRadius: 2 }}>
            <span style={{ fontFamily: T.mono, fontSize: 10, color: T.ink3 }}>Risk Score: <strong style={{ color: T.ink }}>{Math.round((probability * severity) / 100)}</strong><span style={{ color: T.ink4 }}> = {probability}% x {severity}% / 100</span></span>
          </div>
          <div><label style={LBL}>Response Plan / Mitigation</label><textarea value={responsePlan} onChange={e => setResponsePlan(e.target.value)} placeholder="How will this be mitigated or managed..." rows={2} style={{ ...INP, resize: "vertical", fontFamily: T.body, fontSize: 13, color: T.ink, background: "#fff" }} /></div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div><label style={LBL}>Next Steps</label><textarea value={nextSteps} onChange={e => setNextSteps(e.target.value)} placeholder="Immediate actions to take..." rows={2} style={{ ...INP, resize: "vertical", fontFamily: T.body, fontSize: 13, color: T.ink, background: "#fff" }} /></div>
            <div><label style={LBL}>Notes</label><textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Additional context or comments..." rows={2} style={{ ...INP, resize: "vertical", fontFamily: T.body, fontSize: 13, color: T.ink, background: "#fff" }} /></div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <div><label style={LBL}>Due Date</label><input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} style={{ ...INP, colorScheme: "light" }} /></div>
          </div>
          {error && <div style={{ padding: "10px 14px", borderRadius: 2, background: RAG_CONFIG.R.bg, border: "1px solid " + RAG_CONFIG.R.border, fontFamily: T.mono, fontSize: 11, color: RAG_CONFIG.R.fg }}>{error}</div>}
        </div>
...
const SCOPES:   { v: ScopeParam;  l: string }[] = [{ v: "all", l: "All" }, { v: "window", l: "In Window" }, { v: "overdue", l: "Overdue" }];
const TYPES:    { v: TypeParam;   l: string }[] = [{ v: "all", l: "All" }, { v: "Risk", l: "Risk" }, { v: "Issue", l: "Issue" }, { v: "Assumption", l: "Assumption" }, { v: "Dependency", l: "Dependency" }];
const STATUSES: { v: StatusParam; l: string }[] = [{ v: "all", l: "All" }, { v: "open", l: "Open" }, { v: "in_progress", l: "In Progress" }, { v: "mitigated", l: "Mitigated" }, { v: "closed", l: "Closed" }];
const WINDOWS = [7, 14, 30, 60] as const;
const SORTS: { v: SortKey; l: string }[] = [{ v: "score", l: "Risk Score" }, { v: "sla", l: "SLA Threat" }, { v: "exposure", l: "Exposure" }, { v: "due_date", l: "Due Date" }, { v: "type", l: "Type" }];

/* - Main component - */

export default function RaidPortfolioClient({ defaultScope = "all", defaultWindow = 30 }: { defaultScope?: ScopeParam; defaultWindow?: 7 | 14 | 30 | 60 }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setData({ ok: false, error: String(e?.message ?? "Failed"), scope, windowDays: win, type, status, items: [], meta: { projectCount: 0, scope: "org", active_only: true } });
    } finally {
      setLoading(false);
    }
  ...
  const items = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter(i => `${i.title} ${i.project_title} ${i.owner_label} ${i.type}`.toLowerCase().includes(q));
    }
    return [...list].sort((a, b) => {
      ...
      if (sortKey === "due_date") { const [ad, bd] = [a.due_date ?? "9999", b.due_date ?? "9999"]; return ad < bd ? -1 : ad > bd ? 1 : 0; }
  ...
  const kpis = useMemo(() => {
    ...
    return {
      ...
      sym:      all.find(i => i.currency_symbol)?.currency_symbol ?? "GBP",
    };
  }, [data]);
    ...
    <>
      ...
      <div style={{ minHeight: "100vh", background: T.bg, fontFamily: T.mono, opacity: mounted ? 1 : 0, transition: "opacity 0.35s ease" }}>
        <div style={{ maxWidth: 1360, margin: "0 auto", padding: "40px 40px 100px" }}>
          ...
          <div style={{ borderBottom: `2px solid ${T.ink}`, paddingBottom: 22, marginBottom: 30, animation: "fadeUp 0.4s ease both" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <Link href="/insights" style={{ fontFamily: T.mono, fontSize: 10, color: T.ink4, textDecoration: "none", letterSpacing: "0.08em" }}>&larr; PORTFOLIO INTELLIGENCE</Link>
                <span style={{ color: T.ink5 }}>-</span>
                <Cap>RAID REGISTER</Cap>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <Mono size={10} color={T.ink5}>{nowUK()}</Mono>
                <button onClick={() => setShowModal(true)} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 16px", fontFamily: T.mono, fontSize: 10, fontWeight: 600, letterSpacing: "0.07em", textTransform: "uppercase", background: T.ink, color: "#fff", border: "none", borderRadius: 2, cursor: "pointer" }}>
                  <span style={{ fontSize: 14, lineHeight: 1 }}>+</span> Raise Risk / Issue
                </button>
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 32 }}>
              <div>
                <h1 style={{ fontFamily: T.serif, fontSize: 42, fontWeight: 700, margin: 0, letterSpacing: "-0.02em", lineHeight: 1.05, color: T.ink }}>RAID Portfolio Register</h1>
                <p style={{ fontFamily: T.body, fontSize: 14, color: T.ink3, marginTop: 8, fontWeight: 300, lineHeight: 1.5, maxWidth: 560 }}>
                  Consolidated risks, issues, assumptions and dependencies across all active projects.
                </p>
              </div>
              {data?.meta && (
          ...
          {kpis && (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", border: `1px solid ${T.hr}`, background: T.surface, marginBottom: 24, animation: "fadeUp 0.4s 0.08s ease both" }}>
              ...
              <div style={{ padding: "22px 28px" }}>
                ...
                <div style={{ fontFamily: T.serif, fontSize: 40, fontWeight: 700, lineHeight: 1, marginTop: 10, marginBottom: 6, color: kpis.exposure > 0 ? RAG_CONFIG.A.fg : T.ink4 }}>
          ...
          {/* Controls */}
          <div style={{ background: T.surface, border: `1px solid ${T.hr}`, padding: "14px 18px", marginBottom: 12, display: "flex", flexWrap: "wrap", gap: 16, alignItems: "center", animation: "fadeUp 0.4s 0.14s ease both" }}>
            ...
            <div style={{ display: "flex", gap: 3 }}>{TYPES.map(t => <Pill key={t.v} label={t.l} active={type === t.v} dotColor={t.v !== "all" ? TYPE_COLOR[t.v] : undefined} onClick={() => setType(t.v)} />)}</div>
            <div style={{ width: 1, height: 20, background: T.hr }} />
            <div style={{ display: "flex", gap: 3 }}>{STATUSES.map(s => <Pill key={s.v} label={s.l} active={status === s.v} onClick={() => setStatus(s.v)} />)}</div>
            <div style={{ width: 1, height: 20, background: T.hr }} />
            <div style={{ display: "flex", gap: 3 }}>{WINDOWS.map(w => <Pill key={w} label={`${w}D`} active={win === w} onClick={() => setWin(w)} />)}</div>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search title, project, owner..." style={{ marginLeft: "auto", padding: "6px 12px", fontFamily: T.mono, fontSize: 11, background: T.bg, border: `1px solid ${T.hr}`, borderRadius: 2, color: T.ink, width: 234 }} />
          </div>

          {/* Sort bar */}
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 14, animation: "fadeUp 0.4s 0.18s ease both" }}>
            <Cap>Sort:</Cap>
            <div style={{ display: "flex", gap: 3, marginLeft: 4 }}>{SORTS.map(s => <Pill key={s.v} label={s.l} active={sortKey === s.v} onClick={() => setSortKey(s.v)} />)}</div>
          ...
          {!loading && data && !data.ok && (
            <div style={{ background: RAG_CONFIG.R.bg, border: `1px solid ${RAG_CONFIG.R.border}`, borderLeft: `3px solid ${RAG_CONFIG.R.fg}`, padding: "18px 24px" }}>
              <Cap>Data Error</Cap>
              <p style={{ fontFamily: T.body, fontSize: 13, color: RAG_CONFIG.R.fg, margin: "8px 0 0" }}>{data.error}</p>
            </div>
          )}

          {/* Loading */}
          {loading && <div style={{ background: T.surface, border: `1px solid ${T.hr}`, padding: "64px", textAlign: "center" }}><Mono size={11} color={T.ink5}>RETRIEVING RAID DATA...</Mono></div>}

          {/* Table */}
          {!loading && data?.ok && (
            <div style={{ background: T.surface, border: `1px solid ${T.hr}`, overflow: "hidden", animation: "fadeUp 0.4s 0.22s ease both" }}>
              {items.length === 0 ? (
                <div style={{ padding: "64px", textAlign: "center" }}><Mono size={12} color={T.ink5}>No items match the current filters.</Mono></div>
              ) : (
                      ...
                      {items.map((item) => {
                        ...
                        return (
                          <React.Fragment key={item.id}>
                            <tr className="raid-row" onClick={() => setExpandedId(expanded ? null : item.id)} style={{ cursor: "pointer", background: expanded ? "#faf9f7" : T.surface }}>
                              <td style={{ width: 3, padding: 0 }}>
                                <div style={{ width: 3, minHeight: 58, background: rag === "N" ? "transparent" : rc.fg, opacity: rag === "G" ? 0.45 : 1 }} />
                              </td>
                              <td style={{ ...TD, minWidth: 310 }}>
                                <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                                  <TypeFlag type={item.type} />
                                  <div className="item-title" style={{ fontFamily: T.body, fontSize: 13.5, color: T.ink2, fontWeight: 400, lineHeight: 1.35, transition: "color 0.1s" }}>{item.title}</div>
                                  {item.owner_label && <Mono size={10} color={T.ink4}>&darr; {item.owner_label}</Mono>}
                                </div>
                              </td>
                              <td style={TD}><ScoreMeter score={item.score} /></td>
                              <td style={TD}>
                                {slr === "N" ? <Mono color={T.ink5}>-</Mono> : (
                                  <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                                      <Pip rag={slr} pulse={slr === "R"} />
                                      <Mono size={12} color={RAG_CONFIG[slr].fg} weight={600}>{item.sla_breach_probability != null ? `${item.sla_breach_probability}%` : "-"}</Mono>
                                    </div>
                                    {item.sla_days_to_breach != null && <Mono size={10} color={T.ink4}>~{item.sla_days_to_breach}d</Mono>}
                                  </div>
                                )}
                              </td>
                              <td style={TD}><Mono size={12} color={exposure > 500_000 ? RAG_CONFIG.A.fg : exposure > 0 ? T.ink3 : T.ink5} weight={exposure > 0 ? 600 : 400}>{fmtMoney(item.currency_symbol, exposure || null)}</Mono></td>
                              <td style={TD}>
                                <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                                  <Mono size={12} color={over ? RAG_CONFIG.R.fg : item.due_date ? T.ink3 : T.ink5} weight={over ? 600 : 400}>{item.due_date_uk ?? "-"}</Mono>
                                  {over && <Mono size={9} color={RAG_CONFIG.R.fg} weight={600} upper>Overdue</Mono>}
                                </div>
                              </td>
                              <td style={TD}><StatusChip status={item.status} /></td>
                              <td style={{ ...TD, maxWidth: 170 }}><div style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}><Mono size={11} color={T.ink4}>{item.project_title}</Mono></div></td>
                              <td style={{ ...TD, textAlign: "center" as const, paddingLeft: 6, paddingRight: 12 }}>
                                <span style={{ display: "inline-block", fontFamily: T.mono, fontSize: 11, color: T.ink4, transform: expanded ? "rotate(180deg)" : "none", transition: "transform 0.2s ease" }}>v</span>
                              </td>
                            </tr>
                            {expanded && (
                              <tr>
          ...
          {!loading && data?.ok && (
            <div style={{ marginTop: 24, display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 16, borderTop: `1px solid ${T.hr}` }}>
              <Mono size={10} color={T.ink5} upper>Org scope - active projects only - {data.meta.projectCount} projects - {items.length} items</Mono>
              <Mono size={10} color={T.ink5}>{nowUK()}</Mono>
            </div>
          )}
        </div>
      ...
      {showModal && <RaiseItemModal projects={raiseProjects} onClose={() => setShowModal(false)} onSuccess={() => fetchData()} />}
    </>
  );
}