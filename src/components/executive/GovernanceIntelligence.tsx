// src/components/executive/GovernanceIntelligence.tsx — v8 + UUID fix
...
function HeroStat({ label, value, sub, accent, accentGlow, numColor, delay=0, onClick }: {
  label: string; value: string|number; sub?: string; accent: string; accentGlow: string; numColor?: string; delay?: number; onClick?: () => void;
}) {
  return (
...
function AiOutlook({ counts, ai, aiLoading }: {
  ...
  // ✅ Scrub UUIDs from AI summary before display
  const rawText = ai?.summary || cfg.fallback;
  const text = scrubUuids(rawText);
  const rawRec = ai?.recommended;
  const rec = rawRec ? scrubUuids(rawRec) : null;
...
function HeatTile({ p, idx, expanded, onToggle }: { p: UiProject; idx: number; expanded: boolean; onToggle: () => void }) {
  ...
  return (
    <m.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{duration:0.38,delay:idx*0.055,ease:[0.16,1,0.3,1]}} onClick={onToggle}
      ...
      <div style={{padding:"14px 14px 14px 22px"}}>
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:8,alignItems:"center"}}>
          ...
          <span style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:3,fontSize:9,color:"#94a3b8"}}>{expanded?<ChevronUp size={12}/>:<ChevronDown size={12}/>}</span>
        </div>
  ...
  useEffect(() => {
    ...
    fetch(`/api/ai/governance-brain`, { method:"GET", cache:"no-store", credentials:"include", headers:{ Accept:"application/json", "Cache-Control":"no-store, no-cache" } })
      ...
      .then(j => {
        ...
        // ✅ Scrub UUIDs from the AI summary coming from the API
        ...
        const rec = overdue>0 ? `Escalate overdue approvals and unblock ${blocked} project(s).` : score<70 ? `Portfolio health is under pressure — review worst projects.` : blocked>0 ? `Resolve blockers to keep flow moving.` : `Governance flow is clear.`;
