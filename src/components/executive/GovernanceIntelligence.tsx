...
function appendFiltersToApi(baseUrl: string, f?: PortfolioFilters): string {
  try {
    const origin =
    ...
    (f?.projectCode ?? []).forEach((v) => sp.append("code", v));
    (f?.projectName ?? []).forEach((v) => sp.append("name", v));
...
function scrubUuids(text: string): string {
  ...
  let out = text.replace(
    /user:[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}/gi,
    "a team member"
  );
  out = out.replace(
    ...
    >
      <div
        style={{
          ...
          background:
            "linear-gradient(90deg,transparent,rgba(255,255,255,1) 28%,rgba(255,255,255,1) 72%,transparent)",
          pointerEvents: "none",
        }}
      />
      <div
...
function AiOutlook({
  ...
  ai: AiOutlookState | null;
  aiLoading: boolean;
}) {
  const urgency =
    counts.breached >= 3
      ? "critical"
      : counts.breached > 0 || counts.at_risk >= 3
        ? "elevated"
        : counts.at_risk > 0
          ? "moderate"
          : "clear";
  ...
  const cfg = {
    critical: {
      ...
      dot: "#f43f5e",
      badge: "Critical",
      badgeBg: "rgba(255,228,230,0.8)",
      badgeBorder: "rgba(252,165,165,0.5)",
      badgeColor: "#9f1239",
      fallback: `${counts.breached} approval${counts.breached !== 1 ? "s" : ""} breached SLA. Immediate escalation required.`,
    },
    elevated: {
      ...
      badgeColor: "#92400e",
      fallback: `${counts.at_risk} approval${counts.at_risk !== 1 ? "s" : ""} approaching SLA threshold.`,
    },
    moderate: {
      ...
      badgeBorder: "rgba(103,232,249,0.5)",
      badgeColor: "#0e7490",
      fallback: `${counts.at_risk} item${counts.at_risk !== 1 ? "s" : ""} approaching threshold.`,
    },
    clear: {
    ...
    >
      <div
        style={{
          ...
          boxShadow: `0 0 10px ${rc.accentGlow}`,
          pointerEvents: "none",
        }}
      />
      ...
      <div style={{ padding: "14px 14px 14px 22px" }}>
        ...
        >
          <span
            style={{
              ...
              border: `1px solid ${rc.badgeBorder}`,
              color: rc.badgeColor,
            }}
          >
            <span
        ...
        >
          {p.counts.overdue > 0 && (
            <m.div
              initial={{ width: 0 }}
              animate={{ width: `${(p.counts.overdue / tot) * 100}%` }}
              transition={{ duration: 0.8, delay: idx * 0.055 + 0.2 }}
              style={{
                height: "100%",
                background: rc.barColor,
                borderRadius: "4px 0 0 4px",
                boxShadow: rc.barGlow,
              }}
            />
          )}
          {p.counts.warn > 0 && (
            <m.div
              initial={{ width: 0 }}
              animate={{ width: `${(p.counts.warn / tot) * 100}%` }}
              transition={{ duration: 0.8, delay: idx * 0.055 + 0.3 }}
          ...
          {p.counts.ok > 0 && (
            <m.div
              initial={{ width: 0 }}
              animate={{ width: `${(p.counts.ok / tot) * 100}%` }}
              transition={{ duration: 0.8, delay: idx * 0.055 + 0.4 }}
              style={{
                height: "100%",
                background: "#34d399",
...
function BnTile({
  ...
  const heat: "high" | "medium" | "low" =
    b.max_wait_days > 14 ? "high" : b.max_wait_days > 7 ? "medium" : "low";
  ...
  return (
    ...
    >
      <m.div
        initial={{ width: 0 }}
        animate={{ width: `${pct}%` }}
        transition={{ duration: 0.9, delay: idx * 0.07 + 0.15 }}
        style={{
          position: "absolute",
          left: 0,
          top: 0,
...
export default function GovernanceIntelligence({
  ...
  const filterKey = useMemo(
    () =>
      JSON.stringify({
        q: filters?.q ?? "",
        projectCode: uniqStrings(filters?.projectCode ?? []),
        projectName: uniqStrings(filters?.projectName ?? []),
        projectManagerId: uniqStrings(filters?.projectManagerId ?? []),
        department: uniqStrings(filters?.department ?? []),
      }),
    [filters]
  );
      ...
      .then((j) => {
        ...
        setAppResp(
          j?.ok === false
            ...
            : { ok: true, items: Array.isArray(j?.items) ? j.items : [] }
        );
      })
      .catch(() => {
  ...
  }, [days, parentItems, filterKey, filters]);

  useEffect(() => {
    let dead = false;
    setAiLoading(true);

    const url = appendFiltersToApi(`/api/ai/governance-brain`, filters);
  ...
  const bottlenecks: UiBottleneck[] = useMemo(() => {
    ...
    for (const it of items) {
      ...
      let b = map.get(key);
      if (!b) {
        b = { kind, label, ages: [], projects: new Set(), count: 0 };
        map.set(key, b);
      }
  ...
  return (
    <LazyMotion features={domAnimation}>
      ...
      <DrillDrawer
        ...
        approverLabel={drawer.approverLabel}
        items={items}
              ...
              {[
                {
                  ...
                  color: "#64748b",
                  label: `${loading ? "—" : counts.pending} waiting`,
                  filter: "all" as DrawerFilter,
                },
                {
                  dot: "#f59e0b",
                  ...
                  border: "rgba(252,211,77,0.6)",
                  color: "#92400e",
                  label: `${loading ? "—" : counts.at_risk} at risk`,
                  filter: "at_risk" as DrawerFilter,
                },
                {
                  dot: "#f43f5e",
                  ...
                  color: "#9f1239",
                  label: `${loading ? "—" : counts.breached} breached`,
                  filter: "breached" as DrawerFilter,
                  pulse: true,
                },
              ].map((pill, i) => (
                <span
                  key={i}
                  onClick={() => setDrawer({ open: true, filter: pill.filter })}
                  style={{
                    display: "inline-flex",
                    alignItems: "center",
                    gap: 6,
