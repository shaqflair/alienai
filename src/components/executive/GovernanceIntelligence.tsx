...
import {
  ShieldCheck, Users, Layers, ArrowUpRight, X,
  AlertTriangle, CheckCircle2, Brain, Flame, Eye, Activity,
  Clock, ChevronDown, ChevronUp, FileText, Calendar,
} from "lucide-react";
...
function DrillDrawer({ open, filter, approverLabel, items, onClose }: {
  ...
  const title = filter === "breached" ? "SLA Breaches" : filter === "at_risk" ? "At Risk Approvals" : filter === "approver" ? `Approver: ${approverLabel}` : "All Pending Approvals";
...
function AiOutlook({ counts, ai, aiLoading }: { counts: { pending: number; at_risk: number; breached: number }; ai: AiOutlookState | null; aiLoading: boolean }) {
  const urgency = counts.breached >= 3 ? "critical" : counts.breached > 0 || counts.at_risk >= 3 ? "elevated" : counts.at_risk > 0 ? "moderate" : "clear";
  const cfg = {
  ...
  return (
    <div style={{ position: "relative", overflow: "hidden", borderRadius: 14, border: `1px solid ${cfg.border}`, background: cfg.bg, backdropFilter: "blur(16px)", padding: "14px 16px", marginBottom: 20 }}>
      ...
      <p style={{ fontSize: 12, color: "#475569", lineHeight: 1.6, margin: 0 }}>
        {aiLoading ? <span style={{ display: "inline-flex", gap: 4, alignItems: "center" }}>{[0, 150, 300].map(d => <span key={d} style={{ width: 6, height: 6, borderRadius: "50%", background: "#94a3b8", display: "inline-block", animation: "bounce 1s infinite", animationDelay: `${d}ms` }} />)}</span> : text}
        {!aiLoading && ai?.recommended && ai.recommended !== text && <span style={{ display: "block", marginTop: 4, color: "#64748b" }}>{ai.recommended}</span>}
      </p>
    </div>
  );
}
...
function HeatTile({ p, idx, expanded, onToggle }: { p: UiProject; idx: number; expanded: boolean; onToggle: () => void }) {
  const rc = RAG[p.rag]; const tot = Math.max(1, p.counts.total);
  return (
    <m.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div style={{ padding: "14px 14px 14px 22px" }}>
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 8, alignItems: "center" }}>
          ...
          {p.project_code && <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 9, fontWeight: 700, borderRadius: 6, padding: "2px 7px", background: "rgba(238,242,255,0.8)", border: "1px solid rgba(199,210,254,0.6)", color: "#4338ca" }}>{p.project_code}</span>}
        ...
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 8 }}>
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: "#94a3b8", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{[p.stage, p.approver_label && p.approver_label !== "—" ? `by ${p.approver_label}` : null].filter(Boolean).join(" · ")}</div>
          </div>
...
function BnTile({ b, maxCount, idx, onClick }: { b: UiBottleneck; maxCount: number; idx: number; onClick: () => void }) {
  ...
  const s = { high: { bg: "rgba(255,241,242,0.88)", border: "rgba(253,164,175,0.7)", fill: "#f43f5e", waitColor: "#e11d48" }, medium: { bg: "rgba(255,251,235,0.88)", border: "rgba(252,211,77,0.6)", fill: "#f59e0b", waitColor: "#d97706" }, low: { bg: "rgba(248,250,255,0.88)", border: "rgba(226,232,240,0.7)", fill: "#6366f1", waitColor: "#6366f1" } }[heat];
  return (
...
export default function GovernanceIntelligence({ days = 30, approvalItems: parentItems }: { days?: 7 | 14 | 30 | 60; approvalItems?: any[] }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/executive/approvals/pending?limit=200&days=${days}`, { cache: "no-store", credentials: "include", headers: { Accept: "application/json", "Cache-Control": "no-store, no-cache" } })
      .then(r => r.json()).then(j => { if (dead) return; setAppResp(j?.ok === false ? { ok: false, error: j.error } : { ok: true, items: Array.isArray(j?.items) ? j.items : [] }); })
  ...
  useEffect(() => {
    ...
    fetch("/api/ai/events", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json", "Cache-Control": "no-store" }, body: JSON.stringify({ eventType: "artifact_due", windowDays: days }) })
      .then(r => r.json()).then(j => { if (dead || !j?.ok) return; const aiBlock = j?.ai ?? {}; setAi({ summary: ss(aiBlock?.summary) || null, recommended: ss(aiBlock?.recommendedMessage) || null }); })
  ...
  const bottlenecks: UiBottleneck[] = useMemo(() => {
    ...
    return Array.from(map.values()).map(x => ({ kind: x.kind, label: x.label, pending_count: x.count, projects_affected: x.projects.size, avg_wait_days: Math.round((x.ages.reduce((a, n) => a + n, 0) / (x.ages.length || 1)) * 10) / 10, max_wait_days: x.ages.length ? Math.max(...x.ages) : 0 }))
          ...
          <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: 16, marginBottom: 20 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              {[
                ...
                <span key={i} onClick={() => setDrawer({ open: true, filter: pill.filter })} style={{ display: "inline-flex", alignItems: "center", gap: 6, borderRadius: 20, padding: "6px 12px", fontSize: 11, fontWeight: 700, background: pill.bg, border: `1px solid ${pill.border}`, color: pill.color, backdropFilter: "blur(10px)", cursor: "pointer" }}>
            ...
            <div>
              ...
              {loading ? (
                ...
                <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ borderRadius: 16, border: "1px dashed rgba(226,232,240,0.8)", background: "rgba(255,255,255,0.4)", backdropFilter: "blur(10px)", padding: "56px 24px", textAlign: "center" }}>
                  <div style={{ width: 56, height: 56, borderRadius: 16, background: "rgba(236,253,245,0.8)", border: "1px solid rgba(110,231,183,0.6)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}><CheckCircle2 size={28} color="#10b981" /></div>
            ...
            <div>
              ...
              {loading ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {bottlenecks.slice(0, 6).map((b, i) => <BnTile key={`${b.label}-${i}`} b={b} maxCount={maxBn} idx={i} onClick={() => setDrawer({ open: true, filter: "approver", approverLabel: b.label })} />)}
                </div>
              )}
