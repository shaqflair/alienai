// src/components/executive/GovernanceIntelligence.tsx — v9
...
//   ✅ Pass memberProjectIds (string[]) and isAdmin (boolean) from your server component
//      e.g. <GovernanceIntelligence memberProjectIds={memberProjectIds} isAdmin={isAdmin} />
...
interface UiBottleneck {
  kind: "user" | "email" | "unknown"; label: string; pending_count: number;
  projects_affected: number; avg_wait_days: number; max_wait_days: number;
}
...
function scrubUuids(text: string): string {
  if (!text) return text;
  let out = text.replace(/user:[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}/gi, "a team member");
  out = out.replace(/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}/gi, "a team member");
  return out;
}
...
function daysWaiting(it: ReturnType<typeof normalise>): number {
  ...
  if (it._ts) { const t = new Date(ss(it._ts)).getTime(); if (isFinite(t)) return Math.max(0, Math.round((Date.now() - t) / 86400000)); }
...
const CARD_STYLE: React.CSSProperties = {
  ...
  backdropFilter: "blur(28px) saturate(1.9)", borderRadius: 20,
};

const RAG: Record<Rag, { label: string; dot: string; badgeBg: string; badgeBorder: string; badgeColor: string; accent: string; accentGlow: string; tileBg: string; dayColor: string; barColor: string; barGlow?: string }> = {
  R: { label:"RED",   dot:"#f43f5e", badgeBg:"rgba(255,241,242,0.9)", badgeBorder:"rgba(253,164,175,0.6)", badgeColor:"#9f1239", accent:"#f43f5e", accentGlow:"rgba(244,63,94,0.4)",   tileBg:"linear-gradient(135deg,rgba(255,241,242,0.92),rgba(255,255,255,0.82))", dayColor:"#e11d48", barColor:"#fb7185", barGlow:"0 0 8px rgba(244,63,94,0.4)" },
  A: { label:"AMBER", dot:"#f59e0b", badgeBg:"rgba(255,251,235,0.9)", badgeBorder:"rgba(252,211,77,0.6)",  badgeColor:"#92400e", accent:"#f59e0b", accentGlow:"rgba(245,158,11,0.35)", tileBg:"linear-gradient(135deg,rgba(255,251,235,0.92),rgba(255,255,255,0.82))", dayColor:"#d97706", barColor:"#fbbf24" },
  G: { label:"GREEN", dot:"#10b981", badgeBg:"rgba(236,253,245,0.9)", badgeBorder:"rgba(110,231,183,0.6)", badgeColor:"#065f46", accent:"#10b981", accentGlow:"rgba(16,185,129,0.3)",  tileBg:"linear-gradient(135deg,rgba(255,255,255,0.96),rgba(248,250,255,0.88))", dayColor:"#374151", barColor:"#34d399" },
};

// ─── DRILL-DOWN DRAWER ────────────────────────────────────────────────────────

function DrillDrawer({
  open, filter, approverLabel, items, onClose, memberProjectIds, isAdmin,
}: {
  open: boolean; filter: DrawerFilter; approverLabel?: string;
  items: ReturnType<typeof normalise>[]; onClose: () => void;
  memberProjectIds: string[]; isAdmin: boolean;
}) {
  ...
  const title = filter === "breached" ? "SLA Breaches"
  ...
  function canAccess(it: ReturnType<typeof normalise>): boolean {
    ...
    const pid = ss(it?.project_id).trim();
    if (!pid || !looksLikeUuid(pid)) return true; // no project constraint
    return memberProjectIds.includes(pid);
  }

  function projectHref(it: ReturnType<typeof normalise>): string {
    const pid = ss(it?.project_id).trim();
    if (!pid) return "/approvals";
    return `/projects/${pid}`;
  }
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          <m.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ duration: 0.35, ease: [0.16, 1, 0.3, 1] }}
                  ...
                  {filtered.map((it, i) => {
                    ...
                    return (
                      <m.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.28, delay: i * 0.04 }}
                        ...
                        {/* ✅ Access-controlled Open button */}
                        {access ? (
                          <a href={href}
                            style={{ display: "inline-flex", alignItems: "center", gap: 6, borderRadius: 10, border: "1px solid rgba(226,232,240,0.8)", background: "rgba(255,255,255,0.8)", padding: "7px 14px", fontSize: 11, fontWeight: 700, color: "#475569", textDecoration: "none", cursor: "pointer" }}>
                            Open project <ArrowUpRight size={12} />
                          </a>
                        ) : (
                          <div style={{ display: "inline-flex", alignItems: "center", gap: 6, borderRadius: 10, border: "1px solid rgba(226,232,240,0.5)", background: "rgba(248,250,252,0.8)", padding: "7px 14px", fontSize: 11, fontWeight: 600, color: "#94a3b8", cursor: "not-allowed", userSelect: "none" }}>
                            <Lock size={11} /> No access — not a project member
                          </div>
                        )}
...
function HeroStat({ label, value, sub, accent, accentGlow, numColor, delay = 0, onClick }: {
  label: string; value: string | number; sub?: string; accent: string; accentGlow: string;
  numColor?: string; delay?: number; onClick?: () => void;
}) {
  return (
    <m.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} whileHover={onClick ? { y: -2 } : undefined}
      ...
      {onClick && <div style={{ position: "absolute", top: 10, right: 10, width: 20, height: 20, borderRadius: 6, background: "rgba(241,245,249,0.8)", border: "1px solid rgba(226,232,240,0.6)", display: "flex", alignItems: "center", justifyContent: "center" }}><ArrowUpRight size={10} color="#94a3b8" /></div>}
...
function HeatTile({
  ...
  p: UiProject; idx: number; expanded: boolean; onToggle: () => void;
  memberProjectIds: string[]; isAdmin: boolean;
}) {
  const rc = RAG[p.rag];
  const tot = Math.max(1, p.counts.total);

  const hasAccess = isAdmin || memberProjectIds.includes(p.project_id);

  return (
    <m.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.38, delay: idx * 0.055, ease: [0.16, 1, 0.3, 1] }}
      onClick={onToggle}
      ...
      <div style={{ padding: "14px 14px 14px 22px" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 8, alignItems: "center" }}>
          ...
          {/* Access badge */}
          {!hasAccess && (
            <span style={{ display: "inline-flex", alignItems: "center", gap: 3, borderRadius: 6, padding: "2px 6px", fontSize: 9, fontWeight: 700, background: "rgba(241,245,249,0.8)", border: "1px solid rgba(226,232,240,0.7)", color: "#94a3b8", marginLeft: "auto" }}>
              <Lock size={9} /> view only
            </span>
          )}
          <span style={{ marginLeft: hasAccess ? "auto" : 0, display: "flex", alignItems: "center", gap: 3, fontSize: 9, color: "#94a3b8" }}>
          ...
          {expanded && (
            <m.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} style={{ overflow: "hidden" }}>
              <div style={{ marginTop: 14, borderTop: "1px solid rgba(226,232,240,0.6)", paddingTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                {/* Open project button — access-controlled */}
                <div style={{ marginTop: 4 }}>
                  {hasAccess ? (
                    <a href={`/projects/${p.project_id}`}
                      onClick={(e) => e.stopPropagation()}
                      style={{ display: "inline-flex", alignItems: "center", gap: 6, borderRadius: 10, border: "1px solid rgba(226,232,240,0.8)", background: "rgba(255,255,255,0.8)", padding: "7px 14px", fontSize: 11, fontWeight: 700, color: "#475569", textDecoration: "none" }}>
                      Open project <ArrowUpRight size={12} />
                    </a>
                  ) : (
                    <div style={{ display: "inline-flex", alignItems: "center", gap: 6, borderRadius: 10, border: "1px solid rgba(226,232,240,0.5)", background: "rgba(248,250,252,0.8)", padding: "7px 14px", fontSize: 11, fontWeight: 600, color: "#94a3b8", cursor: "not-allowed", userSelect: "none" }}>
                      <Lock size={11} /> Not a project member
                    </div>
                  )}
                </div>
              </div>
  ...
  return (
    <m.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} whileHover={{ y: -1 }}
      ...
      <div style={{ position: "relative", padding: "12px 14px" }}>
        ...
        {heat === "high" && <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 6, fontSize: 10, fontWeight: 600, color: "#e11d48" }}><Flame size={12} /> Max wait: {b.max_wait_days}d</div>}
      </div>
    </m.div>
  );
}
...
export default function GovernanceIntelligence({
  ...
  useEffect(() => {
    ...
    let dead = false;
    fetch(`/api/executive/approvals/pending?limit=200&days=${days}`, { cache: "no-store", credentials: "include", headers: { Accept: "application/json", "Cache-Control": "no-store, no-cache" } })
      .then(r => r.json())
  ...
  useEffect(() => {
    ...
    fetch(`/api/ai/governance-brain`, { method: "GET", cache: "no-store", credentials: "include", headers: { Accept: "application/json", "Cache-Control": "no-store, no-cache" } })
      .then(r => r.json())
      .then(j => {
        if (dead || !j?.ok) return;
        ...
        const rec = overdue > 0 ? `Escalate overdue approvals and unblock ${blocked} project(s).`
          : score < 70 ? `Portfolio health is under pressure — review worst projects.`
          : blocked > 0 ? `Resolve blockers to keep flow moving.`
  ...
  return (
    <LazyMotion features={domAnimation}>
      ...
      <DrillDrawer
        open={drawer.open} filter={drawer.filter} approverLabel={drawer.approverLabel}
        items={items} onClose={() => setDrawer(d => ({ ...d, open: false }))}
        memberProjectIds={memberProjectIds} isAdmin={isAdmin}
      />

      {/* Hero stats — collapsible */}
      {showHero && (
        <div style={{ marginBottom: 20 }}>
          <button onClick={() => setHeroCollapsed(v => !v)}
            style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%", marginBottom: heroCollapsed ? 0 : 12, padding: "8px 4px", background: "none", border: "none", cursor: "pointer", color: "#64748b" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <AnimatePresence>
            {!heroCollapsed && (
              <m.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }} style={{ overflow: "hidden" }}>
      ...
      <m.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, ease: [0.16, 1, 0.3, 1] }} style={{ ...CARD_STYLE, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", padding: 24 }}>
          ...
          {error && <div style={{ marginBottom: 16, borderRadius: 14, border: "1px solid rgba(253,164,175,0.7)", background: "rgba(255,241,242,0.72)", padding: "12px 16px", fontSize: 13, color: "#9f1239", display: "flex", alignItems: "center", gap: 8 }}><AlertTriangle size={16} /> {error}</div>}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 20 }}>
            {/* Heatmap */}
            <div>
              <SectionLabel color="#6366f1" glow="rgba(99,102,241,0.5)" text="Portfolio Approval Heatmap" right={!loading && projects.length > 0 ? `${projects.length} stalled — click to expand` : undefined} />
              {loading ? (
