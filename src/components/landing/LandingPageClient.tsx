...
function trackEvent(name: string, payload: TrackPayload = {}) {
  ...
  if (process.env.NODE_ENV !== "production") {
    // Helpful during local validation
...
function Logo({ size = "md" }: { size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: s.gap }}>
      ...
      >
        {L.map((l, i) => (
          <span key={i} style={{ color: l.a ? T.cyan : "inherit" }}>
            {l.ch}
          </span>
        ))}
      </span>
...
const GNODES: GN[] = [
  ...
  { id: "ai", x: 58, y: 73, label: "AI Governance", sub: "Insight Layer", color: T.purple, health: 99, Icon: Cpu },
  { id: "reporting", x: 50, y: 90, label: "Executive View", sub: "Unified View", color: T.cyan, health: 100, Icon: Activity },
];
...
function GovernanceGraph() {
  ...
  const trackedNodeRef = useRef<Set<string>>(new Set());
  ...
  const handleSelect = useCallback(
    (nodeId: string) => {
      setSel((current) => {
        ...
        if (next && !trackedNodeRef.current.has(next)) {
          trackedNodeRef.current.add(next);
          trackEvent("landing_graph_node_open", { node_id: next });
        }
        return next;
      });
    },
    ...
    <div style={{ position: "relative", width: "100%", height: "100%", minHeight: 480 }}>
      <div
        ...
        style={{
          ...
          opacity: 0.06,
          backgroundImage:
            "linear-gradient(rgba(0,194,232,.55) 1px,transparent 1px),linear-gradient(90deg,rgba(0,194,232,.55) 1px,transparent 1px)",
      ...
      <svg viewBox="0 0 800 500" style={{ width: "100%", height: "100%", overflow: "visible" }} preserveAspectRatio="xMidYMid meet">
        ...
        {EDGES.map(([a, b]) => {
          const s = nm[a];
          const e = nm[b];
          const hi = hov === a || hov === b || sel === a || sel === b;
          return (
            <line
              key={`${a}-${b}`}
              x1={nx(s.x)}
              ...
              x2={nx(e.x)}
              y2={ny(e.y)}
              stroke={hi ? T.cyan : "url(#ge)"}
              strokeWidth={hi ? 1.6 : 0.85}
              strokeDasharray="4 4"
              opacity={hi ? 1 : 0.42}
              markerEnd="url(#ga)"
            />
          ...
          return (
            <circle key={`p-${a}-${b}`} r={toAi ? 3.5 : 2.5} fill={toAi ? T.purple : T.cyanLt} filter="url(#gp)" opacity="0">
              <animateMotion
                ...
                path={`M${nx(s.x)},${ny(s.y)} L${nx(e.x)},${ny(e.y)}`}
              />
              <animate
        ...
        {GNODES.map((node) => {
          ...
          const y = ny(node.y);
          const ai = node.id === "ai";
          const rep = node.id === "reporting";
          const chg = node.id === "change";
          ...
          return (
            ...
            >
              ...
              {rep && (
                <circle r="34" fill="none" stroke={T.cyan} strokeWidth="1.25" opacity="0.22">
              ...
              {chg && (
                <circle r="30" fill="none" stroke={T.orange} strokeWidth="1" opacity="0.25">
              ...
              {(hi || sc) && (
                <circle r="34" fill="none" stroke={node.color} strokeWidth="1.5" opacity="0.35" filter="url(#gg)" />
              )}

              <circle
                r="22"
                fill="none"
                stroke={hcol(node.health)}
                strokeWidth="2.5"
              ...
              <text y="-28" textAnchor="middle" fill={hcol(node.health)} fontSize="9" fontWeight="600" fontFamily={F.mono}>
                {node.health}%
              </text>
              ...
              <text y="34" textAnchor="middle" fill={T.text} fontSize="10" fontWeight="600" fontFamily={F.display}>
                {node.label}
              </text>
              <text y="45" textAnchor="middle" fill={T.muted2} fontSize="8" fontFamily={F.mono}>
                {node.sub}
              </text>
            </g>
          );
      ...
      {sn && (
        ...
        >
          ...
          <div style={{ textAlign: "right", fontSize: 11, color: hcol(sn.health), fontFamily: F.mono, marginTop: 5 }}>
            {sn.health}%
          </div>
        </div>
      )}
      ...
      >
        <div style={{ fontFamily: F.mono, fontSize: 9, color: T.muted2, marginBottom: 7, letterSpacing: "0.12em" }}>
          SIGNAL LEGEND
        </div>
        {[
          [T.green, "Healthy (80%+)"],
          [T.amber, "Warning (60-79%)"],
          [T.red, "Critical (<60%)"],
        ].map(([c, l]) => (
          <div key={String(l)} style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 4, fontSize: 10, color: T.muted }}>
...
export default function LandingPageClient() {
  ...
  const trackCta = useCallback((ctaName: string, location: string, href?: string) => {
  ...
  useEffect(() => {
    ...
    const sections = [
      { id: "hero", ref: heroRef },
      { id: "built_for", ref: builtForRef },
      { id: "proof", ref: proofRef },
      { id: "platform", ref: platformRef },
      { id: "intelligence", ref: intelligenceRef },
      { id: "how_it_works", ref: howRef },
      { id: "outcomes", ref: outcomesRef },
      { id: "final_cta", ref: ctaRef },
    ];
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          ...
          if (!sectionId || !entry.isIntersecting || trackedSectionsRef.current.has(sectionId)) return;

          trackedSectionsRef.current.add(sectionId);
          trackEvent("landing_section_view", { section_id: sectionId });
        });
      },
      { threshold: 0.45 }
    );
  ...
  const audiences = [
    {
      ...
      desc: "Portfolio-level visibility, governance discipline and leadership-ready reporting across complex delivery estates.",
    },
    {
      Icon: Shield,
      title: "Public Sector & Regulated Delivery",
      desc: "Accountability, decision traceability and structured oversight without adding operational drag.",
    },
    {
      Icon: TrendingUp,
      title: "Transformation Leaders",
      desc: "One AI-powered control layer spanning risks, approvals, milestones, commercials and resourcing.",
    },
  ];
  ...
  const howItWorks = [
    {
      ...
      desc: "Approvals, RAID, finance, milestones and resources are brought into one governed operating picture.",
    },
    {
      title: "Surface risk",
      desc: "AI highlights delivery pressure and governance bottlenecks before teams slip into reactive mode.",
    },
    {
      title: "Drive decisions",
      desc: "Leadership acts on live intelligence, not delayed reporting assembled after the fact.",
    },
    {
      title: "Strengthen governance",
      desc: "Every decision is tracked, auditable and controlled across the delivery estate.",
    },
  ];
  ...
  return (
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        @media(max-width:1100px){
          ...
          .hero-grid,.p-grid,.t-grid,.ont-grid,.two-grid,.how-grid{grid-template-columns:1fr!important}
      ...
      >
        ...
        >
          ...
          <div className="nav-links" style={{ display: "flex", alignItems: "center", gap: 28 }}>
            {[
              ...
              <a
                key={l}
                href={h}
                style={{ fontSize: 13, color: T.muted, fontWeight: 500, transition: "color 0.2s" }}
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <a href="/login" className="al-btn al-btn-g" onClick={() => trackCta("sign_in", "nav", "/login")}>
              Sign in
            </a>
            <a
              href={withUtm("mailto:support@aliena.co.uk", "nav_demo")}
              className="al-btn al-btn-p"
              onClick={() => trackCta("book_demo", "nav", "mailto:support@aliena.co.uk")}
            >
              Book a demo
            </a>
          </div>
        ...
        <div className="al-shell" style={{ position: "relative", zIndex: 2 }}>
          <div className="hero-grid" style={{ minHeight: "calc(100vh - 88px)", padding: "60px 0" }}>
            <div>
              ...
              >
                Aliena brings approvals, RAID, finance, milestones, resources, and executive
                reporting into one AI-powered governance platform for PMOs, delivery leaders,
                and regulated organisations.
              </p>

              <div className="al-f4 hero-actions" style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 22 }}>
                <a
                  href={withUtm("mailto:support@aliena.co.uk", "hero_demo")}
                  className="al-btn al-btn-p al-btn-lg"
                  onClick={() => trackCta("book_demo", "hero", "mailto:support@aliena.co.uk")}
                >
                  Book a demo <ArrowRight size={16} />
                </a>
                <a
                  href="#intelligence"
                  className="al-btn al-btn-o al-btn-lg"
                  onClick={() => trackCta("explore_governance_intelligence", "hero", "#intelligence")}
                >
              ...
              <div className="al-f5" style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 28 }}>
                {["Live governance view", "AI risk signals", "Approval control", "Audit-ready workflows"].map((p) => (
            ...
            <div className="hero-r" style={{ position: "relative", height: 620 }}>
              ...
              >
                ...
                >
                  ...
                  <span style={{ fontFamily: F.mono, fontSize: 10, color: T.muted2, letterSpacing: "0.08em" }}>
                  ...
                  <div style={{ display: "grid", gap: 10, marginBottom: 14 }}>
                    {[
                      ...
                      >
                        ...
                        <span style={{ fontSize: 11, color: T.text, textAlign: "right", fontFamily: F.mono }}>
                          {v}%
                        </span>
                      </div>
                    ))}
              ...
              >
                <div style={{ fontFamily: F.mono, fontSize: 10, color: T.purple, letterSpacing: "0.1em", marginBottom: 8 }}>
                  AI INSIGHT
                </div>
                <div style={{ fontFamily: F.display, fontSize: 15, fontWeight: 600, marginBottom: 8 }}>
      ...
      >
        <div className="al-shell" style={{ textAlign: "center" }}>
          <div className="al-kicker">Built for</div>
          <div style={{ display: "flex", justifyContent: "center", flexWrap: "wrap", gap: 12, marginTop: 12 }}>
            {["Enterprise PMOs", "Regulated delivery", "Transformation portfolios"].map((i) => (
              <span key={i} className="chip" style={{ fontSize: 13, padding: "10px 16px" }}>
                {i}
              </span>
            ))}
          </div>
      ...
      >
        <div className="al-shell" style={{ display: "flex", flexWrap: "wrap", gap: 10, justifyContent: "center" }}>
          {["Built in the UK", "Security-first architecture", "Audit-grade workflows", "Early access pilot available"].map((i) => (
        ...
        <div className="al-shell">
          ...
          <div
            style={{
              padding: "30px 32px",
              borderRadius: 24,
              border: "1px solid rgba(0,194,232,.14)",
              background: "linear-gradient(135deg,rgba(0,194,232,.08) 0%,rgba(255,255,255,.02) 100%)",
              display: "flex",
              justifyContent: "space-between",
              alignItems: "center",
              gap: 20,
          ...
          >
            <div style={{ maxWidth: 760 }}>
              ...
              <div style={{ color: T.muted, fontSize: 15, lineHeight: 1.8 }}>
                One control layer for programme oversight, one source of truth for governance,
                and one AI brain to help leaders act before issues escalate.
              </div>
            </div>
            <a
              href="#platform"
              className="al-btn al-btn-p al-btn-lg"
              onClick={() => trackCta("explore_platform", "problem_band", "#platform")}
            >
      ...
      <section
        id="platform"
        ref={platformRef}
        style={{ position: "relative", padding: "108px 0", overflow: "hidden", background: "#020507" }}
      >
        ...
        <div
          ...
          style={{
            ...
            zIndex: 0,
            backgroundImage:
              "repeating-linear-gradient(0deg,transparent,transparent 38px,rgba(0,194,232,.03) 38px,rgba(0,194,232,.03) 39px),repeating-linear-gradient(60deg,transparent,transparent 38px,rgba(0,194,232,.025) 38px,rgba(0,194,232,.025) 39px)",
          ...
          <div className="p-grid">
            {pillars.map((p) => (
              <div
                ...
                onMouseEnter={(e) => {
                  ...
                  el.style.borderColor = "rgba(0,194,232,.22)";
                  el.style.transform = "translateY(-3px)";
                  el.style.boxShadow = "0 24px 60px rgba(0,0,0,.28)";
                }}
                onMouseLeave={(e) => {
                  const el = e.currentTarget as HTMLDivElement;
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    right: 0,
                    height: 1,
                    background: "linear-gradient(90deg,transparent,rgba(87,231,255,.5),transparent)",
                  }}
                />
                <div style={{ fontFamily: F.mono, fontSize: 11, color: T.cyanLt, marginBottom: 14, letterSpacing: "0.1em" }}>
                  {p.k}
                </div>
                <div style={{ marginBottom: 16, color: T.cyan }}>
                  <p.Icon size={22} />
                </div>
                <div style={{ fontFamily: F.display, fontSize: 24, letterSpacing: "-0.04em", marginBottom: 10 }}>
                  {p.title}
                </div>
            ...
            <div
              style={{
                padding: 30,
                borderRadius: 22,
                border: "1px solid rgba(0,194,232,.15)",
                background: "linear-gradient(135deg,rgba(0,194,232,.07) 0%,rgba(255,255,255,.02) 100%)",
                display: "flex",
            ...
            >
              ...
              <div style={{ fontFamily: F.display, fontSize: 23, fontWeight: 700, letterSpacing: "-0.03em" }}>
                Ready to see it live?
              </div>
              <div style={{ color: T.muted, fontSize: 14, lineHeight: 1.7, maxWidth: 230 }}>
                Book a leadership demo and see Aliena in your delivery context.
              </div>
              <a
                href={withUtm("mailto:support@aliena.co.uk", "platform_demo")}
                className="al-btn al-btn-p"
                style={{ marginTop: 4 }}
                onClick={() => trackCta("book_demo", "platform_card", "mailto:support@aliena.co.uk")}
              >
      ...
      <section
        ...
        style={{
          ...
          padding: "110px 0",
          overflow: "hidden",
          background: "radial-gradient(circle at 20% 20%,rgba(0,194,232,.06) 0%,transparent 30%),linear-gradient(180deg,#020408 0%,#03060B 100%)",
        }}
      >
        <div
          aria-hidden
          style={{
            ...
            maskImage: "linear-gradient(to bottom,rgba(0,0,0,.65),rgba(0,0,0,.08))",
            WebkitMaskImage: "linear-gradient(to bottom,rgba(0,0,0,.65),rgba(0,0,0,.08))",
            pointerEvents: "none",
          }}
        />
          ...
          <div className="ont-grid">
            <div
              style={{
                borderRadius: 28,
                border: `1px solid ${T.lineS}`,
                background: "linear-gradient(180deg,rgba(10,14,22,.96) 0%,rgba(6,9,15,.92) 100%)",
            ...
            >
              <div>
                ...
                >
                  One platform.
                  <br />
                  <span className="tg">Total governance.</span>
                </h2>
                ...
                <div style={{ display: "grid", gap: 12, marginBottom: 24 }}>
                  {[
                    { title: "Live governance graph", desc: "Every node contributes to one connected delivery model.", color: T.cyan },
                    { title: "Health scoring", desc: "Critical pressure points are surfaced automatically.", color: T.green },
                    { title: "AI synthesis", desc: "Signals converge into one intelligence layer for action.", color: T.purple },
                    { title: "Change control", desc: "Every scope change is tracked, costed and approved before it moves.", color: T.amber },
                  ].map((item) => (
                    <div
                      key={item.title}
                      style={{
              ...
              <div style={{ borderTop: `1px solid ${T.line}`, paddingTop: 18 }}>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 16 }}>
                  {[
                    ...
                    >
                      <div style={{ fontFamily: F.display, fontSize: 22, fontWeight: 700, lineHeight: 1, color: T.text, marginBottom: 6 }}>
                        {m.v}
                      </div>
                      <div
                        style={{
                ...
                <a
                  ...
                  onClick={() => trackCta("see_outcomes", "intelligence_panel", "#outcomes")}
                >
            ...
            <div
              style={{
                ...
                borderRadius: 30,
                border: `1px solid ${T.lineS}`,
                background: "linear-gradient(180deg,rgba(8,12,20,.96) 0%,rgba(5,8,14,.94) 100%)",
            ...
            >
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 12, minWidth: 0 }}>
                  <div
                    style={{
                      ...
                      height: 34,
                      borderRadius: 12,
                      background: "linear-gradient(135deg,rgba(0,194,232,.18),rgba(168,85,247,.18))",
                      border: `1px solid ${T.line}`,
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                ...
                >
                  ...
                  />
                  <span style={{ fontFamily: F.mono, fontSize: 10, color: "#86EFAC", letterSpacing: "0.08em", textTransform: "uppercase" }}>
                    Live
                  </span>
                </div>
              </div>
              ...
              <div style={{ padding: 20, position: "relative" }}>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 12, marginBottom: 16 }}>
                  {[
                    { label: "AI signals", value: "27", tone: T.purple },
                    { label: "At-risk areas", value: "3", tone: T.amber },
                    { label: "Governed flow", value: "84%", tone: T.cyan },
                    { label: "Change requests", value: "6", tone: T.orange },
                  ].map((item) => (
                    <div
                      key={item.label}
                      style={{
                ...
                <div
                  style={{
                    borderRadius: 24,
                    border: `1px solid ${T.line}`,
                    background: `radial-gradient(circle at 50% 20%,rgba(0,194,232,.05) 0%,transparent 30%),rgba(4,8,14,.8)`,
                    padding: 16,
                    minHeight: 500,
                  }}
                >
      ...
      {/* HOW IT WORKS */}
      <section ref={howRef} style={{ padding: "100px 0", background: "#020507" }}>
        <div className="al-shell">
          <div className="al-kicker">How it works</div>
          <h2 className="al-h2" style={{ maxWidth: 720 }}>
            From signals to decisions.
            <br />
            <span className="tg">In one flow.</span>
          </h2>

          <div className="how-grid" style={{ marginTop: 40 }}>
            {howItWorks.map((step) => (
              <div key={step.title} className="al-card" style={{ padding: 24 }}>
                <div style={{ fontFamily: F.display, fontSize: 18, marginBottom: 8 }}>{step.title}</div>
                <div style={{ fontSize: 13, color: T.muted, lineHeight: 1.7 }}>{step.desc}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      >
        <div
          ...
          style={{
            ...
            width: 800,
            height: 400,
            background: `radial-gradient(ellipse,rgba(0,194,232,.05) 0%,transparent 70%)`,
            pointerEvents: "none",
          }}
        />
        <div className="al-shell" style={{ position: "relative", zIndex: 1 }}>
          ...
          <h2 className="al-h2">
            Better decisions.
            <br />
            <span className="tg">Earlier intervention.</span>
          </h2>
          <p className="al-sub" style={{ marginBottom: 40 }}>
            The goal is not more dashboards. The goal is better control, earlier intervention and more confident delivery.
          </p>

          <div className="f-grid">
            {outcomes.map((o) => (
              <div key={o.label} className="al-card" style={{ padding: "28px 22px", textAlign: "center" }}>
                <div className="tg" style={{ fontFamily: F.display, fontSize: 42, fontWeight: 800, lineHeight: 1, marginBottom: 10 }}>
                  {o.value}
                </div>
      ...
      <section style={{ padding: "104px 0", background: "#020507", position: "relative", overflow: "hidden" }}>
        ...
        <div className="al-shell" style={{ position: "relative", zIndex: 1 }}>
          ...
          <h2 className="al-h2">
            ...
            <br />
            <span className="tg">Aliena interprets.</span>
          </h2>
          <p className="al-sub" style={{ marginBottom: 40 }}>
            From system of record to system of intelligence. That is the category shift.
          </p>
          ...
          <div className="two-grid">
            ...
            <div
              style={{
                padding: "28px 26px",
                borderRadius: 22,
                border: "1px solid rgba(0,194,232,.18)",
                background: "linear-gradient(135deg,rgba(0,194,232,.06) 0%,rgba(255,255,255,.02) 100%)",
              }}
            >
              <div
                style={{
              ...
              <div style={{ display: "grid", gap: 14 }}>
                {[
                  ...
                  "Change control tracked and costed before it moves",
                ].map((t) => (
                  <div key={t} style={{ display: "flex", alignItems: "flex-start", gap: 10, fontSize: 14, lineHeight: 1.6 }}>
                    <CheckCircle2 size={16} color={T.green} style={{ flexShrink: 0, marginTop: 2 }} />
                    {t}
                  </div>
                ))}
              </div>
      ...
      <section style={{ padding: "104px 0", background: "linear-gradient(180deg,#04060C 0%,#050810 100%)" }}>
        <div className="al-shell">
          ...
          <h2 className="al-h2">
            ...
            <br />
            <span className="tg">governance and delivery</span> both matter.
          </h2>
          <p className="al-sub" style={{ marginBottom: 40 }}>
            Aliena is strongest where complexity, accountability and executive visibility are all non-negotiable.
          </p>
      ...
      <section style={{ padding: "72px 0", background: "#020407", position: "relative", overflow: "hidden" }}>
        <div
          ...
          style={{
            ...
            right: 0,
            height: 1,
            background: `linear-gradient(90deg,transparent,${T.lineCyan},transparent)`,
          }}
        />
        <div className="al-shell">
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 40, flexWrap: "wrap" }}>
            <div style={{ maxWidth: 560 }}>
              ...
              <p style={{ color: T.muted, fontSize: 15, lineHeight: 1.8 }}>
                Row-level security, governed approval workflows, audit-grade decision trails and UK-built infrastructure.
            ...
            <div style={{ display: "grid", gap: 12, minWidth: 280 }}>
              {[
                { Icon: Lock, label: "Row-level security enforced at database level" },
                { Icon: Shield, label: "Governed approval workflows with full traceability" },
                { Icon: CheckCircle2, label: "UK GDPR compliant, ISO 27001 in progress" },
              ].map((item) => (
                <div
                  key={item.label}
                  style={{
              ...
              <a
                ...
                onClick={() => trackCta("view_security_details", "security_strip", "/security")}
              >
      ...
      {/* CTA */}
      <section ref={ctaRef} style={{ padding: "110px 0", background: "#04060C" }}>
        <div className="al-shell">
          <div
            style={{
              borderRadius: 30,
              border: "1px solid rgba(0,194,232,.15)",
              background: "radial-gradient(ellipse at top center,rgba(0,194,232,.09) 0%,transparent 50%),rgba(255,255,255,.025)",
              padding: "64px 32px",
              textAlign: "center",
              boxShadow: "0 0 80px rgba(0,194,232,.05)",
            }}
          >
            <div
              style={{
            ...
            <div style={{ display: "flex", justifyContent: "center", gap: 12, flexWrap: "wrap" }}>
              <a
                href={withUtm("mailto:support@aliena.co.uk", "final_demo")}
                className="al-btn al-btn-p al-btn-lg"
                onClick={() => trackCta("book_demo", "final_cta", "mailto:support@aliena.co.uk")}
              >
                Book a demo <ArrowRight size={16} />
              </a>
              <a
                href="#how-it-works"
                className="al-btn al-btn-g al-btn-lg"
                onClick={() => trackCta("view_how_it_works", "final_cta", "#how-it-works")}
              >
                View how it works
              </a>
            </div>
      ...
      <footer style={{ borderTop: "1px solid rgba(255,255,255,.06)", background: "#02040A", padding: "44px 0 52px" }}>
        <div className="al-shell">
            ...
            <div>
              ...
              <div style={{ marginTop: 14 }}>
                <a
                  ...
                  style={{ fontSize: 12, color: T.muted, transition: "color 0.2s" }}
                  onMouseEnter={(e) => (e.currentTarget.style.color = T.text)}
                  onMouseLeave={(e) => (e.currentTarget.style.color = T.muted)}
                  onClick={() => trackCta("email_support", "footer", "mailto:support@aliena.co.uk")}
                >
              ...
              <div style={{ display: "grid", gap: 10 }}>
                {[
                  ...
                  <a
                    key={l}
                    href={h}
                    style={{ fontSize: 13, color: T.muted, transition: "color 0.2s" }}
                    onMouseEnter={(e) => (e.currentTarget.style.color = T.text)}
                    onMouseLeave={(e) => (e.currentTarget.style.color = T.muted)}
                    onClick={() => trackCta(String(l).toLowerCase().replace(/\s+/g, "_"), "footer_product", String(h))}
                  >
                    {l}
                  </a>
                ))}
            ...
            <div>
              ...
              <div style={{ display: "grid", gap: 10 }}>
                {[
                  ...
                  ["Contact", "mailto:support@aliena.co.uk"],
                ].map(([l, h]) => (
                  <a
                    key={l}
                    href={h}
                    ...
                    onMouseEnter={(e) => (e.currentTarget.style.color = T.text)}
                    onMouseLeave={(e) => (e.currentTarget.style.color = T.muted)}
                    onClick={() => trackCta(String(l).toLowerCase().replace(/\s+/g, "_"), "footer_legal", String(h))}
                  >
                    {l}
                  </a>
                ))}
          ...
          >
            <span style={{ fontSize: 12, color: T.muted2, fontFamily: F.mono }}>
              &#169; 2026 Aliena AI. Built in the UK.
            </span>
            <div style={{ display: "flex", gap: 20 }}>
              <a
                href="/security"
                style={{ fontSize: 12, color: T.muted2, transition: "color 0.2s" }}
                onMouseEnter={(e) => (e.currentTarget.style.color = T.muted)}
                onMouseLeave={(e) => (e.currentTarget.style.color = T.muted2)}
                onClick={() => trackCta("security", "footer_bottom", "/security")}
              >
                Security
              </a>
              <a
                ...
                onMouseEnter={(e) => (e.currentTarget.style.color = T.muted)}
                onMouseLeave={(e) => (e.currentTarget.style.color = T.muted2)}
                onClick={() => trackCta("privacy", "footer_bottom", "/privacy")}
              >
                Privacy
              </a>
            </div>
