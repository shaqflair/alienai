      ...
      >
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-black text-slate-900" style={{ letterSpacing: "-0.01em" }}>
                Executive Briefing
              </div>
              <div className="mt-0.5 text-xs text-slate-400">
                AI-generated portfolio narrative · Ready
              </div>
            </div>
          </div>
        </div>
...
const KpiCard = memo(function KpiCard({
  ...
  return (
    ...
    >
      ...
      <div className={["text-4xl font-bold leading-none tracking-tight", t.valueColor].join(" ")}>
        {value}
      </div>
      ...
      <div className="flex w-32 shrink-0 items-center gap-2.5">
        <div className="h-2 flex-1 overflow-hidden rounded-full bg-gray-100">
          <div
            ...
            style={{ width: `${health ?? 0}%`, background: dotColor, transition: "width 0.6s ease" }}
          />
        </div>
...
export default function HomePage({
  ...
  const activeProjects = useMemo(() => {
    return Array.isArray(activeProjectsLive) ? activeProjectsLive : [];
  }, [activeProjectsLive]);
  ...
  const ragMap = useMemo(() => {
    ...
    for (const it of rag || []) {
      if (it?.project_id) {
        m2.set(String(it.project_id), { rag: it.rag as RagLetter, health: Number(it.health) });
      }
    }
    return m2;
  }, [rag]);
        ...
        try {
          ...
          if (cancelled || !j?.ok) {
            if (!cancelled) {
              ...
              setActiveProjectsLive([]);
            }
            return;
          }

          const nextScope: DashboardScope = {
            scopedProjectCount: Number((j as any).scope?.scopedProjectCount ?? 0),
            activeProjectCount: Number((j as any).scope?.activeProjectCount ?? 0),
            scopedProjectIds: Array.isArray((j as any).scope?.scopedProjectIds) ? (j as any).scope.scopedProjectIds : [],
            activeProjectIds: Array.isArray((j as any).scope?.activeProjectIds) ? (j as any).scope.activeProjectIds : [],
            windowDays: Number((j as any).scope?.windowDays ?? numericWindowDays),
            dueWindowDays: Number((j as any).scope?.dueWindowDays ?? dueWindowDays),
          };
          ...
          setActiveProjectsLive(nextActiveProjects);
          setPhData(nextPh);
          ...
          if (nextPh?.ok) {
            const liveScore = Math.max(
              0,
              Math.min(100, Math.round(Number((nextPh as any).score ?? (nextPh as any).portfolio_health ?? 0))),
            );
            ...
            if (liveScore > 0) {
              ...
              setBriefingData((prev: any) => {
                ...
                const liveTalkingPoints = [
                  ...
                  ...((prev.talking_points ?? []) as string[]).filter(
                    (tp: string) =>
                ...
                const patchedSections = (prev.sections ?? []).map((s: any) => {
                  ...
                  if (s.id === "delivery" && sentiment === "red") {
                    return {
                      ...s,
                      sentiment: "red",
                      body: `${patchRag.r} project${patchRag.r !== 1 ? "s" : ""} ha${patchRag.r === 1 ? "s" : "ve"} overdue milestones. Schedule delivery requires immediate attention.`,
                    };
                  }
                  if (s.id === "delivery" && sentiment === "amber") {
                    return {
                      ...s,
                      sentiment: "amber",
                      body: `Delivery signals require monitoring — some milestones are approaching or past due dates.`,
                    };
                  }
                  if (s.id === "finance") {
                    const fpData = nextFp as any;
                    const budget = fpData?.ok
                      ? (fpData.portfolio?.totalBudget ??
                          fpData.portfolio?.total_budget ??
                          fpData.total_approved_budget ??
                          fpData.approved_budget ??
                          fpData.budget ??
                          null)
                      : null;
                    const spent = fpData?.ok
                      ? (fpData.portfolio?.totalActual ??
                          fpData.portfolio?.total_actual ??
                          fpData.portfolio?.totalSpent ??
                          fpData.total_spent ??
                          fpData.actual_spent ??
                          fpData.spent ??
                          null)
  ...
  const fpBudgetRaw = fpHasData
    ? (() => {
        ...
        for (const [k, v] of Object.entries(s)) {
          ...
          if ((kl.includes("budget") || kl.includes("approved")) && Number.isFinite(Number(v)) && Number(v) > 0) {
            return v;
          }
        }
  ...
  const fpSpentRaw = fpHasData
    ? (() => {
        ...
        for (const [k, v] of Object.entries(p)) {
          const kl = k.toLowerCase();
          if ((kl.includes("spent") || kl.includes("actual")) && Number.isFinite(Number(v)) && Number(v) >= 0) {
            return v;
          }
        }
        return undefined;
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-baseline gap-2.5">
                ...
                <span className="hidden text-xs text-gray-400 md:block">
                  ...
                  {filtersActive ? ` - filtered (${activeProjectCountLabel})` : ""}
                </span>
              </div>
            </div>
              ...
              >
                <div className="flex items-center justify-between border-b border-gray-50 px-6 py-4">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-gray-400">{activeProjectCountLabel} projects</p>
                  </div>
                  <span className="pr-12 text-xs text-gray-400">Health</span>
                </div>

                {(dashboardScope?.activeProjectCount ?? 0) > 0 && sortedProjects.length === 0 && (
                  <div className="mx-6 my-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                    Project list is temporarily syncing. Refreshing scope is recommended.
                  </div>
                )}
                ...
                {sortedProjects.length === 0 && (
                  <div className="py-14 text-center text-sm text-gray-400">
                    {filtersActive
                      ...
                      : "No active projects found in this portfolio"}
                  </div>
                )}
                ...
                {sortedProjects.length > 9 && (
                  <div className="border-t border-gray-50 px-6 py-3 text-center">
                    ...
                    >
                      View all {activeProjectCountLabel} projects
                    </button>
                  </div>
                )}
              </div>
