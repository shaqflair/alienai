...
const PortfolioAskDrawer = dynamic(
  () => import("@/components/home/PortfolioAskDrawer"),
  {
    ssr: false,
  },
);
...
type DashboardSummaryResponse =
  ...
  | {
      ...
      dueDigest?: ArtifactDueResp | null;
      due?: ArtifactDueResp | null;
      raidPanel?: { ok?: boolean; panel?: RaidPanel | null } | RaidPanel | null;
      resourceActivity?:
        | { ok?: boolean; weeks?: ResourceWeek[] | null }
        | ResourceWeek[]
        | null;
      recentWins?:
        | { ok?: boolean; wins?: RecentWin[] | null }
        | RecentWin[]
        | null;
...
function searchParamsToFilters(sp: URLSearchParams): PortfolioFilters {
  ...
  const projectId = uniqStrings(
    sp.getAll("projectId").flatMap((x) => x.split(",")),
  );
  const projectCode = uniqStrings([
    ...sp.getAll("projectCode").flatMap((x) => x.split(",")),
    ...sp.getAll("code").flatMap((x) => x.split(",")),
  ]);
  const projectName = uniqStrings(
    sp.getAll("name").flatMap((x) => x.split(",")),
  );
...
function notifIcon(n: NotifRow) {
  ...
  if (t.includes("success") || t.includes("trophy")) {
...
function orderBriefingInsights(xs: Insight[]) {
  return [...(Array.isArray(xs) ? xs : [])].sort(
    (a, b) =>
      (a?.id === "ai-warning" ? 0 : 1) - (b?.id === "ai-warning" ? 0 : 1),
  );
}
...
function normalizeDueItemsFromEvent(
  j: ArtifactDueResp | null | undefined,
): DueDigestItem[] {
  if (!j || !j.ok) return [];
  ...
  return list
    ...
    .map((x: any) => ({
      ...
      dueDate:
        x?.dueDate || x?.due_date || x?.due_at || x?.deadline || null,
      ownerLabel:
        x?.ownerLabel ||
        x?.owner_label ||
        x?.owner_name ||
      ...
      ownerEmail:
        ...
        null,
      link:
        safeStr(x?.link || x?.href || x?.url || x?.project_link).trim() ||
        null,
      meta: {
        ...x?.meta,
        project_code:
...
const ProjectRow = memo(function ProjectRow({
  ...
  return (
    <div
      ...
      onKeyDown={(e) =>
        e.key === "Enter" && pid && router.push(`/projects/${encodeURIComponent(pid)}`)
      }
            ...
            <div className="flex items-center justify-between border-b border-gray-200 px-5 py-4">
              <div>
                ...
                <div className="text-xs text-gray-500">
                  Filter portfolio by project and ownership
                </div>
              </div>
              <button
...
export default function HomePage({
  ...
  const urlFilters = useMemo(
    () => searchParamsToFilters(new URLSearchParams(searchParamString)),
    [searchParamString],
  );
  ...
  // seed immediately from server render
  const [briefingData, setBriefingData] = useState<BriefingData | null>(
  ...
  const derivedApiFilters = useMemo(
    () => deriveApiFilters(urlFilters, projectOptions),
    [urlFilters, projectOptions],
  );
  const apiFilterSnapshot = useMemo(
    () => stableFilterKey(derivedApiFilters),
    [derivedApiFilters],
  );
  ...
  const filteredProjectsClient = useMemo(() => {
    ...
    const idSet = new Set(
      (f.projectId ?? []).map((s) => String(s).trim()).filter(Boolean),
    );
    ...
    return rows.filter((p: any) => {
      ...
      if (nameNeedles.length && !nameNeedles.some((n) => title.includes(n))) {
        return false;
      }
      if (codeNeedles.length && !codeNeedles.some((c) => code.includes(c))) {
        return false;
      }
  ...
  const activeProjects = useMemo(() => {
    ...
    return (Array.isArray(filteredProjectsClient) ? filteredProjectsClient : []).filter(
      (p: any) => {
        if (String(p?.resource_status ?? "").toLowerCase() === "pipeline") {
          return false;
        }
    ...
    const idleId = runIdle(() => {
      (async () => {
        try {
          ...
          const j: DashboardSummaryResponse = await r
            .json()
          ...
          const nextInsights = orderBriefingInsights(
            Array.isArray(j.insights)
              ? j.insights
              : Array.isArray(j.aiBriefing?.insights)
                ? j.aiBriefing?.insights
                : [],
          );
          const nextFp = j.financialPlanSummary ?? j.financialPlan ?? null;
          const nextDueItems = normalizeDueItemsFromEvent(j.dueDigest ?? j.due ?? null);
          ...
          const executiveBriefingFromApi =
            j.executiveBriefing ||
            j.aiBriefing?.executive_briefing ||
            j.aiBriefing?.briefing ||
            executiveBriefing ||
            null;
          ...
          if (nextPh?.ok) {
            ...
            if (liveScore > 0) {
              ...
              setBriefingData((prev) => {
                ...
                const sentiment =
                  liveScore >= 85 ? "green" : liveScore >= 70 ? "amber" : "red";
  ...
  const fpRag = fpHasData
    ? ((((fpSummary as any).rag || fpPortfolioPre?.rag) as RagLetter) ?? null)
    : null;

  const fpCurrency = fpHasData
    ? safeStr((fpSummary as any).currency || fpPortfolioPre?.currency).trim() || "£"
    : "£";
  ...
  const fpBudgetRaw = fpHasData
    ? (() => {
        ...
        for (const [k, v] of Object.entries(s)) {
          ...
          if (
            (kl.includes("budget") || kl.includes("approved")) &&
  ...
  const fpSpentRaw = fpHasData
    ? (() => {
        ...
        for (const [k, v] of Object.entries(p)) {
          const kl = k.toLowerCase();
          if (
            (kl.includes("spent") || kl.includes("actual")) &&
            Number.isFinite(Number(v)) &&
            Number(v) >= 0
          ) {
            return v;
  ...
  const fpTotalBudget: number | null =
    fpBudgetRaw != null && Number.isFinite(Number(fpBudgetRaw))
      ? Number(fpBudgetRaw)
      : null;

  const fpTotalSpent: number | null =
    fpSpentRaw != null && Number.isFinite(Number(fpSpentRaw))
      ? Number(fpSpentRaw)
      : null;
  ...
  const fpSubLabel = fpHasData
    ? fpTotalSpent != null
      ? `${formatBudget(fpTotalSpent, fpCurrency)} spent${
          fpVarianceNum != null
            ? ` - ${fpVarianceNum > 0 ? "+" : ""}${fpVarianceNum}% variance`
            : ""
        }`
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <Search
                  className={[
                    "h-4 w-4",
                    drawerOpenViaSearch ? "text-white" : "text-gray-700",
                  ].join(" ")}
                />
              </button>
        ...
        <main className="mx-auto max-w-screen-2xl space-y-5 px-6 py-6">
          ...
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            <KpiCard
              ...
              trendLabel={
                phDelta != null && phDelta !== 0 ? `${Math.abs(Math.round(phDelta))}` : undefined
              }
            ...
            <KpiCard
              ...
              onClick={() =>
                router.push(appendFiltersToUrl(`/insights?tab=raid&days=${numericWindowDays}`, urlFilters))
              }
            />
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0">
                ...
                <p className="mt-1 max-w-2xl text-sm text-gray-500">
                  Open the governance intelligence centre for approvals, control
                  signals, oversight, and delivery decision support.
                </p>
              </div>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            <div className="space-y-4 lg:col-span-2">
              {(phData?.ok
                ? liveRagCounts.g + liveRagCounts.a + liveRagCounts.r
                : ragAgg.scored) > 0 && (
                <div
                ...
                >
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      ...
                      const total =
                        displayRagCounts.g + displayRagCounts.a + displayRagCounts.r;
