...
const KPI_THEMES: Record<
  string,
  { bg: string; iconBg: string; iconColor: string; valueColor: string; labelColor: string; subColor: string; trendBg: string; trendColor: string }
> = {
  green: {
    bg: "bg-green-50",
    iconBg: "bg-green-100",
  ...
  return (
    <m.div
      ...
      className={[
        ...
        t.bg,
        clickable ? "cursor-pointer hover:brightness-[0.97] hover:-translate-y-0.5" : "",
      ].join(" ")}
    >
  ...
  return (
    ...
    >
      <div className="relative shrink-0 group/rag" onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()}>
        <div
          className="h-3 w-3 rounded-full cursor-help ring-2 ring-transparent group-hover/rag:ring-offset-1"
          style={{ background: dotColor, boxShadow: `0 0 0 2px ${dotColor}22` }}
        />
        <div
          className="pointer-events-none absolute left-5 top-1/2 -translate-y-1/2 z-50 w-64 opacity-0 group-hover/rag:opacity-100 transition-opacity duration-150 rounded-xl bg-white border border-gray-200 p-3 text-left"
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-1.5">
        ...
        <span className={["text-[10px] font-semibold rounded-full px-2.5 py-0.5 whitespace-nowrap shrink-0", statusCfg.badge].join(" ")}>
          {statusCfg.text}
        </span>
      </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {pmOptions.slice(0, 28).map((pm) => {
                    ...
                    return (
                      <button key={pm.id} type="button" onClick={() => toggle("projectManagerId", pm.id)} className={["px-3 py-1.5 rounded-full text-xs border", pill(on)].join(" ")}>
                        {pm.name}
                      </button>
                    );
                  })}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {deptOptions.slice(0, 28).map((d) => {
                    ...
                    return (
                      <button key={d.value} type="button" onClick={() => toggle("department", d.value)} className={["px-3 py-1.5 rounded-full text-xs border", pill(on)].join(" ")}>
                        {d.label}
                      </button>
                    );
                  })}
...
export default function HomePage({ data }: { data: HomeData }) {
  ...
  const raidDueTotal = useMemo(() => {
    ...
    const typedAvailable =
      raidPanel.risk_due != null || raidPanel.issue_due != null || raidPanel.dependency_due != null || raidPanel.assumption_due != null;
    if (typedAvailable) {
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className={[
                    ...
                    drawerOpen ? "bg-gray-900 border-gray-900" : "bg-white border-gray-200 hover:bg-gray-50",
                  ].join(" ")}
                  aria-label="Search"
                  title="Search"
                >
                ...
                <button
                  ...
                  className={[
                    "h-9 w-9 rounded-xl border flex items-center justify-center transition-colors",
                    drawerOpen || filtersActive ? "bg-gray-900 border-gray-900" : "bg-white border-gray-200 hover:bg-gray-50",
                  ].join(" ")}
                  aria-label="Filter"
                  title="Filter"
                >
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
                  <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-50">
                    ...
                    <button onClick={() => router.push(appendFiltersToUrl("/success-stories", urlFilters))} className="text-xs text-blue-600 hover:text-blue-700 font-medium">
                      View all
                    </button>
                  </div>
                  <div className="p-4 space-y-2.5">
