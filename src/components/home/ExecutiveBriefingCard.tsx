        ...
        <div>
          ...
          {data?.ok && !fetchErr && !loading && (
            <div className="space-y-5 p-6">
              ...
              {/* Collapsible sections — same pattern as GapsPanel */}
              {sections.length > 0 && (
                <div>
                  <button
                    type="button"
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-sm font-bold text-slate-800">Section detail</span>
                      <span className="inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-slate-200 px-1.5 text-[10px] font-bold text-slate-600">
                        {sections.length}
                      </span>
                    </div>
                    <ChevronRight className={`h-4 w-4 text-slate-400 transition-transform duration-200${showSections ? " rotate-90" : ""}`}/>
                  </button>
                  {showSections && (
                    <div className="mt-2">
