...
// Accepts liveRagCounts + projectScores from HomePage (already proven correct)
// so no health table queries needed in the API.
...
type NarrativeSection = {
  id: SectionId; title: string; body: string; sentiment: Sentiment;
};
type Gap = {
  severity: "high" | "medium" | "low";
  type: string; detail: string; project?: string; href?: string;
};
type BriefingData = {
  ok: boolean;
...
export type ProjectScores = Record<string, { score: number; rag: "G" | "A" | "R" }>;

/* -- Helpers --------------------------------------------------------------- */
function safeStr(x: unknown): string {
  if (typeof x === "string") return x; if (x == null) return ""; return String(x);
}
...
function BriefingInner({
  ...
  const load = useCallback(async (force = false) => {
    ...
    try {
      const body: any = {};
      // Pass live health data from the homepage so the API doesn't need to query health tables
      if (liveRagCounts) body.ragCounts = liveRagCounts;
      if (projectScores && Object.keys(projectScores).length > 0) body.projectScores = projectScores;
      if (force) body.force = true;

      const res = await fetch("/api/ai/portfolio-narrative", {
        method: "POST",
  ...
  // Use live rag counts if available (more accurate than what API returns)
  const displayRag = liveRagCounts && data
        ...
        <div>
            ...
            <div className="p-6">
              <div className="rounded-xl border border-red-100 bg-red-50 p-4 flex items-start gap-3">
                ...
                <div>
                  ...
                  <button type="button" onClick={() => load(true)} className="mt-2 text-xs text-red-600 hover:text-red-800 font-medium underline">Try again</button>
                </div>
              </div>
            </div>
          )}
          ...
          {data && !fetchErr && !loading && (
            <div className="p-6 space-y-5">
              ...
              {/* Pills -- always use live counts from homepage */}
              {(displayRag || sig) && (
                ...
                <div>
                  <button type="button" onClick={() => setShowGaps((v) => !v)}
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      {highG.length > 0 && <span className="inline-flex items-center justify-center h-5 min-w-5 px-1.5 rounded-full bg-red-500 text-[10px] font-bold text-white">{highG.length}</span>}
                      {otherG.length > 0 && <span className="inline-flex items-center justify-center h-5 min-w-5 px-1.5 rounded-full bg-gray-300 text-[10px] font-bold text-gray-700">{otherG.length}</span>}
                    </div>
                        ...
                        return (
                          <div key={i} className={"flex items-start gap-3 rounded-xl border px-4 py-3 " + gs.bg}>
                            ...
                            <div className="min-w-0 flex-1">
                              {g?.project && <div className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-0.5">{safeStr(g.project)}</div>}
              ...
              {points.length > 0 && (
                <div className="rounded-xl border border-indigo-100 bg-indigo-50/50 p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button type="button" onClick={copy} className="text-xs text-indigo-500 hover:text-indigo-700 font-medium flex items-center gap-1 transition-colors">
                  ...
                  <ol className="space-y-2">
                    {points.map((tp, i) => (
                      <li key={i} className="flex items-start gap-2.5">
                        <span className="h-5 w-5 rounded-full bg-indigo-100 border border-indigo-200 flex items-center justify-center text-[10px] font-bold text-indigo-600 shrink-0 mt-0.5">{i + 1}</span>
...
export default function ExecutiveBriefingCard({
  ...
  return (
    <CardErrorBoundary>
      <BriefingInner liveRagCounts={liveRagCounts} projectScores={projectScores} />
    </CardErrorBoundary>
  );
}
