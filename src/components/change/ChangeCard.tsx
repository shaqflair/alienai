...
function riskLevelFromText(risk: unknown): RiskLevel {
  const t0 = safeStr(risk).trim(); const t = t0.toLowerCase();
  if (!t) return "None";
  ...
  const highWords = /\bhalt\b|\bhalted\b|\bhalting\b|\bstopp?age\b|\bshutdown\b|\boutage\b|\bservice down\b|\bdown\b|\bbreach\b|\bsecurity\b|\bdata loss\b|\bprivacy\b|\bregulatory\b|\bfine\b|\bpenalt(y|ies)\b|\bmajor\b|\bsev[ -]?(1|2)\b|\bcritical path\b|\brollback fails?\b|\bcatastrophic\b/;
  const medWords = /\bdelay\b|\bslip(page)?\b|\bdegrad(e|ation)\b|\bperformance\b|\bcapacity\b|\bvendor\b|\bdependency\b|\bblocked\b|\brework\b|\btesting\b|\bintegration\b|\bapproval\b|\bcab\b|\bchange window\b|\breschedule\b/;
...
function isDraftStatus(s: ChangeStatus) { return s === "new" || s === "analysis"; }
...
function DeleteConfirm({ title, onConfirm, onCancel, busy }: { title: string; onConfirm: () => void; onCancel: () => void; busy: boolean }) {
  return (
    <div style={{ position: "absolute", inset: 0, zIndex: 10, borderRadius: 12, background: "rgba(10,11,18,0.92)", backdropFilter: "blur(6px)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "16px 14px", gap: 10, textAlign: "center" }}>
      <div style={{ width: 36, height: 36, borderRadius: "50%", background: "rgba(248,113,113,0.15)", border: "1px solid rgba(248,113,113,0.35)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
      ...
      <div>
        <div style={{ fontSize: 11.5, fontWeight: 700, color: "#f1f3fc", marginBottom: 3 }}>Delete draft CR?</div>
        <div style={{ fontSize: 10.5, color: "#7880a0", lineHeight: 1.5, maxWidth: 200, margin: "0 auto" }}>{title}</div>
      </div>
      <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
        <button type="button" onClick={onCancel} disabled={busy} style={{ padding: "7px 14px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.06)", color: "#c8cfe0", fontSize: 11.5, fontWeight: 600, cursor: "pointer" }}>Cancel</button>
        <button type="button" onClick={onConfirm} disabled={busy} style={{ padding: "7px 14px", borderRadius: 8, border: "none", background: "rgba(248,113,113,0.22)", color: "#fca5a5", fontSize: 11.5, fontWeight: 600, cursor: "pointer" }}>
...
function injectStyles() {
  ...
  el.textContent = `
    ...
    .kc-card { background:#fff; border-radius:14px; border:1px solid #e4e7f0; box-shadow:0 1px 4px rgba(15,20,50,0.04),0 2px 8px rgba(15,20,50,0.03); transition:box-shadow 0.18s,transform 0.18s,border-color 0.18s; cursor:grab; position:relative; overflow:hidden; font-family:'Syne',system-ui,sans-serif; }
    .kc-card:hover { box-shadow:0 4px 20px rgba(15,20,50,0.09),0 2px 8px rgba(15,20,50,0.05); border-color:#c8cde0; transform:translateY(-2px); }
    ...
    .kc-requester-icon { width:18px; height:18px; background:linear-gradient(135deg,#6366f1,#8b5cf6); border-radius:50%; display:inline-flex; align-items:center; justify-content:center; font-size:8px; color:white; font-weight:700; flex-shrink:0; }
    ...
    .kc-submit-btn { width:100%; margin-top:8px; padding:8px 12px; background:linear-gradient(135deg,#6366f1,#8b5cf6); color:#fff; border:none; border-radius:9px; font-size:11.5px; font-weight:700; cursor:pointer; transition:opacity 0.15s,transform 0.15s; font-family:'Syne',system-ui,sans-serif; box-shadow:0 2px 10px rgba(99,102,241,0.28); }
...
export default function ChangeCard({
  ...
  const status: ChangeStatus = (item as any).delivery_status ?? (item as any).status ?? "new";
  const navId = safeStr((item as any)?.id).trim();
  const displayId = crHumanId(item);
  const href = projectId && navId ? `/projects/${projectId}/change/${navId}` : "#";
  const hrefWithPanel = usePanelHref(href);
  const myLaneIdx = laneIndex(status);
  const prevLane = myLaneIdx > 0 ? CHANGE_COLUMNS[myLaneIdx - 1].key : null;
  const nextLane = myLaneIdx < CHANGE_COLUMNS.length - 1 ? CHANGE_COLUMNS[myLaneIdx + 1].key : null;
  const lockReview = status === "review";
  const decisionStatus = safeStr((item as any)?.decision_status ?? (item as any)?.decisionStatus).trim().toLowerCase();
  const canArrowPrev = !!prevLane && !lockReview && decisionStatus !== "submitted" && decisionStatus !== "approved";
  const canArrowNext = !!nextLane && !lockReview && status !== "analysis" && decisionStatus !== "submitted" && decisionStatus !== "approved";
  const aiDays = safeNum((item as any)?.ai_schedule ?? (item as any)?.impact_analysis?.days, 0);
  const aiCost = safeNum((item as any)?.ai_cost ?? (item as any)?.impact_analysis?.cost, 0);
  const aiRiskRaw = safeStr((item as any)?.impact_analysis?.risk ?? (item as any)?.ai_risk).trim();
  const structured = normalizeRiskLevel((item as any)?.risk_level ?? (item as any)?.riskLevel);
  ...
  async function submitForApproval() {
    ...
    try {
      ...
      try {
        await postJson("/api/ai/events", { projectId, artifactId: null, eventType: "change_submitted_for_approval", severity: "info", source: "change_card", payload: { changeId: navId, title: safeStr(item.title) } });
      } catch {}
      window.location.reload();
    } catch (e: any) { setErr(safeStr(e?.message) || "Submit failed"); }
  ...
  const lockedMsg = lockReview ? "Awaiting approval" : decisionStatus === "approved" ? "Approved" : decisionStatus === "rejected" ? "Rejected" : "";
  ...
  return (
    ...
    >
      <div className="kc-rail" style={{ background: railColor }}/>
        ...
        <div className="kc-header">
          ...
          <div className="kc-nav">
            ...
            <button type="button" className="kc-nav-btn" disabled={!canArrowNext}
              title={status === "analysis" && nextLane === "review" ? "Use Submit for approval" : !canArrowNext ? (lockReview ? "Review locked" : "Locked") : `Move to ${nextLane}`}
        ...
        {!compact && (
          <>
            ...
            <div className="kc-impact">
              ...
              <div className="kc-risk-orb" style={{ background: risk.bg, border: `1.5px solid ${risk.border}` }} title={riskTitle}>
