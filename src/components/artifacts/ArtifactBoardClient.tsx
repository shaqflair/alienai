...
// Single canonical daysUntil — used everywhere in this file
...
function aiItemHref(args: { item: AiItem | unknown; fallbackProjectRef: string }): string {
  ...
  if (artifactId && looksLikeUuid(artifactId)) {
    ...
    if (kind.includes("milestone") || kind.includes("schedule")) {
      params.set("panel", "schedule");
    } else if (kind.includes("work_item") || kind.includes("work item") || kind.includes("wbs")) {
      params.set("panel", "wbs");
    } else if (kind.includes("change")) {
      params.set("panel", "change");
    }
...
function canonType(value: unknown): string {
  ...
  if (
    ...
    normalized === "delivery_governance" ||
    normalized === "delivery governance" ||
    normalized === "governance_hub" ||
    normalized === "governance hub"
  ) return "GOVERNANCE";
  if ([
    "weekly_report","weekly report","weekly_status","weekly status",
    "weekly_update","weekly update","delivery_report","delivery report",
    "status_report","status report",
  ].includes(normalized)) return "WEEKLY_REPORT";
  if (normalized === "status_dashboard" || normalized === "status dashboard")
  ...
  if (normalized.includes("schedule") || normalized.includes("roadmap") || normalized.includes("gantt"))
    return "SCHEDULE";
  if ([
    "financial_plan","financial plan","financial","budget_plan","budget plan","financials",
  ].includes(normalized)) return "FINANCIAL_PLAN";
...
function applyCurrentFallback(rows: ArtifactBoardRow[]): ArtifactBoardRow[] {
  ...
  return rows.map((row) => {
    const typeKey =
      safeStr(row.typeKey || canonType(row.artifactType)).trim() ||
      safeStr(row.artifactType).trim();
...
interface VirtualRow extends ArtifactBoardRow {
  isVirtual?: boolean;
}
...
function rowHref(projectRef: string, row: VirtualRow | unknown): string {
  ...
  if (typeKey === "CHANGE_REQUESTS" || typeKey === "CHANGE" || typeKey === "CHANGE_REQUEST")
    return `/projects/${projectRef}/change`;
  if (typeKey === "RAID" || typeKey === "RAID_LOG")
    return `/projects/${projectRef}/raid`;
  if (typeKey === "GOVERNANCE")
    return `/projects/${projectRef}/governance`;
  const id = safeStr(candidate.id).trim();
  ...
  return (
    <div className="flex w-full items-center gap-2.5">
      ...
      <span
        ...
        style={{ color: "var(--ui-faint)", minWidth: 30, textAlign: "right" }}
      >
        {pct}%
      </span>
    </div>
...
function StatusBadge({ status }: { status: UiStatus }) {
  ...
  return (
    <span
      className="artifact-mono inline-flex items-center gap-1.5 rounded-full px-2 py-[3px] text-[9px] font-medium uppercase tracking-[0.08em]"
...
function AvatarChip({ email, name }: { email: string; name?: string }) {
  ...
  const hue = email
    ? email.split("").reduce((sum, char) => sum + char.charCodeAt(0), 0) % 360
    : 220;
  return (
    <div className="flex min-w-0 items-center gap-2">
      ...
      <span className="truncate text-[13px]" style={{ color: "var(--ui-text-soft)" }}>
        {displayName}
      </span>
    </div>
  );
...
function TagPill({ children, color, bg }: { children: ReactNode; color: string; bg: string }) {
  return (
    <span
      className="artifact-mono inline-flex items-center gap-1 rounded-full px-1.5 py-[2px] text-[9px] font-medium uppercase tracking-[0.08em]"
...
function ArtifactTableRow({
  ...
  const typeKey = rowTypeKey(row);
  const isBoardManageable = BOARD_MANAGEABLE_TYPES.has(typeKey);
  const canDelete =
    !virtual && isBoardManageable && row.canDeleteDraft !== false &&
      ...
      <div className="flex h-full min-w-0 items-center gap-2 px-4 py-3">
        <div className="min-w-0 flex-1">
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            ...
            {virtual && (
              <TagPill color="var(--ui-muted)" bg="var(--ui-panelAlt)">module</TagPill>
            )}
          </div>
...
function InlineFilterBar({
  ...
  toggleStatus: (status: UiStatus) => void;
  phaseSet: Set<Phase>;
    ...
    <div className="toolbar-artifacts">
      <div className="toolbar-search-wrap">
        ...
        <button
          ...
          type="button"
          style={{
            borderColor: activeCount > 0 ? "var(--ui-accentSoft)" : "var(--ui-border)",
            background: activeCount > 0 ? "var(--ui-accentSoft)" : "var(--ui-panel)",
            color: activeCount > 0 ? "var(--ui-accent)" : "var(--ui-muted)",
          }}
        >
      ...
      {showFilters && (
        <div className="toolbar-filter-panel">
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    color: active ? style.color : "var(--ui-faint)",
                    borderColor: active ? style.bg : "var(--ui-border)",
                  }}
                >
                  {status}
                </button>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    color: active ? config.color : "var(--ui-faint)",
                    borderColor: active ? config.bg : "var(--ui-border)",
                  }}
                >
                  {config.label}
                </button>
          ...
          {activeCount > 0 && (
            <>
              <div className="toolbar-divider" />
              <button onClick={clearAll} type="button" className="toolbar-clear-btn artifact-mono">
                Clear all
              </button>
            </>
          )}
...
function StatsRow({ rows }: { rows: ArtifactBoardRow[] }) {
  const stats = useMemo(() => {
    ...
    const avgProgress = total
      ? Math.round(rows.reduce((sum, r) => sum + r.progress, 0) / total)
      : 0;
  ...
  return (
    <div className="artifact-stats-row artifact-mono">
      ...
      {stats.blocked > 0 && (
        <span><b style={{ color: "var(--ui-danger)", fontWeight: 500 }}>{stats.blocked}</b> blocked</span>
      )}
...
/* =========================================================
   AI Panel
   NOTE: daysUntil is defined once in utilities above
========================================================= */

type AiScope = "project" | "org";

function AiScopeButton({
  active, disabled, onClick, children, title,
}: {
  active: boolean; disabled?: boolean; onClick: () => void; children: ReactNode; title?: string;
}) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      title={title}
      className="artifact-mono px-2.5 text-[10px] font-medium uppercase tracking-[0.08em] transition-colors disabled:cursor-not-allowed"
      style={{
        background: active ? "var(--ui-accentSoft)" : "transparent",
...
function AiMetricPill({ label, value }: { label: string; value: number }) {
  return (
    <span
      ...
      style={{
        fontSize: 10, fontWeight: 500, color: "var(--ui-text-soft)",
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-1.5 gap-2">
        <span
          className="artifact-mono"
          style={{ fontSize: 9, fontWeight: 500, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--ui-faint)", background: "var(--ui-panelAlt)", padding: "4px 6px", borderRadius: 999 }}
        >
          {label}
        </span>
        {days !== null && (
          <span
            className="artifact-mono"
            style={{ fontSize: 10, fontWeight: 500, color: isOverdue ? "var(--ui-danger)" : "var(--ui-warning)" }}
          >
      ...
      <h4 style={{ fontSize: 13, fontWeight: 600, color: "var(--ui-text)", marginBottom: 4 }}>
      ...
      <div className="flex items-center gap-1.5 artifact-mono" style={{ fontSize: 10, color: "var(--ui-faint)", marginBottom: 10 }}>
        ...
        {dueRaw ? fmtUkDateOnly(dueRaw) : "No due date"}
      </div>
      ...
      <div className="flex gap-2">
        <Link
          ...
          style={{ background: "var(--ui-panelAlt)", borderColor: "var(--ui-border)", color: "var(--ui-text-soft)" }}
        >
          Open
        </Link>
        <button
          onClick={() => navigator.clipboard.writeText(`Reminder: ${safeStr(item?.title)} due ${dueRaw ? fmtUkDateOnly(dueRaw) : "TBC"}`)}
...
function AiPanel({
  ...
  projectUuid: string; projectCode: string;
  projectName: string; projectHumanId: string;
}) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[12vh]">
      ...
      <div
        className="relative w-full max-w-md max-h-[70vh] rounded-2xl overflow-hidden flex flex-col"
        style={{ background: "var(--ui-panel)", border: "1px solid var(--ui-border)", boxShadow: "0 25px 50px -12px rgba(0,0,0,0.15)" }}
      >
        <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: "var(--ui-border)" }}>
          <div className="flex items-center gap-2">
            <div className="h-6 w-6 rounded-md flex items-center justify-center" style={{ background: "var(--ui-accentSoft)" }}>
              <Sparkles className="h-3.5 w-3.5" style={{ color: "var(--ui-accent)" }} />
            </div>
            <div>
              <span style={{ fontSize: 13, fontWeight: 600, color: "var(--ui-text)" }}>AI Assistant</span>
              <span className="artifact-mono" style={{ fontSize: 10, color: "var(--ui-faint)", marginLeft: 8 }}>
                {scope === "org" ? "Portfolio" : projectCode || projectHumanId || projectName || "—"}
              </span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div
              ...
              style={{ borderColor: "var(--ui-border)", background: "var(--ui-panelAlt)", height: 28 }}
            >
              <AiScopeButton
                active={scope === "project"} disabled={!canProject}
                onClick={() => { if (!canProject) return; setScope("project"); setResult(null); setError(""); }}
              ...
              <AiScopeButton
                active={scope === "org"}
                onClick={() => { setScope("org"); setResult(null); setError(""); }}
          ...
          {scope === "org" && counts && (
            <div className="mb-3 p-3 rounded-xl border" style={{ background: "var(--ui-accentSoft)", borderColor: "var(--ui-border)" }}>
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="artifact-mono" style={{ fontSize: 10, color: "var(--ui-muted)" }}>
                  Due soon: <span style={{ fontWeight: 500, color: "var(--ui-text)" }}>{safeNum(counts?.dueSoon ?? counts?.due_soon ?? items.length)}</span>
                </div>
              </div>
          ...
          {error ? (
            <div className="p-3 rounded-xl text-[13px] flex items-start gap-2" style={{ background: "var(--ui-dangerSoft)", color: "var(--ui-danger)" }}>
              ...
              <div>
                ...
                <button onClick={runCheck} disabled={loading} className="mt-2 px-3 py-1 rounded-md text-[12px] font-medium disabled:opacity-50" style={{ background: "var(--ui-accent)", color: "white" }}>
                ...
                <div key={g.key} className="rounded-xl border overflow-hidden" style={{ borderColor: "var(--ui-border)" }}>
                  ...
                  <div className="p-3 space-y-2">
                    {g.items.slice(0, 25).map((item: any, idx: number) => (
                      <AiItemCard key={`${g.key}:${idx}`} item={item} href={aiItemHref({ item, fallbackProjectRef: projectRef })} />
                    ))}
                  </div>
                </div>
              ))}
            ...
            <div className="space-y-2">
              {items.map((item: any, idx: number) => (
                <AiItemCard key={idx} item={item} href={aiItemHref({ item, fallbackProjectRef: projectRef })} />
              ))}
            </div>
          )}
        </div>
        ...
        <div className="px-4 py-2.5 border-t" style={{ borderColor: "var(--ui-border)" }}>
          <button
            ...
            style={{ color: "var(--ui-text-soft)", background: "var(--ui-panelAlt)", borderColor: "var(--ui-border)" }}
          >
...
export default function ArtifactBoardClient(props: {
  ...
  const toggleStatus = (s: UiStatus) => {
    setStatusSet((prev) => { const next = new Set(prev); next.has(s) ? next.delete(s) : next.add(s); return next; });
  };
  const togglePhase = useCallback((p: Phase) => {
    setPhaseSet((prev) => { const next = new Set(prev); next.has(p) ? next.delete(p) : next.add(p); return next; });
  }, []);
  ...
  const openRow = useCallback(
    (row: ArtifactBoardRow) => router.push(rowHref(projectRef, row)),
    [projectRef, router],
  );
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .notion-board {
          background: linear-gradient(to bottom, var(--ui-bg) 0%, var(--ui-bg) 280px, var(--ui-panelAlt) 100%);
          color: var(--ui-text);
            ...
            <div className="flex items-center justify-between py-5 gap-4">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <div style={{ fontSize: 28, fontWeight: 700, color: "var(--ui-text)", letterSpacing: "-0.035em", lineHeight: 1 }}>
                    Artifacts
                  </div>
        ...
        <main className="max-w-[1320px] mx-auto px-6 py-4 relative z-10">
          ...
          {sortedRows.length === 0 ? (
            <div className="text-center" style={{ padding: "88px 24px", border: "1px solid var(--ui-border)", background: "var(--ui-panel)", borderRadius: 16 }}>
              ...
              <div style={{ fontSize: 28, fontWeight: 600, color: "var(--ui-text)", letterSpacing: "-0.02em", marginBottom: 10 }}>
                No artifacts found.
              </div>
            ...
            <div className="overflow-hidden overflow-x-auto" style={{ background: "var(--ui-panel)", border: "1px solid var(--ui-border)", borderRadius: 16, boxShadow: "var(--ui-shadow-soft)" }}>
              <div
                ...
                style={{ gridTemplateColumns: COL_TEMPLATE, minHeight: 42, background: "var(--ui-panelAlt)", borderBottom: "1px solid var(--ui-border)" }}
              >
                {["Name", "Owner", "Phase", "Status", "Progress", ""].map((label) => (
                  <span key={label} className="artifact-mono px-4 text-[9px] font-medium uppercase tracking-[0.14em]" style={{ color: "var(--ui-faint)" }}>
                    {label}
                  </span>
                ))}
              </div>
