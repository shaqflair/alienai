...
function normalizeGroup(x: any) { return String(x ?? "").trim() || "Project"; }
function inferMapping(influence?: Influence, impact?: Impact): Mapping {
  const i = String(influence ?? "").toLowerCase(); const p = String(impact ?? "").toLowerCase();
...
function normalizeChannel(x: any) { return String(x ?? "").trim().replace(/\s+/g, " "); }
function normalizeNameKey(x: any) { return String(x ?? "").trim().toLowerCase().replace(/\s+/g, " "); }
...
const DEFAULT_COL_WIDTHS: Record<string, number> = {
  name: 200, point_of_contact: 190, role: 155, internal_external: 110,
  title_role: 155, impact_level: 110, influence_level: 110,
  stakeholder_mapping: 155, involvement_milestone: 175,
  stakeholder_impact: 230, channels: 215, actions: 56,
};

// ─── Shared primitives ────────────────────────────────────────────────────────
const TH_CLS = "px-4 py-2.5 text-left text-[11px] font-semibold tracking-widest uppercase text-gray-400 bg-[#f9f9f8] border-b border-gray-200 whitespace-nowrap select-none";
...
function BadgeSelect({
  value, onChange, options, disabled, pillFn, showDot = false,
}: {
  value: string;
...
export default function StakeholderRegisterEditor(props: {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const mapped: StakeholderRow[] = rows.map((r: any) => ({
        id: String(r?.id ?? uuid()), name: String(r?.name ?? ""), point_of_contact: String(r?.point_of_contact ?? ""),
        role: String(r?.role ?? ""), internal_external: (r?.internal_external as InternalExternal) || "Internal",
        title_role: String(r?.title_role ?? ""), impact_level: (r?.impact_level as Impact) || "Medium",
        ...
        involvement_milestone: String(r?.involvement_milestone ?? ""), stakeholder_impact: String(r?.stakeholder_impact ?? ""),
        channels: Array.isArray(r?.channels) ? r.channels.map((x: any) => normalizeChannel(x)).filter(Boolean) : ["Teams"],
        group: normalizeGroup(r?.group ?? "Project"), __draft: false,
      }));
  ...
  useEffect(() => {
    const h = (e: MouseEvent) => { if (downloadMenuRef.current && !downloadMenuRef.current.contains(e.target as Node)) setShowDownloadMenu(false); };
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => setColWidths((p) => ({ ...p, [resizingCol]: Math.max(60, resizeStartWidth.current + (e.clientX - resizeStartX.current)) }));
  ...
  async function safeJson(res: Response) {
    try { return await res.json(); } catch { try { const t = await res.text(); return t ? { _raw: t } : null; } catch { return null; } }
  }

  const channelOptions = useMemo<MultiSelectOption[]>(() => {
    const set = new Set<string>();
    for (const p of CHANNEL_PRESETS) set.add(normalizeChannel(p));
    for (const r of doc.rows ?? []) for (const c of r.channels ?? []) { const v = normalizeChannel(c); if (v) set.add(v); }
    return Array.from(set).sort((a, b) => a.localeCompare(b)).map((v) => ({ value: v, label: v }));
  }, [doc.rows]);

  async function loadSuggestions() {
    try {
      const res = await fetch(`/api/ai/suggestions/list`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectId, target_artifact_type: "stakeholder_register" }) });
      const json = await safeJson(res);
      setPendingSugs(!res.ok || !json?.ok ? [] : Array.isArray(json?.suggestions) ? json.suggestions : []);
    } catch { setPendingSugs([]); }
  }
  ...
  async function acceptAllSuggestions() {
    ...
    startSugTransition(async () => {
      for (const id of ids) await fetch(`/api/ai/suggestions/apply`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectId, suggestionId: id }) }).catch(() => null);
      await loadFromDb().catch(() => null);
      await loadSuggestions().catch(() => null);
      window.dispatchEvent(new CustomEvent<StakeholdersChangedDetail>("alienai:stakeholders-changed", { detail: { projectId, artifactId, reason: "suggestions_accepted_all" } }));
    });
  }

  async function seedFromCharter() {
    try {
      const res = await fetch(`/api/stakeholders/seed-from-charter`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectId, artifactId }) });
      const json = await safeJson(res);
      if (res.ok && json?.ok) window.dispatchEvent(new CustomEvent<StakeholdersChangedDetail>("alienai:stakeholders-seeded", { detail: { projectId, artifactId, reason: "seeded_from_charter" } }));
    } catch { }
  }
  ...
  function mapServerRow(s: any): StakeholderRow {
    ...
    return {
      id: String(s?.id ?? uuid()), name: String(s?.name ?? ""), role: String(s?.role ?? ""),
      influence_level: influence, impact_level: impact,
      ...
      title_role: String(contact?.title_role ?? ""), point_of_contact: String(contact?.point_of_contact ?? ""),
      involvement_milestone: String(contact?.involvement_milestone ?? ""), stakeholder_impact: String(contact?.stakeholder_impact ?? ""),
      channels: channels.length ? channels : ["Teams"], group: normalizeGroup(contact?.group ?? "Project"), __draft: false,
    };
  }
  ...
  async function loadFromDb() {
    ...
    try {
      ...
      if (!seededFromCharterOnceRef.current && rows.length === 0) {
        ...
        if (res2.ok && json2?.ok) { setDoc({ version: 1, type: "stakeholder_register", rows: (Array.isArray(json2.stakeholders) ? json2.stakeholders : []).map(mapServerRow) }); setDirty(false); await loadSuggestions().catch(() => null); }
        return;
      }
      setDoc({ version: 1, type: "stakeholder_register", rows }); setDirty(false);
      await loadSuggestions().catch(() => null);
    } catch (e: any) { setErr(String(e?.message ?? e)); }
    finally { setLoading(false); loadingRef.current = false; }
  }

  useEffect(() => { seededFromCharterOnceRef.current = false; loadFromDb(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [projectId, artifactId]);
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener("alienai:stakeholders-changed", onChanged as any); window.removeEventListener("alienai:stakeholders-seeded", onChanged as any); };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const filtered = useMemo(() => {
    ...
    return (doc.rows ?? []).filter((r) => {
      const matchQ = !q || [r.name, r.role, r.title_role, r.point_of_contact, ...(r.channels ?? [])].some((v) => String(v ?? "").toLowerCase().includes(q));
      const matchP = personFilter === "all" || (personFilter === "internal" ? r.internal_external === "Internal" : r.internal_external === "External");
  ...
  const groups = useMemo(() => {
    ...
    for (const r of filtered) { const g = normalizeGroup(r.group ?? ""); if (!m.has(g)) m.set(g, []); m.get(g)!.push(r); }
    return Array.from(m.entries()).sort((a, b) => a[0].localeCompare(b[0]));
  }, [filtered]);

  function updateRow(id: string, patch: Partial<StakeholderRow>) { setDoc((p) => ({ ...p, rows: p.rows.map((r) => r.id === id ? { ...r, ...patch } : r) })); setDirty(true); }

  function addRow(group?: string) {
    const g = normalizeGroup(group ?? "");
    setDoc((p) => ({ ...p, rows: [{ id: uuid(), name: "", point_of_contact: "", role: "Stakeholder", internal_external: "Internal", title_role: "", impact_level: "Medium", influence_level: "Medium", stakeholder_mapping: "Keep Informed", involvement_milestone: "", stakeholder_impact: "", channels: ["Teams"], group: g, __draft: true }, ...p.rows] }));
    setDirty(true); setCollapsed((c) => ({ ...c, [g]: false }));
  }

  async function removeRow(id: string) {
    if (readOnly) return; setErr(null);
    const row = (doc.rows ?? []).find((r) => r.id === id);
    if (!row || row.__draft || !isDbUuid(row.id)) { setDoc((p) => ({ ...p, rows: p.rows.filter((r) => r.id !== id) })); setDirty(true); return; }
    const prevRows = doc.rows;
    setDoc((p) => ({ ...p, rows: p.rows.filter((r) => r.id !== id) })); setDeletingId(id);
    try {
      const res = await fetch(`/api/stakeholders/${encodeURIComponent(id)}?projectId=${encodeURIComponent(projectId)}&artifactId=${encodeURIComponent(artifactId)}`, { method: "DELETE" });
      const json = await safeJson(res);
    ...
    } catch (e: any) { setDoc((p) => ({ ...p, rows: prevRows })); setErr(String(e?.message ?? e)); }
    finally { setDeletingId(null); }
  }

  async function deleteAll() {
    if (readOnly || !window.confirm("Delete ALL stakeholders? This cannot be undone.")) return;
    setErr(null); const prevRows = doc.rows;
    setDoc((p) => ({ ...p, rows: [] })); setDeletingAll(true);
    try {
  ...
  function buildDoc(rows: StakeholderRow[]) {
    ...
    for (const r of rows) { const g = normalizeGroup(r.group ?? "Project"); if (!byGroup.has(g)) byGroup.set(g, []); byGroup.get(g)!.push(r); }
    return {
      version: 1, type: "stakeholder_register",
      groups: Array.from(byGroup.entries()).sort((a, b) => a[0].localeCompare(b[0])).map(([name, rs]) => ({
        name, rows: rs.filter((x) => String(x?.name ?? "").trim()).map((x) => ({
          id: String(x.id), name: String(x.name).trim(), point_of_contact: String(x.point_of_contact ?? "").trim(),
          role: String(x.role ?? "").trim(), internal_external: x.internal_external ?? "Internal",
          title_role: String(x.title_role ?? "").trim(), impact_level: x.impact_level ?? "Medium",
          influence_level: x.influence_level ?? "Medium", stakeholder_mapping: x.stakeholder_mapping ?? inferMapping(x.influence_level, x.impact_level),
          involvement_milestone: String(x.involvement_milestone ?? "").trim(), stakeholder_impact: String(x.stakeholder_impact ?? "").trim(),
          channels: Array.isArray(x.channels) ? x.channels.map(normalizeChannel).filter(Boolean) : [], group: name,
        })),
      })),
    };
  }
  ...
  async function persistArtifactContentJson(rows: StakeholderRow[]) {
    const res = await fetch(`/api/artifacts/${encodeURIComponent(artifactId)}/content-json`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectId, title: "Stakeholder Register", content_json: buildDoc(rows) }) });
    const json = await safeJson(res);
  ...
  async function saveNow() {
    ...
    startSaving(async () => {
      try {
        ...
        for (const r of doc.rows ?? []) { const key = normalizeNameKey(r?.name); if (!key) continue; if (seen.has(key)) throw new Error(`Duplicate name: "${String(r?.name ?? "").trim()}".`); seen.add(key); }
        const items = (doc.rows ?? []).map((r) => {
          const name = String(r.name ?? "").trim(); if (!name) return null;
          const impact_ui: Impact = r.impact_level === "High" ? "High" : r.impact_level === "Low" ? "Low" : "Medium";
          return { id: isDbUuid(r.id) ? r.id : undefined, name, name_key: normalizeNameKey(name), role: String(r.role ?? "").trim() || null, influence_level: normalizeUiInfluenceToDb(r.influence_level), expectations: null, communication_strategy: null, contact_info: { point_of_contact: String(r.point_of_contact ?? "").trim(), internal_external: r.internal_external ?? "Internal", title_role: String(r.title_role ?? "").trim(), stakeholder_mapping: r.stakeholder_mapping ?? inferMapping(r.influence_level, impact_ui), involvement_milestone: String(r.involvement_milestone ?? "").trim(), stakeholder_impact: String(r.stakeholder_impact ?? "").trim(), channels: Array.isArray(r.channels) ? r.channels.map(normalizeChannel).filter(Boolean) : [], group: normalizeGroup(r.group ?? "Project"), impact_level: impact_ui } };
        }).filter(Boolean);
        const res = await fetch(`/api/stakeholders`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectId, artifactId, items }) });
        const json = await safeJson(res);
        if (!res.ok || !json?.ok) { const msg = String(json?.error || "Save failed"); throw new Error(msg.toLowerCase().includes("duplicate") || msg.toLowerCase().includes("unique") ? "A stakeholder with that name already exists. Please use a unique name." : msg); }
        await persistArtifactContentJson(doc.rows ?? []);
        setDirty(false); await loadFromDb();
  ...
  const toggleExpand = (id: string) => setExpandedRows((p) => { const n = new Set(p); n.has(id) ? n.delete(id) : n.add(id); return n; });
  ...
  function filenameFromDisposition(h: string | null) {
    ...
    const raw = decodeURIComponent(m?.[1] || m?.[2] || m?.[3] || ""); return raw ? raw.trim() : null;
  }
  async function downloadFromRoute(opts: { url: string; fallbackName: string; mime?: string }) {
    ...
    u.searchParams.set("projectId", projectId); u.searchParams.set("artifactId", artifactId);
    const res = await fetch(u.toString(), { method: "GET", cache: "no-store" });
    if (!res.ok) { const j = await safeJson(res); throw new Error(j?.error || `Export failed (${res.status})`); }
  ...
  async function runExport(opts: { url: string; fallbackName: string; mime?: string }) {
    ...
    try { await downloadFromRoute(opts); } catch (e: any) { setErr(String(e?.message ?? e)); } finally { setShowDownloadMenu(false); }
  }
  const D = new Date().toISOString().slice(0, 10);
  const downloadExcel = () => runExport({ url: "/api/artifacts/stakeholder-register/export/xlsx", fallbackName: `Stakeholder_Register_${D}.xlsx`, mime: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
  ...
  const downloadWord = () => runExport({ url: "/api/artifacts/stakeholder-register/export/docx", fallbackName: `Stakeholder_Register_${D}.docx`, mime: "application/vnd.openxmlformats-officedocument.wordprocessingml.document" });
  ...
  const Th = ({ colKey, children, last }: { colKey: string; children?: React.ReactNode; last?: boolean }) => (
    <th className={`${TH_CLS} relative group/th ${last ? "" : "border-r border-gray-200"}`}
      style={{ width: colWidths[colKey], minWidth: colWidths[colKey] }}>
      {children}
      <div className="absolute right-0 top-0 h-full w-3 cursor-col-resize flex items-center justify-center opacity-0 group-hover/th:opacity-100 z-10 transition-opacity"
      ...
      <div className="max-w-[1920px] mx-auto">
        ...
        <div className="flex items-center justify-between px-5 py-3 bg-white border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            {[
              ...
              <span key={label} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-gray-100 border border-gray-200 text-[11px] font-medium text-gray-600">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center bg-gray-100 border border-gray-200 rounded-lg p-0.5 gap-0.5">
              {(["table", "cards"] as const).map((m) => (
                <button key={m} onClick={() => setMode(m)} disabled={busy} type="button"
                  className={`flex items-center gap-1 px-3 py-1.5 rounded-md text-[12px] font-medium transition-all ${mode === m ? "bg-white shadow-sm text-gray-900 border border-gray-200" : "text-gray-500 hover:text-gray-700"}`}>
            ...
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-2.5 flex items-center text-gray-400 pointer-events-none"><Icons.Search /></div>
              <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} disabled={busy}
            ...
            {/* Filter */}
            <div className="relative">
              <select value={personFilter} onChange={(e) => setPersonFilter(e.target.value as any)} disabled={readOnly || busy}
              ...
              <div className="absolute inset-y-0 right-0 pr-2 flex items-center pointer-events-none text-gray-400"><Icons.ChevronDown /></div>
            </div>

            <div className="w-px h-5 bg-gray-200" />

            <button onClick={loadFromDb} disabled={busy} type="button"
            ...
            <button onClick={saveNow} disabled={readOnly || saving || !dirty || busy} type="button"
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-medium transition-all ${dirty ? "bg-gray-900 text-white hover:bg-gray-800" : "bg-gray-100 text-gray-400 cursor-default"}`}>
              <Icons.Save />
            ...
            <div className="relative" ref={downloadMenuRef}>
              <button onClick={() => setShowDownloadMenu((v) => !v)} disabled={busy} type="button"
              ...
              {showDownloadMenu && (
                <div className="absolute right-0 mt-1 w-44 bg-white rounded-lg border border-gray-200 shadow-xl py-1 z-50">
                  <button onClick={downloadExcel} disabled={busy} type="button" className="flex items-center gap-2 w-full px-3 py-2 text-[12px] text-gray-700 hover:bg-gray-50">
                  ...
                  <button onClick={downloadPDF} disabled={busy} type="button" className="flex items-center gap-2 w-full px-3 py-2 text-[12px] text-gray-700 hover:bg-gray-50">
                    <span className="text-red-500"><Icons.FilePdf /></span> PDF
                  </button>
                  <button onClick={downloadWord} disabled={busy} type="button" className="flex items-center gap-2 w-full px-3 py-2 text-[12px] text-gray-700 hover:bg-gray-50">
                    <span className="text-blue-600"><Icons.FileText /></span> Word (.docx)
                  </button>
                </div>
              )}
            </div>
            ...
            {!readOnly && (
              <button onClick={deleteAll} disabled={busy || !doc.rows?.length} type="button"
        ...
        {err && (
          <div className="flex items-center gap-2 px-5 py-2.5 bg-red-50 border-b border-red-200 text-[13px] text-red-700">
            <Icons.AlertCircle /><span className="flex-1">{err}</span>
            <button onClick={() => setErr(null)} type="button" className="p-0.5 hover:bg-red-100 rounded"><Icons.X /></button>
          </div>
        )}
        ...
        {pendingSugs.length > 0 && (
          <div className="flex items-center gap-3 px-5 py-2.5 bg-amber-50 border-b border-amber-200">
            <Icons.Lightbulb />
            <span className="text-[13px] text-gray-700 flex-1"><strong>{pendingSugs.length}</strong> stakeholder suggestion{pendingSugs.length > 1 ? "s" : ""} from Project Charter</span>
            <button onClick={() => document.getElementById("ai-suggestions-panel")?.scrollIntoView({ behavior: "smooth" })} disabled={busy} type="button"
              className="px-3 py-1 bg-white border border-amber-200 rounded-md text-[12px] text-gray-700 hover:bg-amber-50 transition-colors">Review</button>
            {!readOnly && (
              <button onClick={acceptAllSuggestions} disabled={busy} type="button"
                className="px-3 py-1 bg-gray-900 text-white rounded-md text-[12px] hover:bg-gray-800 transition-colors">{sugBusy ? "Accepting…" : "Accept All"}</button>
            )}
          </div>
        )}
          ...
          <div className="p-6">
            {filtered.length === 0 ? (
              <div className="text-center py-20">
                ...
                {!readOnly && <button onClick={() => addRow()} disabled={busy} type="button" className="inline-flex items-center gap-1.5 px-4 py-2 bg-gray-900 text-white rounded-lg text-[13px] font-medium hover:bg-gray-800 transition-colors"><Icons.Plus /> Add Stakeholder</button>}
              </div>
            ) : (
                ...
                {filtered.map((s) => {
                  ...
                  return (
                    <div key={s.id} className="group relative bg-white rounded-xl border border-gray-200 p-4 hover:border-gray-300 hover:shadow-sm transition-all">
                      {!readOnly && (
                        <button onClick={() => removeRow(s.id)} disabled={busy || deletingId === s.id} type="button"
                      ...
                      {(s.channels ?? []).length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-2.5">
                          {s.channels!.map((c) => <span key={c} className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-medium border ${channelChip(c)}`}>{c}</span>)}
                        </div>
                      )}
                      {s.point_of_contact && (
                        <div className="flex items-center gap-1.5 text-[11px] text-gray-400 mt-2.5 pt-2.5 border-t border-gray-100">
                          <Icons.Mail /><span className="truncate">{s.point_of_contact}</span>
                        </div>
                      )}
                    </div>
                  );
          ...
          <div className="relative">
            ...
            {groups.length === 0 ? (
              <div className="text-center py-20 bg-white">
                ...
                {!readOnly && <button onClick={() => addRow()} disabled={busy} type="button" className="inline-flex items-center gap-1.5 px-4 py-2 bg-gray-900 text-white rounded-lg text-[13px] font-medium hover:bg-gray-800 transition-colors"><Icons.Plus /> Add First Stakeholder</button>}
              </div>
            ) : (
                  ...
                  <div key={groupName} className="border-b border-gray-200">
                    ...
                    <div className="flex items-center justify-between px-5 py-3 bg-white group/grp">
                      ...
                      <div className="flex items-center gap-2 opacity-0 group-hover/grp:opacity-100 transition-opacity">
                        <button type="button" aria-label="Group options" className="p-1.5 rounded text-gray-400 hover:bg-gray-100 transition-colors"><Icons.DotsVertical /></button>
                        {!readOnly && (
                          <button onClick={() => addRow(groupName)} disabled={busy} type="button"
                      ...
                      <>
                        <div className="overflow-x-auto border-t border-gray-200" style={{ cursor: resizingCol ? "col-resize" : "default" }}>
                          <table ref={tableRef} className="w-full border-collapse text-[13px]">
                            <colgroup>
                              {Object.entries(DEFAULT_COL_WIDTHS).filter(([k]) => k !== "actions" || !readOnly).map(([k]) => (
                                ...
                                return (
                                  <tr key={r.id} className="group/row hover:bg-[#f9f9f8] transition-colors">
                                    ...
                                    <td className={`${TD_CLS} border-r border-gray-100`} style={{ width: colWidths.name }}>
                                      <input className={INLINE_INPUT_CLS} value={r.name ?? ""} onChange={(e) => updateRow(r.id, { name: e.target.value })} readOnly={readOnly} placeholder="Stakeholder name…" disabled={busy} aria-label="Stakeholder name" />
                                    </td>

                                    {/* Contact */}
                                    <td className={`${TD_CLS} border-r border-gray-100`} style={{ width: colWidths.point_of_contact }}>
                                      <div className="flex items-center">
                                        <span className="pl-4 text-gray-300 shrink-0 pointer-events-none"><Icons.Mail /></span>
                                        <input className={`${INLINE_INPUT_CLS} pl-2`} value={r.point_of_contact ?? ""} onChange={(e) => updateRow(r.id, { point_of_contact: e.target.value })} readOnly={readOnly} placeholder="Email / Phone…" disabled={busy} aria-label="Contact details" />
                                      </div>
                                    </td>

                                    {/* Role */}
                                    <td className={`${TD_CLS} border-r border-gray-100`} style={{ width: colWidths.role }}>
                                      <input className={INLINE_INPUT_CLS} value={r.role ?? ""} onChange={(e) => updateRow(r.id, { role: e.target.value })} readOnly={readOnly} placeholder="Role…" disabled={busy} aria-label="Role" />
                                    </td>

                                    {/* Type — badge select */}
                                    <td className={`${TD_CLS} border-r border-gray-100 px-4`} style={{ width: colWidths.internal_external }}>
                                      <BadgeSelect value={r.internal_external ?? "Internal"} onChange={(v) => updateRow(r.id, { internal_external: v as InternalExternal })}
                                        options={[{ value: "Internal", label: "Internal" }, { value: "External", label: "External" }]}
                                        disabled={readOnly || busy} pillFn={typePill} />
                                    </td>

                                    {/* Title */}
                                    <td className={`${TD_CLS} border-r border-gray-100`} style={{ width: colWidths.title_role }}>
                                      <input className={INLINE_INPUT_CLS} value={r.title_role ?? ""} onChange={(e) => updateRow(r.id, { title_role: e.target.value })} readOnly={readOnly} placeholder="Title…" disabled={busy} aria-label="Title / Role" />
                                    </td>

                                    {/* Impact — badge select */}
                                    <td className={`${TD_CLS} border-r border-gray-100 px-4`} style={{ width: colWidths.impact_level }}>
                                      <BadgeSelect value={r.impact_level ?? "Medium"} onChange={(v) => updateRow(r.id, { impact_level: v as Impact })}
                                        options={[{ value: "High", label: "High" }, { value: "Medium", label: "Medium" }, { value: "Low", label: "Low" }]}
                                        disabled={readOnly || busy} pillFn={levelPill} showDot />
                                    </td>

                                    {/* Influence — badge select */}
                                    <td className={`${TD_CLS} border-r border-gray-100 px-4`} style={{ width: colWidths.influence_level }}>
                                      <BadgeSelect value={r.influence_level ?? "Medium"} onChange={(v) => updateRow(r.id, { influence_level: v as Influence })}
                                        options={[{ value: "High", label: "High" }, { value: "Medium", label: "Medium" }, { value: "Low", label: "Low" }]}
                                        disabled={readOnly || busy} pillFn={levelPill} showDot />
                                    </td>
                                    ...
                                    <td className={`${TD_CLS} border-r border-gray-100 px-4`} style={{ width: colWidths.stakeholder_mapping }}>
                                      <BadgeSelect value={r.stakeholder_mapping ?? "Keep Informed"} onChange={(v) => updateRow(r.id, { stakeholder_mapping: v as Mapping })}
                                        options={[{ value: "Manage Closely", label: "Manage Closely" }, { value: "Keep Satisfied", label: "Keep Satisfied" }, { value: "Keep Informed", label: "Keep Informed" }, { value: "Monitor", label: "Monitor" }]}
                                        disabled={readOnly || busy} pillFn={mappingPill} />
                                    </td>

                                    {/* Milestone */}
                                    <td className={`${TD_CLS} border-r border-gray-100`} style={{ width: colWidths.involvement_milestone }}>
                                      <input className={INLINE_INPUT_CLS} value={r.involvement_milestone ?? ""} onChange={(e) => updateRow(r.id, { involvement_milestone: e.target.value })} readOnly={readOnly} placeholder="Milestone…" disabled={busy} aria-label="Involvement milestone" />
                                    </td>

                                    {/* Impact Notes */}
                                    <td className={`${TD_CLS} border-r border-gray-100`} style={{ width: colWidths.stakeholder_impact }}>
                                      <div className="relative">
                                        <textarea className={`${INLINE_INPUT_CLS} resize-none leading-snug`} value={r.stakeholder_impact ?? ""} onChange={(e) => updateRow(r.id, { stakeholder_impact: e.target.value })} readOnly={readOnly} placeholder="Notes…" disabled={busy} rows={isExpanded ? 3 : 1} style={{ minHeight: isExpanded ? 64 : 36 }} aria-label="Impact notes" />
                                        {r.stakeholder_impact && r.stakeholder_impact.length > 40 && (
                                          <button onClick={() => toggleExpand(r.id)} type="button" disabled={busy} aria-label={isExpanded ? "Collapse notes" : "Expand notes"} className="absolute bottom-1 right-1 p-0.5 text-gray-300 hover:text-gray-600 rounded transition-colors"><Icons.Expand /></button>
                                        )}
                                      </div>
                                    </td>
                                    ...
                                    <td className={`${TD_CLS} border-r border-gray-100`} style={{ width: colWidths.channels }}>
                                      <div className="px-3 py-1.5">
                                        <MultiSelectPopover
                                          ...
                                          onChange={(next) => updateRow(r.id, { channels: Array.isArray(next) ? next.map(normalizeChannel).filter(Boolean) : [] })}
                                    ...
                                    {!readOnly && (
                                      <td className={TD_CLS} style={{ width: colWidths.actions }}>
                                        <div className="flex items-center justify-center opacity-0 group-hover/row:opacity-100 transition-opacity px-2">
                                          <button onClick={() => removeRow(r.id)} disabled={busy || deletingId === r.id} type="button"
