...
function IconArrowUpDown() {
  return (
    <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
      <path d="M6.5 1v11M3.5 4l3-3 3 3M3.5 9l3 3 3-3" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}

function IconArrowChildIndent() {
  return (
    <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
      <path d="M2 2v7h9M8 6l3 3-3 3" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
function IconArrowRight() {
  return (
    <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
      <path d="M2 6.5h9M8 3l3 3.5-3 3.5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
function IconArrowLeft() {
  return (
    <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
      <path d="M11 6.5H2M5 3L2 6.5 5 10" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
function IconSparkle() {
  return (
    <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
      <path d="M6.5 1L8 4.5h3.5L8.5 6.5 10 10 6.5 8 3 10l1.5-3.5L2 4.5h3.5L6.5 1z" fill="currentColor" fillOpacity="0.8" />
    </svg>
  );
}
...
function uuidish() {
  try {
    ...
    const c = g?.crypto;
    return c?.randomUUID?.() ?? `r_${Math.random().toString(16).slice(2)}_${Date.now()}`;
  } catch {
...
async function syncWbsItems(args: { projectId: string; artifactId: string; rows: any[] }) {
  try {
    ...
    if (!resp.ok) {
      ...
      console.warn("[WBS sync] failed", resp.status, t.slice(0, 500));
    }
  } catch (e) {
...
const LEVEL_BG = ["bg-slate-50", "bg-white", "bg-white", "bg-white", "bg-white", "bg-white"];

/* ---------------- artifact creation (robust) ---------------- */

async function tryCreateArtifactViaEndpoints(body: any): Promise<any> {
  const endpoints = ["/api/artifacts", "/api/artifacts/create", "/api/artifacts/new"];
  ...
  for (const url of endpoints) {
    try {
      ...
      if (!resp.ok || (j as any)?.ok === false) {
        throw new Error(
          safeStr((j as any)?.error) ||
            safeStr((j as any)?.message) ||
            safeStr((j as any)?.details) ||
      ...
      <div className="flex items-center gap-2 px-3 pt-3 pb-2.5">
        ...
        {isParent ? (
          <button
            ...
            className="w-6 h-6 rounded-md flex items-center justify-center text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-all shrink-0"
          >
...
export default function WBSEditor({
  ...
  const [rowsState, dispatchRows] = useReducer(rowsReducer, undefined as any, () => {
    return { byId: {}, order: [] } as RowsState;
  });
  ...
  function progressShownForRow(r: any, autoRollup: boolean) {
    ...
    return Math.round(statusScore((((r.status ?? "not_started") as WbsStatus) || "not_started") as WbsStatus) * 100);
  }
  ...
  function rowMatchesSlicers(r: any) {
    const qq = safeLower(q),
      ownerF = safeLower(ownerFilter),
      tagF = safeLower(tagFilter);

    const deliverable = safeLower(r.deliverable),
      desc = safeLower(r.description),
      ac = safeLower(r.acceptance_criteria),
      owner = safeLower(r.owner),
      pred = safeLower(r.predecessor),
      tags = (r.tags ?? []).map((t: string) => safeLower(t));
    ...
    if (tagF) {
      const hit =
        tags.some((t: string) => t.includes(tagF)) ||
        deliverable.includes(tagF) ||
        safeLower(r.predecessor).includes(tagF);
      if (!hit) return false;
    }
  ...
  async function ensureArtifactIdOrCreate(content: any): Promise<string> {
    ...
    try {
      window.dispatchEvent(new CustomEvent("artifact-created", { detail: { artifactId: newId, projectId: safeProjectId } }));
    } catch {}
    ...
    function onBeforeUnload() {
      try {
        ...
        (navigator as any).sendBeacon?.(
          `/api/artifacts/${aid}/content-json`,
      ...
      try {
        ...
        for (let k = 0; k < children.length; k++) {
          ...
          insertAt(insertIndex + k, {
            ...
            tags: Array.isArray((c as any).tags) ? (c as any).tags.map((t: any) => safeStr(t)).filter(Boolean) : [],
          });
        }
    ...
    const menu = open ? (
      ...
      >
        {[
          {
            label: "Add sibling",
            action: () => addSibling(rowId),
            disabled: !!readOnly,
            icon: <IconArrowUpDown />,
          },
          {
            label: "Add child",
            action: () => addChild(rowId),
            disabled: !!readOnly,
            icon: <IconArrowChildIndent />,
          },
          null,
          {
            label: "Indent",
          ...
          item === null ? (
            ...
            <button
              ...
              className={`w-full text-left px-3.5 py-2 text-sm flex items-center gap-2.5 transition-colors disabled:opacity-35 ${
                (item as any).danger ? "text-rose-600 hover:bg-rose-50" : "text-slate-700 hover:bg-slate-50"
              }`}
              onClick={() => {
                setOpenRowId(null);
                (item as any).action();
      ...
      <header className="sticky top-0 z-40 bg-white border-b border-slate-200 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
        <div className="max-w-[1800px] mx-auto px-5 h-14 flex items-center gap-3">
          ...
          <div className="hidden lg:flex items-center gap-1.5">
            <span className="text-xs px-2.5 py-1 rounded-full bg-slate-100 text-slate-600 font-medium tabular-nums">
              {totalRows} items
            </span>
            <span className="text-xs px-2.5 py-1 rounded-full bg-teal-100 text-teal-700 font-medium tabular-nums">
              {doneCount} done
            </span>
            {blockedCount > 0 && (
              <span className="text-xs px-2.5 py-1 rounded-full bg-rose-100 text-rose-700 font-medium tabular-nums">
                {blockedCount} blocked
              </span>
            )}
            {missingEffortCount > 0 && (
              <span className="text-xs px-2.5 py-1 rounded-full bg-amber-100 text-amber-700 font-medium tabular-nums">
          ...
          <div className="flex items-center gap-1.5">
            ...
            >
              <svg width="11" height="11" viewBox="0 0 11 11" fill="none">
                <path
                  d="M5.5 1L7 4h3.5L7.5 6l1.5 3.5L5.5 8 2.5 9.5 4 6 1 4h3.5L5.5 1z"
                  fill="currentColor"
                  fillOpacity="0.7"
                />
              </svg>
            ...
            >
              <span className="text-sm leading-none font-bold">+</span> Add item
            </button>
          ...
          <div className="px-4 py-3 flex flex-wrap items-center gap-2">
            ...
            <div className="flex flex-wrap items-center gap-2 ml-auto">
              ...
              <button
                ...
                disabled={!!readOnly}
                className="text-xs px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 disabled:opacity-40 transition-all"
              >
                Save view
              </button>
              ...
              {activeViewId !== "__all" && (
                <>
                  <button
                    ...
                    className="text-xs px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 transition-all"
                  >
                    Rename
                  </button>
                  <button
                    onClick={deleteActiveView}
                    className="text-xs px-2.5 py-1.5 rounded-lg border border-rose-200 text-rose-600 hover:bg-rose-50 transition-all"
                  >
                    Delete
                  </button>
                </>
              ...
              <button
                onClick={applyMyWorkFilter}
                className="text-xs px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 transition-all"
              >
                My Work
              </button>
              <button
                onClick={setMyWorkFromOwner}
                className="text-xs px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-50 transition-all"
              >
                Set owner
              </button>
              ...
              <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-slate-50">
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 rounded-md bg-slate-900 flex items-center justify-center">
                    <svg width="11" height="11" viewBox="0 0 11 11" fill="none">
                      <path
                        d="M5.5 1L7 3.8h3L7.5 5.6l1.5 3.4-3.5-2-3.5 2 1.5-3.4L1 3.8h3L5.5 1z"
                        fill="white"
                        fillOpacity="0.85"
                      />
                    </svg>
                  ...
                  <div className="text-center py-5">
                    <div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center mx-auto mb-2">
                      <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                        ...
                        <path d="M7 4.5v3M7 9v.5" stroke="#94a3b8" strokeWidth="1.3" strokeLinecap="round" />
                      </svg>
                    </div>
                  ...
                  {generatedDoc && (
                    <>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={() => generateWbs()}
                          className="px-3 py-2 text-sm rounded-lg border border-slate-200 hover:bg-slate-50 transition-all"
                        >
                          Retry
                        </button>
                      </div>
