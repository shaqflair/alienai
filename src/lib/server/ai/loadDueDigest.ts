...
export type DueDigestAi = {
  summary: string;
  windowDays: number;
  counts: DueDigestCounts;
  dueSoon: DueSoonItem[];
  dueSoon_legacy: DueDigestItem[];
  recommendedMessage: string;
};

export type PortfolioDueDigestResult = {
  scope: "org";
  ...
  dueSoon: DueSoonItem[];
  ai: DueDigestAi;
  stats: DueDigestStats;
};

export type ProjectDueDigestResult = {
  scope: "project";
...
export function parseWindowDays(raw: any, fallback: number): number {
...
async function buildDueDigestProject(args: {
  supabase: any;
  projectUuid: string;
  meta: ProjectMeta;
  windowDays: number;
}) {
  const { supabase, projectUuid, meta, windowDays } = args;
  const from = startOfUtcDay(new Date());
  const to = endOfUtcWindow(from, windowDays);
...
export async function loadPortfolioDueDigest(args: {
  supabase: any;
  userId: string;
  windowDays: number;
}): Promise<PortfolioDueDigestResult> {
  const { supabase, userId, windowDays } = args;

  const scoped = await loadScopedPortfolioProjects(supabase, userId);
  const projects = scoped.projects;

  if (!projects.length) {
    const ai: DueDigestAi = {
...
export async function loadProjectDueDigest(args: {
  supabase: any;
  projectUuid: string;
  meta: ProjectMeta;
  windowDays: number;
}): Promise<ProjectDueDigestResult> {
  const { supabase, projectUuid, meta, windowDays } = args;
  const ai = await buildDueDigestProject({ supabase, projectUuid, meta, windowDays });
