...
export type PortfolioAiBriefingFilters = {
  q?: string;
  projectId?: string[];
  projectCode?: string[];
  pm?: string[];
...
function clampDays(v: string | null | undefined): 7 | 14 | 30 | 60 | "all" {
...
export function parseAiBriefingFiltersFromUrl(url: URL): PortfolioAiBriefingFilters {
  ...
  const out: PortfolioAiBriefingFilters = {};
  if (q) out.q = q;
...
export function parseAiBriefingFiltersFromBody(body: any): PortfolioAiBriefingFilters {
  const f = body?.filters ?? body?.filter ?? body?.where ?? null;
  const out: PortfolioAiBriefingFilters = {};

  const q = safeStr(f?.q ?? body?.q).trim();
  const projectId = uniqList(f?.projectId ?? f?.projectIds);
  const projectCode = uniqList(f?.projectCode ?? f?.projectCodes ?? f?.code);
  const pm = uniqList(f?.pm ?? f?.projectManagerId ?? f?.projectManagerIds);
  const dept = uniqList(f?.dept ?? f?.department ?? f?.departments);
...
function hasActiveFilters(f: PortfolioAiBriefingFilters) {
  return Boolean(
    (f.q && f.q.trim()) ||
      f.projectId?.length ||
...
async function applyPortfolioFiltersToProjectIds(args: {
  ...
  filters: PortfolioAiBriefingFilters;
}) {
...
function buildFlowWarnings(args: {
  ...
  warnings.sort(
    (a, b) => (b.score ?? 0) - (a.score ?? 0) || sevRank(b.severity) - sevRank(a.severity),
  );
  return warnings;
}
...
export async function loadAiBriefing(input: {
  userId: string;
  days?: unknown;
  filters?: PortfolioAiBriefingFilters;
