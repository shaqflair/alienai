...
function dedupeApprovers(rows: Array<{ userId: string; role: string }>) {
  ...
  for (const row of rows) {
    ...
    if (!userId) continue;
    if (!out.has(userId)) out.set(userId, { userId, role: safeStr(row?.role).trim() || "Approval" });
  }
 ...
 *   Math.max(1, Math.min(1, approverCount || 1))
 * which always returns 1 regardless of approver count.
 *
...
function resolveMinApprovals(mode: string, approverCount: number, templateMinApprovals: number): number {
  ...
  if (m === "any") return 1;
  // serial / parallel / explicit — respect template but never exceed actual approver count
  return Math.max(1, Math.min(templateMinApprovals || 1, approverCount || 1));
}

type RuleRow = {
  id: string; step: number; approval_role: string;
  approver_user_id: string | null; approval_group_id: string | null;
  min_amount: number; max_amount: number | null;
};
type StepApprover = { userId: string; role: string };
type ChainStepInsert = {
  chain_id: string; step_order: number; step_name: string; mode: string;
  min_approvals: number; max_rejections: number; round: number; is_active: boolean;
};
type PersistedChainStep = {
  id: string; chain_id: string; step_order: number;
  step_name?: string | null; mode?: string | null;
  min_approvals?: number | null; max_rejections?: number | null;
  round?: number | null; is_active?: boolean | null;
};
type ArtifactStepInsert = {
  artifact_id: string; chain_id: string; project_id: string; artifact_type: string;
  step_order: number; name: string; mode: string; min_approvals: number;
  max_rejections: number; round: number; status: string;
  approval_step_id: string; pending_since: string | null;
};
type PersistedArtifactStep = {
  id: string; artifact_id: string; chain_id: string; step_order: number;
  approval_step_id?: string | null; name?: string | null; mode?: string | null;
  min_approvals?: number | null; max_rejections?: number | null;
  round?: number | null; status?: string | null;
};
...
async function supersedeExistingArtifactChains(supabase: SupabaseClient, artifactId: string) {
  const { error } = await supabase.from("approval_chains").update({ is_active: false, status: "superseded" }).eq("artifact_id", artifactId).eq("is_active", true);
  if (error) throw new Error(`approval_chains supersede failed: ${error.message}`);
}

async function loadRulesForArtifact(supabase: SupabaseClient, organisationId: string, artifactTypeRaw: string, amount: number): Promise<RuleRow[]> {
  const artifactType = normalizeArtifactType(artifactTypeRaw);
  const { data, error } = await supabase.from("artifact_approver_rules")
    .select("id, step, approval_role, approver_user_id, approval_group_id, min_amount, max_amount, is_active")
    .eq("organisation_id", organisationId).eq("artifact_type", artifactType).eq("is_active", true)
  ...
  const rows = (Array.isArray(data) ? data : []).map((r: any) => ({
    id: String(r.id), step: Number(r.step ?? 1),
 ...
 * The approval_steps table is keyed by chain_id on the governance template chain,
 * not the runtime chain — so we query by organisation and artifact type instead.
 * Non-fatal: if the table doesn't exist or has no rows, we fall back to "all".
 */
async function loadGovernanceStepSettings(
  ...
  try {
    ...
    // non-fatal — governance template may not be set up
  }
  return out;
}
...
async function insertApprovalStepApprovers(supabase: SupabaseClient, rows: Array<{ step_id: string; user_id: string; email: string | null; role: string; status: string; }>) {
...
async function purgeExistingArtifactStepRows(supabase: SupabaseClient, artifactId: string) {
  const { data: existingSteps, error: existingStepsErr } = await supabase.from("artifact_approval_steps").select("id").eq("artifact_id", artifactId);
  ...
  if (stepIds.length) {
    const { error: approverDeleteErr } = await supabase.from("approval_step_approvers").delete().in("step_id", stepIds);
    if (approverDeleteErr) throw new Error(`approval_step_approvers cleanup failed: ${approverDeleteErr.message}`);
  }
  const { error: stepDeleteErr } = await supabase.from("artifact_approval_steps").delete().eq("artifact_id", artifactId);
...
async function insertApprovalChainSteps(supabase: SupabaseClient, rows: ChainStepInsert[]): Promise<PersistedChainStep[]> {
  if (!rows.length) return [];
  const { data, error } = await supabase.from("approval_chain_steps").insert(rows)
...
async function refetchApprovalChainSteps(supabase: SupabaseClient, chainId: string): Promise<PersistedChainStep[]> {
  const { data, error } = await supabase.from("approval_chain_steps")
    .select("id, chain_id, step_order, step_name, mode, min_approvals, max_rejections, round, is_active")
    .eq("chain_id", chainId).order("step_order", { ascending: true });
...
async function insertArtifactApprovalSteps(supabase: SupabaseClient, rows: ArtifactStepInsert[]): Promise<PersistedArtifactStep[]> {
  if (!rows.length) return [];
  const { data, error } = await supabase.from("artifact_approval_steps").insert(rows)
...
async function refetchArtifactApprovalSteps(supabase: SupabaseClient, artifactId: string, chainId: string): Promise<PersistedArtifactStep[]> {
  const { data, error } = await supabase.from("artifact_approval_steps")
    .select("id, artifact_id, chain_id, step_order, approval_step_id, name, mode, min_approvals, max_rejections, round, status")
    .eq("artifact_id", artifactId).eq("chain_id", chainId).order("step_order", { ascending: true });
...
async function verifyRuntimeChain(supabase: SupabaseClient, args: { chainId: string; artifactId: string }) {
  ...
  for (const row of artifactSteps) {
    ...
    if (!expectedDefinitionId) throw new Error(`artifact_approval_steps row ${stepId} has no matching approval_chain_steps row for order ${order}.`);
    if (!definitionId) throw new Error(`artifact_approval_steps row ${stepId} has null approval_step_id.`);
    if (definitionId !== expectedDefinitionId) throw new Error(`artifact_approval_steps row ${stepId} has approval_step_id=${definitionId} but expected ${expectedDefinitionId}.`);
  }
...
export async function buildRuntimeApprovalChain(
  ...
  for (const logicalStepNo of logicalStepNumbers) {
    ...
    for (const rule of stepRules) {
      ...
      if (rule.approval_group_id) {
        ...
        for (const memberId of members) collected.push({ userId: memberId, role: rule.approval_role });
      }
    }
  ...
  const activeLogicalSteps = logicalStepNumbers.filter((stepNo) => (approversByLogicalStep.get(stepNo) || []).length > 0);
  if (!activeLogicalSteps.length) throw new Error("No valid approvers resolved for the matching approval rules.");
  ...
  const { data: chain, error: cErr } = await supabase.from("approval_chains").insert({
    organisation_id: args.organisationId, project_id: args.projectId,
    artifact_id: args.artifactId, artifact_type: desiredType,
    is_active: true, status: "active", created_by: args.actorId,
  }).select("id").single();
  ...
  // 5. Load governance template settings to get mode + min_approvals
  const governanceSteps = await loadGovernanceStepSettings(supabase, args.organisationId, desiredType);

  // 6. Insert chain definition steps with correct mode + min_approvals
  ...
  const chainStepRows: ChainStepInsert[] = activeLogicalSteps.map((logicalStepNo, index) => {
    ...
    // Default to "all" — safest: requires everyone to approve
    ...
    return {
      chain_id: chainId, step_order: normalizedOrder,
      step_name: `Step ${normalizedOrder}`, mode, min_approvals: minApprovals,
      max_rejections: 0, round: 1, is_active: true,
    };
  });
  ...
  for (let i = 1; i <= activeLogicalSteps.length; i++) {
    if (!chainStepByOrder.get(i)?.id) throw new Error(`approval_chain_steps missing for normalized step order ${i}.`);
  }

  // 7. Create runtime artifact steps — min_approvals re-resolved from persisted chain step
  const artifactStepRows: ArtifactStepInsert[] = activeLogicalSteps.map((logicalStepNo, index) => {
    ...
    if (!chainStep?.id) throw new Error(`approval_chain_steps missing for normalized step order ${normalizedOrder}.`);
    const approvers = approversByLogicalStep.get(logicalStepNo) || [];
    const approverCount = approvers.length;
    ...
    return {
      artifact_id: args.artifactId, chain_id: chainId, project_id: args.projectId,
      artifact_type: desiredType, step_order: normalizedOrder,
      name: safeStr(chainStep.step_name).trim() || `Step ${normalizedOrder}`,
      mode: persistedMode, min_approvals: minApprovals,
  ...
  for (let i = 1; i <= activeLogicalSteps.length; i++) {
    if (!artifactStepIdByOrder.get(i)) throw new Error(`artifact_approval_steps missing for normalized step order ${i}.`);
  }

  // 8. Resolve approver emails
  const allUserIds = Array.from(new Set(Array.from(approversByLogicalStep.values()).flat().map((a) => safeStr(a.userId).trim()).filter(Boolean)));
  const emails = await getUserEmailsByIds(supabase, allUserIds);

  // 9. Create runtime approver rows
  const finalApproverRows: Array<{ step_id: string; user_id: string; email: string | null; role: string; status: string }> = [];
  for (let normalizedOrder = 1; normalizedOrder <= activeLogicalSteps.length; normalizedOrder++) {
    ...
    if (!runtimeStepId) throw new Error(`Inserted runtime approval step missing for normalized step order ${normalizedOrder}.`);
    const logicalStepNo = activeLogicalSteps[normalizedOrder - 1];
    const approvers = approversByLogicalStep.get(logicalStepNo) || [];
    for (const approver of approvers) {
      const userId = safeStr(approver.userId).trim();
      if (!userId) continue;
      finalApproverRows.push({ step_id: runtimeStepId, user_id: userId, email: emails.get(userId) || null, role: labelToRole(approver.role), status: "pending" });
    }
  }
  await insertApprovalStepApprovers(supabase, finalApproverRows);

  // 10. Verify end-to-end integrity
  const verified = await verifyRuntimeChain(supabase, { chainId, artifactId: args.artifactId });
  const verifiedRuntimeStepIds = verified.artifactSteps.map((s) => safeStr(s.id).trim()).filter(Boolean);

  return { chainId, stepIds: verifiedRuntimeStepIds, chosenType: desiredType };
}