...
export function scoreSchedule(
  ...
  if (wbsItems.length) {
    ...
    detail.wbsComplete = wbsItems.filter((w) => {
      const st = String(w?.status || w?.delivery_status || "")
        .toLowerCase()
        .replace(/\s+/g, "_");
  ...
  for (const m of milestones) {
    ...
    if (end && base) {
      const slip = Math.max(
        0,
        Math.round(
          (new Date(`${end}T00:00:00Z`).getTime() - new Date(`${base}T00:00:00Z`).getTime()) / 86_400_000,
        ),
      );
      slipSum += slip;
      slipCount++;
...
function raidType(r: any): "issue" | "risk" | "dependency" | "assumption" | "other" {
  ...
  if (t.includes("action")) return "issue";
  return "other";
}
...
function raidOpen(r: any): boolean {
  ...
  if (!st) return true;
  return !["closed", "resolved", "complete", "completed", "cancelled", "canceled"].some((k) =>
    st.includes(k),
  );
}
...
export function scoreBudget(
  ...
  if (allocatedDays != null && budgetDays != null && budgetDays > 0) {
    const allocPct = (allocatedDays / budgetDays) * 100;
    if (allocPct > 125) score -= 20;
    else if (allocPct > 110) score -= 10;
  }
...
export function computeHealthFromData(opts: {
  ...
  const { score: scheduleScore, detail: scheduleDetail } = scoreSchedule(
    opts.milestones,
    opts.wbsItems ?? [],
    today,
  );
  ...
  const { score: budgetScore, detail: budgetDetail } = scoreBudget(
    opts.budgetAmount,
    opts.spentAmount,
    opts.allocatedDays ?? null,
    opts.budgetDays ?? null,
  );

  const { score: resourceScore, detail: resourceDetail } = scoreResource(
    opts.allocatedDays ?? null,
    opts.budgetDays ?? null,
  );
...
async function fetchPortfolioBudget(
  ...
  const [budgetResult, spendResult, artResult] = await Promise.allSettled([
    supabase
      .from("projects")
      .select("id, budget_amount, resource_status, status")
      .in("id", projectIds)
      .limit(20000),
    supabase
      .from("project_spend")
  ...
  for (const art of artRows) {
    ...
    if (lineTotal > 0) {
      artSpentByProject.set(pid, (artSpentByProject.get(pid) ?? 0) + lineTotal);
    }
  }
...
async function fetchPortfolioGovernance(supabase: any, projectIds: string[]) {
  ...
  const gates: any[] = gatesRes.status === "fulfilled" ? gatesRes.value.data ?? [] : [];
  ...
  for (const pid of projectIds) {
    ...
    const gate1Row = gates.find(
      (g: any) => String(g.project_id) === pid && Number(g.gate_number) === 1,
    );
    const gate1Complete = !!(
      gate1Row?.passed_at ||
      gate1Row?.status === "passed" ||
      gate1Row?.status === "complete"
    );

    const gate5Row = gates.find(
      (g: any) => String(g.project_id) === pid && Number(g.gate_number) === 5,
    );
...
export async function computePortfolioHealth(
  ...
  const [milestonesRes, wbsRes, raidRes, budgetMap, govMap, allocMap] = await Promise.all([
  ...
  return {
    score: clamp(scoredProjects.reduce((s, r) => s + (r.score ?? 0), 0) / scoredProjects.length),
    parts: {
