...
export async function fetchReportData(filters: ReportFilters) {
  ...
  const [memberRes, allocRes, exceptionRes, projectRes, roleRes] = await Promise.all([
    ...
    // Allocations — filtered by org to avoid cross-org leakage
    supabase
      .from("allocations")
      .select(`
    ...
    // Capacity exceptions — filtered by org
    supabase
    ...
    // Role requirements (optional — silently ignore if table missing)
    supabase
  ...
  for (const m of memberRes.data ?? []) {
    ...
    const id = String(p.user_id || m.user_id);
    peopleMap.set(id, {
      personId:   id,
  ...
  let activePeople = Array.from(peopleMap.values()).filter(p => p.isActive);
  if (filters.personIds?.length)  activePeople = activePeople.filter(p => filters.personIds!.includes(p.personId));
  if (filters.departments?.length) activePeople = activePeople.filter(p => p.department && filters.departments!.includes(p.department));
  ...
  for (const a of allocRes.data ?? []) {
    ...
    const days = parseFloat(String(a.days_allocated)) || 0;
    ...
    if (a.projects && !projectMetaFromAlloc.has(proj)) projectMetaFromAlloc.set(proj, a.projects);
  }
  ...
  const utilisationByPerson: PersonUtilRow[] = activePeople.map(person => {
    ...
    const weekRows = weeks.map(w => {
      ...
      const allocated = Array.from(personAllocs.values()).reduce((s, wm) => s + (wm.get(w) ?? 0), 0);
  ...
  const utilisationByProject: ProjectUtilRow[] = confirmedProjects.map((proj: any) => {
    ...
    for (const [pid, weekMap] of projAllocs) {
      ...
      for (const [w, d] of weekMap) weekTotals.set(w, (weekTotals.get(w) ?? 0) + d);
    }
  ...
  const costReport: CostRow[] = activePeople
    .map(person => {
      const pid          = person.personId;
      const personAllocs = allocByPerson.get(pid) ?? new Map();
  ...
  const leaveSummary: LeaveRow[] = activePeople
    .map(person => {
      const pid              = person.personId;
      const personExceptions = exMap.get(pid) ?? new Map();
      ...
      return {
        ...
        exceptions,
        totals: {
          totalDaysLost: Math.round(exceptions.reduce((s, e) => s + e.daysLost, 0) * 10) / 10,
  ...
  const pipelineForecast: PipelineRow[] = pipelineProjects
    .map((proj: any) => {
      const projectId = String(proj.id);
