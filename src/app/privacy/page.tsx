...
export default function PrivacyPage() {
  const sections = [
    ...
    { title:"8. Security",               body:"We implement appropriate technical and organisational measures including encryption at rest (AES-256), encryption in transit (TLS 1.2+), and database-level Row-Level Security policies. See our Security page for full details." },
    { title:"9. International transfers",body:"Some sub-processors (Vercel, OpenAI, Resend, Stripe) are based in the United States. Transfers are made under Standard Contractual Clauses (SCCs) as approved by the UK ICO." },
  ...
  return (
    <main style={{ minHeight:"100vh", background:"linear-gradient(180deg,#03050A 0%,#060C14 50%,#04080F 100%)", color:C.text, fontFamily:"'Inter',system-ui,sans-serif", WebkitFontSmoothing:"antialiased" as any }}>
      ...
      <div aria-hidden style={{ position:"fixed", top:0, left:"50%", transform:"translateX(-50%)", width:900, height:500, background:"radial-gradient(ellipse,rgba(0,194,232,0.04) 0%,transparent 70%)", pointerEvents:"none", zIndex:0 }} />

      <div style={{ maxWidth:820, margin:"0 auto", padding:"80px 28px", position:"relative", zIndex:1 }}>

        <a href="/" style={{ display:"inline-flex", alignItems:"center", gap:6, fontSize:13, color: hovBack ? C.text : C.muted, transition:"color 0.2s", marginBottom:40 }}
          onMouseEnter={()=>setHovBack(true)} onMouseLeave={()=>setHovBack(false)}>
          &larr; Back to Aliena AI
        </a>

        <div style={{ marginBottom:56 }}>
          <div style={{ fontFamily:mn, fontSize:10, color:C.cyanLt, letterSpacing:"0.14em", textTransform:"uppercase" as any, marginBottom:14 }}>Legal</div>
        ...
        <div style={{ display:"grid", gap:0 }}>
          {sections.map((s,i)=>(
            <section key={s.title} style={{ paddingBottom:36, marginBottom:36, borderBottom: i < sections.length-1 ? "1px solid rgba(255,255,255,0.06)" : "none" }}>
              ...
              <div style={{ fontSize:14, color:C.muted, lineHeight:1.85, whiteSpace:"pre-line" as any }}>{s.body}</div>
            </section>
          ))}
        </div>
        ...
        <div style={{ borderTop:"1px solid rgba(255,255,255,0.07)", paddingTop:28, display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:14 }}>
          ...
          <a href="/security" style={{ fontSize:13, color: hovSec ? C.cyanLt : C.muted, transition:"color 0.2s" }}
