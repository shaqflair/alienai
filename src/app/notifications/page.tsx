...
function projectLabel(n: NotificationRow) {
  ...
  // Prefer explicit human identifiers if you store them
  const code = safeStr(
    md.project_code ??
      md.projectCode ??
  ...
  const name = safeStr(
    md.project_name ?? md.projectName ?? md.project_title ?? md.projectTitle
  ).trim();
                  ...
                  topOverdue.map((n) => (
                    ...
                    >
                      ...
                      <div className="mt-0.5 text-[11px] text-slate-600">
                        Due: <span className="text-rose-600 font-medium">{formatUkDateFromYmd(n.due_date)}</span>
                      </div>
                    </button>
                  ))
                )}
                ...
                {topDueSoon.length === 0 ? (
                  ...
                  topDueSoon.map((n) => (
                    ...
                    >
                      ...
                      <div className="mt-1 truncate text-sm text-slate-900 font-semibold">{n.title}</div>
                      <div className="mt-0.5 text-[11px] text-slate-600">
                        Due: <span className="text-sky-600 font-medium">{formatUkDateFromYmd(n.due_date)}</span>
                      </div>
                    </button>
                  ))
                )}
