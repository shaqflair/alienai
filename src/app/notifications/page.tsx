...
const POLL_MS = 30_000; // ✅ quieter polling (less load, less flicker)
const LIST_LIMIT = 400;
const DEBUG = false; // ✅ never leak debug in prod
 ...
 * If we don't have project code/name in metadata, just show "Project".
 */
function projectLabel(n: NotificationRow) {
  ...
  const code = safeStr(
    md.project_code ??
      md.projectCode ??
      md.project_code_text ??
      md.projectHumanId ??
      md.project_human_id ??
      md.project_code_human
  ).trim();

  const name = safeStr(md.project_name ?? md.projectName ?? md.project_title ?? md.projectTitle).trim();
...
function dueYmd(n: NotificationRow) {
  return (
    parseYmd(n?.due_date) ||
    parseYmd(n?.metadata?.dueDate) ||
    parseYmd(n?.metadata?.due_date) ||
    parseYmd(n?.metadata?.due)
  );
}
...
function normalizeLink(href: any) {
...
export default function NotificationsPage() {
  ...
  // ✅ guards against setState after unmount + overlapping refresh calls
  ...
  const computeKpisFromList = useCallback((list: NotificationRow[]) => {
    const unread = (i: NotificationRow) => i.is_read !== true;
  ...
  const fetchList = useCallback(
    async (signal?: AbortSignal) => {
      ...
      if (!payload || (payload as any).ok !== true) throw new Error((payload as any)?.error || "Failed to load notifications");
    ...
    [tab, kpisLoaded, computeKpisFromList]
  );
  ...
  const grouped = useMemo(() => {
    ...
    return order.map((k) => [k, map.get(k) ?? []] as const).filter(([, rows]) => rows.length > 0);
  }, [filteredItems]);
      ...
      <div className="relative mx-auto max-w-4xl px-6 py-10 z-10">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-slate-600">Overdue, due soon, approvals, AI warnings, actions and portfolio signals.</p>
                  ...
                  topOverdue.map((n) => (
                    ...
                    >
                      ...
                      <div className="mt-0.5 text-[11px] text-slate-600">
                        Due: <span className="text-rose-600 font-medium">{formatUkDateFromYmd(dueYmd(n))}</span>
                      </div>
                    </button>
                  ))
                )}
        ...
        <div className="mt-8">
          <AnimatePresence mode="wait">
            {loading ? (
              ...
              >
                {subtab === "overdue" ? "No overdue items 🎉" : subtab === "due_soon" ? "No due-soon items" : "No notifications 🎉"}
                              ...
                              <div className="min-w-0 flex-1">
                                <div className="flex items-start justify-between gap-3">
                                  <div className="min-w-0">
                                    <div className="text-[11px] font-semibold text-slate-500 truncate">{projectLabel(n)}</div>
                                ...
                                {dueLabel ? (
                                  <div className="mt-1 text-[11px] text-slate-500">
                                    Due:{" "}
                                    <span className={cn("font-medium", overdue ? "text-rose-600" : dueSoon ? "text-sky-600" : "text-slate-700")}>
                                      {dueLabel}
                                    </span>
                                  </div>
                                ) : null}
