...
function ConflictTable({ weeks, onSkipExceptionWeeks }: {
  ...
  return (
    <div style={{ background: "#f8fafc", borderRadius: "10px", border: "1.5px solid #e2e8f0", overflow: "hidden" }}>
      <div style={{
        padding: "10px 14px",
        background: conflictCount > 0 ? "rgba(239,68,68,0.05)" : "rgba(16,185,129,0.05)",
        ...
        {[
          ...
          { l: "Peak util",  v: utilLabel(maxUtil), c: utilColour(maxUtil) },
          ...
          <div key={s.l}>
            ...
            <div style={{ fontSize: "15px", fontWeight: 800, color: s.c, fontFamily: "'DM Mono', monospace" }}>{s.v}</div>
          </div>
        ))}
      </div>

      {exceptionConflicts.length > 0 && (
        <div style={{ padding: "10px 14px", background: "rgba(245,158,11,0.06)", borderBottom: "1px solid #fde68a", display: "flex", alignItems: "flex-start", gap: "10px" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: "11px", color: "#b45309", lineHeight: 1.5 }}>
              ...
              {onSkipExceptionWeeks && firstFullWeek && pureConflicts.length === 0 && (
                <span>
                  {" "}
                  <button type="button" onClick={() => onSkipExceptionWeeks(firstFullWeek.week_start)}
                    style={{ background: "none", border: "none", padding: 0, color: "#d97706", fontWeight: 700, fontSize: "11px", cursor: "pointer", textDecoration: "underline" }}>
          ...
          <thead>
            <tr style={{ background: "#f1f5f9" }}>
              {["Week of", "Existing", "+ New", "Total", "Capacity", "Util %"].map(h => (
                <th key={h} style={{ padding: "7px 10px", fontSize: "10px", fontWeight: 700, color: "#64748b", textAlign: h === "Week of" ? "left" : "center", letterSpacing: "0.04em", textTransform: "uppercase", borderBottom: "1px solid #e2e8f0" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {weeks.map((row, i) => (
              <tr key={row.week_start} style={{ background: row.has_conflict ? utilBg(row.utilisation_pct) : "transparent", borderBottom: i < weeks.length - 1 ? "1px solid #f1f5f9" : "none" }}>
                <td style={{ padding: "7px 10px", fontSize: "12px", color: "#334155", fontFamily: "'DM Mono', monospace" }}>
                  {new Date(row.week_start).toLocaleDateString("en-GB", { day: "2-digit", month: "short" })}
                </td>
                {[row.existing_days+"d", row.proposed_days+"d", row.total_days+"d"].map((v, j) => (
                  <td key={j} style={{ padding: "7px 10px", textAlign: "center", fontSize: "12px", color: "#475569", fontFamily: "'DM Mono', monospace", fontWeight: j === 2 ? 700 : 400 }}>{v}</td>
                ))}
                <td style={{ padding: "7px 10px", textAlign: "center", fontSize: "12px", fontFamily: "'DM Mono', monospace" }}>
                  <span style={{ color: row.is_exception ? "#f59e0b" : "#475569" }} title={row.is_exception ? "Reduced capacity this week" : undefined}>
                    {row.capacity_days}d{row.is_exception && " !"}
                  </span>
                </td>
                <td style={{ padding: "7px 10px", textAlign: "center" }}>
                  <span style={{ fontSize: "12px", fontWeight: 700, fontFamily: "'DM Mono', monospace", color: utilColour(row.utilisation_pct), background: utilBg(row.utilisation_pct), borderRadius: "5px", padding: "2px 7px" }}>
                    {utilLabel(row.utilisation_pct)}{row.has_conflict && " (!)"}
                  </span>
                </td>
              </tr>
            ))}
...
function AlternativesPanel({ alternatives, onSelect }: { alternatives: Alternative[]; onSelect: (personId: string) => void }) {
  if (!alternatives.length) return null;
  const canCover = alternatives.filter(a => a.can_cover_fully);
  const partial  = alternatives.filter(a => !a.can_cover_fully);

  return (
    <div style={{ border: "1.5px solid #bae6f0", borderRadius: "12px", background: "#f0fdff", overflow: "hidden" }}>
      <div style={{ padding: "12px 16px", borderBottom: "1px solid #bae6f0", display: "flex", alignItems: "center", gap: "8px" }}>
        <div>
          <div style={{ fontSize: "13px", fontWeight: 700, color: "#0e7490" }}>Alternative people with capacity</div>
          <div style={{ fontSize: "11.5px", color: "#64748b" }}>{canCover.length} can cover fully &nbsp; {partial.length} partial coverage</div>
        </div>
      </div>
      <div style={{ padding: "12px 16px", display: "flex", flexDirection: "column", gap: "8px" }}>
        {alternatives.map(alt => (
          <div key={alt.person_id} style={{ background: "white", border: "1.5px solid", borderColor: alt.can_cover_fully ? "#86efac" : "#e2e8f0", borderRadius: "10px", padding: "10px 12px", display: "flex", alignItems: "center", gap: "10px" }}>
            <Avatar name={alt.full_name} size={32} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: "13px", fontWeight: 600, color: "#0f172a", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{alt.full_name}</div>
              <div style={{ fontSize: "11px", color: "#64748b" }}>
                {alt.job_title || "--"} <span style={{ color: utilColour(alt.avg_utilisation_pct), fontWeight: 600 }}>{utilLabel(alt.avg_utilisation_pct)} avg util</span>
              </div>
            </div>
            <div style={{ textAlign: "right", flexShrink: 0 }}>
              <div style={{ fontSize: "12px", fontWeight: 700, color: alt.can_cover_fully ? "#10b981" : "#f59e0b", fontFamily: "'DM Mono', monospace" }}>{alt.min_available_days}d min free</div>
              {alt.can_cover_fully && <div style={{ fontSize: "10px", color: "#10b981", fontWeight: 600 }}>Can cover fully</div>}
            </div>
            <button type="button" onClick={() => onSelect(alt.person_id)}
              style={{ background: alt.can_cover_fully ? "#00b8db" : "white", border: `1.5px solid ${alt.can_cover_fully ? "#00b8db" : "#e2e8f0"}`, borderRadius: "7px", padding: "6px 12px", fontSize: "12px", fontWeight: 700, cursor: "pointer", color: alt.can_cover_fully ? "white" : "#475569", fontFamily: "'DM Sans', sans-serif", whiteSpace: "nowrap" }}>
              Use instead
            </button>
          </div>
        ))}
...
export default function AllocateForm({
  ...
  // Auto-fill role from person job title when person changes
  useEffect(() => {
    if (selectedPerson?.job_title) {
      setRoleOnProject(selectedPerson.job_title);
    }
  }, [personId]);
  ...
  const runCheck = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/capacity/check", {
        ...
        body: JSON.stringify({ person_id: personId, project_id: projectId, start_date: startDate, end_date: endDate, days_per_week: effectiveDaysPerWeek, organisation_id: organisationId }),
      });
    ...
    <form action={createAllocation} style={{ display: "flex", flexDirection: "column", gap: "22px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
        <div>
          ...
          {selectedPerson && (
            <div style={{ marginTop: "8px", padding: "8px 12px", borderRadius: "8px", background: "#f8fafc", border: "1px solid #e2e8f0", display: "flex", gap: "10px", alignItems: "center" }}>
              <Avatar name={selectedPerson.full_name} size={28} />
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: "12px", fontWeight: 600, color: "#0f172a" }}>{selectedPerson.full_name}</div>
                <div style={{ fontSize: "11px", color: "#64748b" }}>
                  {selectedPerson.job_title}{" "}
                  <span style={{ fontWeight: 600, color: selectedPerson.employment_type === "part_time" ? "#f59e0b" : "#10b981" }}>
        ...
        <div>
          ...
          {selectedProject && (
            <div style={{ marginTop: "8px", padding: "8px 12px", borderRadius: "8px", background: "#f8fafc", border: "1px solid #e2e8f0", display: "flex", gap: "10px", alignItems: "center" }}>
              <div style={{ width: 4, height: 32, borderRadius: 2, background: selectedProject.colour || "#00b8db", flexShrink: 0 }} />
              <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
        <div>
          ...
          {selectedProject?.start_date && startDate < selectedProject.start_date && (
            <p style={{ fontSize: "11px", color: "#f59e0b", marginTop: "4px" }}>(!) Before project start ({selectedProject.start_date})</p>
          )}
        </div>
        <div>
          ...
          {selectedProject?.finish_date && endDate > selectedProject.finish_date && (
            <p style={{ fontSize: "11px", color: "#ef4444", marginTop: "4px" }}>(!) After project end ({selectedProject.finish_date})</p>
          )}
        </div>
      </div>
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: "0", marginBottom: "12px", borderRadius: "9px", border: "1.5px solid #e2e8f0", overflow: "hidden", width: "fit-content" }}>
          {([
            { id: "per_week",       label: "Days / week" },
            { id: "total_project",  label: "Total days (project)" },
            { id: "total_duration", label: "Days / duration" },
          ] as const).map(({ id, label }) => (
            <button key={id} type="button" onClick={() => { setDurationMode(id); setTotalDaysInput(""); }}
              style={{ padding: "6px 14px", fontSize: "11px", fontWeight: 700, cursor: "pointer", border: "none", fontFamily: "'DM Sans', sans-serif", background: durationMode === id ? "#00b8db" : "white", color: durationMode === id ? "white" : "#64748b", transition: "all 0.15s", borderRight: "1px solid #e2e8f0" }}>
              {label}
            </button>
          ))}
        </div>
        ...
        {durationMode === "per_week" && (
          <div>
            <div style={{ fontSize: "11px", color: "#94a3b8", marginBottom: "8px" }}>
              <span style={{ color: "#00b8db", fontWeight: 700, fontFamily: "'DM Mono', monospace" }}>{daysPerWeek}d</span> per week
              {totalDays > 0 && weekCount > 0 && <span style={{ marginLeft: "8px" }}>= {totalDays} days total over {weekCount} weeks</span>}
            </div>
            <div style={{ display: "flex", gap: "6px", alignItems: "center", flexWrap: "wrap" }}>
              {[0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5].filter(d => d <= capacity).map(d => (
                <button key={d} type="button" onClick={() => setDaysPerWeek(d)}
                  style={{ width: d % 1 === 0 ? "38px" : "34px", height: "38px", borderRadius: "8px", cursor: "pointer", fontWeight: 700, fontSize: d % 1 === 0 ? "14px" : "10px", fontFamily: "'DM Mono', monospace", background: daysPerWeek === d ? "#00b8db" : "#f8fafc", border: `1.5px solid ${daysPerWeek === d ? "#00b8db" : "#e2e8f0"}`, color: daysPerWeek === d ? "white" : "#64748b", transition: "all 0.1s" }}>
                  {d % 1 === 0 ? d : `${d}`}
                </button>
              ))}
            </div>
        ...
        {durationMode === "total_project" && (
          <div>
            <div style={{ fontSize: "11px", color: "#94a3b8", marginBottom: "8px" }}>Total days across the whole project -- spread evenly week by week.</div>
            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              <input type="number" min="0.5" step="0.5" value={totalDaysInput} onChange={e => setTotalDaysInput(e.target.value)} placeholder="e.g. 20"
                style={{ width: "100px", padding: "10px 12px", borderRadius: "9px", border: "1.5px solid #e2e8f0", fontSize: "14px", fontFamily: "'DM Mono', monospace", fontWeight: 700, outline: "none", color: "#0f172a" }} />
              {weekCount > 0 && totalDaysInput && (
                <div style={{ fontSize: "11px", color: "#64748b" }}>= <span style={{ color: "#00b8db", fontWeight: 700 }}>{effectiveDaysPerWeek}d/wk</span> across {weekCount} weeks</div>
              )}
            </div>
          </div>
        )}
        ...
        {durationMode === "total_duration" && (
          <div>
            <div style={{ fontSize: "11px", color: "#94a3b8", marginBottom: "8px" }}>Days allocated per week within this duration only.</div>
            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              <input type="number" min="0.5" max={capacity} step="0.5" value={totalDaysInput} onChange={e => setTotalDaysInput(e.target.value)} placeholder="e.g. 2"
                style={{ width: "100px", padding: "10px 12px", borderRadius: "9px", border: "1.5px solid #e2e8f0", fontSize: "14px", fontFamily: "'DM Mono', monospace", fontWeight: 700, outline: "none", color: "#0f172a" }} />
              <div style={{ fontSize: "11px", color: "#94a3b8" }}>days / week</div>
              {weekCount > 0 && totalDaysInput && (
                <div style={{ fontSize: "11px", color: "#64748b" }}>= <span style={{ color: "#00b8db", fontWeight: 700 }}>{totalDays}d total</span> over {weekCount} weeks</div>
              )}
            </div>
          </div>
        )}
        ...
        {selectedPerson && effectiveDaysPerWeek > 0 && (
          <div style={{ marginTop: "10px" }}>
            ...
            <div style={{ height: "6px", background: "#f1f5f9", borderRadius: "4px", overflow: "hidden" }}>
              <div style={{ height: "100%", borderRadius: "4px", transition: "width 0.3s", width: `${Math.min((effectiveDaysPerWeek / capacity) * 100, 100)}%`, background: utilColour(Math.round((effectiveDaysPerWeek / capacity) * 100)) }} />
            </div>
          </div>
        )}
        <FieldHint>Capacity: {capacity}d/wk for {selectedPerson?.full_name?.split(" ")[0] || "this person"}.</FieldHint>
      </div>
      ...
      <div>
        ...
        <textarea name="notes" value={notes} onChange={e => setNotes(e.target.value)}
          placeholder="Sprint focus, specific responsibilities, part-time schedule..." rows={2}
      ...
      {personId && (checking || checkResult) && (
        <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            {checking && <div style={{ width: "14px", height: "14px", borderRadius: "50%", border: "2px solid #e2e8f0", borderTopColor: "#00b8db", animation: "spin 0.6s linear infinite" }} />}
          </div>
          {checkResult && !checking && (
            <>
              <ConflictTable weeks={checkResult.weeks} onSkipExceptionWeeks={(newStart) => setStartDate(newStart)} />
              {conflictCount > 0 && (
                <div style={{ padding: "12px 16px", borderRadius: "10px", background: "rgba(239,68,68,0.05)", border: "1.5px solid rgba(239,68,68,0.2)" }}>
                  <div style={{ fontSize: "13px", fontWeight: 700, color: "#dc2626", marginBottom: "4px" }}>(!) {conflictCount} week{conflictCount > 1 ? "s" : ""} over capacity</div>
                  <div style={{ fontSize: "12px", color: "#64748b", lineHeight: 1.5 }}>You can still save -- allocation rows will be created and conflicts flagged. Or choose an alternative person below.</div>
                </div>
              )}
              {conflictCount === 0 && checkResult.weeks.length > 0 && (
                <div style={{ padding: "10px 14px", borderRadius: "10px", background: "rgba(16,185,129,0.06)", border: "1.5px solid rgba(16,185,129,0.2)", fontSize: "13px", color: "#059669", fontWeight: 600 }}>
                  No conflicts -- {selectedPerson?.full_name?.split(" ")[0]} has capacity across all {checkResult.weeks.length} weeks.
                </div>
              )}
              {conflictCount > 0 && checkResult.alternatives.length > 0 && (
                <AlternativesPanel alternatives={checkResult.alternatives} onSelect={id => { setPersonId(id); setCheckResult(null); }} />
              )}
            </>
          )}
        </div>
      ...
      <div style={{ paddingTop: "16px", borderTop: "1.5px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px" }}>
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          ...
          <button type="submit" disabled={!formValid || isPending}
            style={{ padding: "9px 24px", borderRadius: "8px", border: "none", background: !formValid || isPending ? "#94a3b8" : (conflictCount > 0 ? "#f59e0b" : "#00b8db"), color: "white", fontSize: "13px", fontWeight: 700, fontFamily: "'DM Sans', sans-serif", cursor: !formValid ? "not-allowed" : "pointer", display: "flex", alignItems: "center", gap: "7px", boxShadow: formValid ? "0 2px 12px rgba(0,184,219,0.3)" : "none", transition: "all 0.15s" }}>
            {isPending ? (
              <><span style={{ width: "14px", height: "14px", borderRadius: "50%", border: "2px solid rgba(255,255,255,0.3)", borderTopColor: "white", animation: "spin 0.6s linear infinite", display: "inline-block" }} />Saving...</>
            ) : conflictCount > 0 ? (
              `(!) Save with ${conflictCount} conflict${conflictCount > 1 ? "s" : ""}`
            ) : (
              <><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>Allocate {weekCount > 0 ? `${weekCount} weeks` : ""}</>
            )}
          </button>
        </div>
      </div>
