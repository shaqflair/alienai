// src/app/wbs/stats/WbsStatsClient.tsx
"use client";
...
/* ------------------------------------------------------------
   ...
   ✅ Production-safe: no useSearchParams in this route
   ✅ Server wrapper passes initialDays
------------------------------------------------------------- */
...
function buildItemsHref(days: DaysParam, params?: Record<string, string | number | boolean | null | undefined>) {
...
export default function WbsStatsClient({ initialDays }: { initialDays: DaysParam }) {
  ...
  // keep URL in sync (deep links + refresh)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const r = await fetch(`/api/ai/briefing?days=${encodeURIComponent(String(days))}`, {
          cache: "no-store",
          signal: ac.signal,
        });
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-6 flex items-center gap-2">
          ...
          {[7, 14, 30, 60].map((d) => (
            <button
              key={d}
              onClick={() => setDays(d as 7 | 14 | 30 | 60)}
              className={[
                "px-4 py-1.5 rounded-full text-sm border transition",
                days === d
                  ? "bg-cyan-500/20 border-cyan-400/50 text-cyan-300"
