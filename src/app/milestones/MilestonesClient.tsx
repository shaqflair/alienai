...
type ApiResp = {
  ...
  kpis: {
    ...
    overdue: number;
    ai_high_risk: number;
    slip_avg_days: number;
    slip_max_days: number;
  };
  items: Milestone[];
  error?: string;
  meta?: {
...
function formatUkDateLoose(iso: string | null | undefined) {
...
function inferTopMessage(data: ApiResp | null, items: Milestone[], scope: Scope, days: WindowDays, statusF: StatusFilter, search: string) {
  if (!data?.ok) return "";

  const searching = Boolean(search.trim());
  const filteredOut = items.length === 0 && data.items.length > 0;

  if (searching && filteredOut) {
    return "No milestones match the current search.";
  }

  if (statusF && filteredOut) {
    return `No milestones match the current status filter (${statusF.replace("_", " ")}).`;
  }

  if (scope === "overdue" && data.items.length === 0) {
    return "No overdue milestones detected across active projects.";
  }

  if (scope === "window" && data.items.length === 0) {
    return `No milestones due in the next ${days} days — schedule on track.`;
  }

  if (scope === "all" && data.items.length === 0) {
    return "No milestones defined for active projects — schedule visibility is limited.";
  }

  return "";
}

function deriveScopeCaption(scope: Scope, days: WindowDays) {
  if (scope === "window") return `Next ${days} days`;
  if (scope === "overdue") return "Overdue only";
...
function Pill({
  ...
  return (
    ...
    >
      {label}
      {count !== undefined && (
        <span style={{ fontFamily: T.mono, fontSize: 9, opacity: 0.7 }}>{count}</span>
      )}
    </button>
  );
}
...
function SlipMeter({ days, known }: { days: number; known: boolean }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      <Mono size={12} color={color} weight={600}>
        {days > 0 ? `+${days}d` : days < 0 ? `${days}d` : "0d"}
      </Mono>
    </div>
  );
}
...
function MilestoneRow({
  ...
  const rc = RAG[rag];
  const dueInDays = daysUntil(m.due_date);
  ...
  return (
    <>
      ...
      >
        ...
        <td style={{ ...TDbase, minWidth: 280 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              ...
              {!over && !m.is_done && dueInDays != null && dueInDays >= 0 && dueInDays <= 5 && (
                <Mono size={9} color={RAG.A.fg} weight={600} upper>Due Soon</Mono>
              )}
            </div>
            <div
              style={{
        ...
        <td style={{ ...TDbase, width: 110 }}>
          <Mono size={12} color={over ? RAG.R.fg : T.ink3} weight={over ? 600 : 400}>
            {m.due_date_uk || formatUkDateLoose(m.due_date)}
          </Mono>
        </td>
        ...
        <td style={{ ...TDbase, width: 90 }}>
          {m.risk_score > 0 ? (
            <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
              ...
              <Mono size={12} color={RAG[rrag].fg} weight={600}>
                {m.risk_score}
              </Mono>
            </div>
          ) : (
              ...
              <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 40 }}>
                <div>
                  {m.last_risk_reason ? (
                    ...
                    <p style={{ fontFamily: T.body, fontSize: 13, color: T.ink4, margin: 0 }}>
                      No AI assessment available for this milestone.
                    </p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {[
                      ["Due Date", m.due_date_uk || formatUkDateLoose(m.due_date)],
                      ["Slip", m.slip_known ? m.slip_label : null],
                      ["Baseline End", m.baseline_end ? formatUkDateLoose(m.baseline_end) : null],
...
export default function MilestonesPage() {
  ...
  const [days, setDays] = useState<WindowDays>(() => {
    ...
    return ([7, 14, 30, 60] as WindowDays[]).includes(d as any) ? (d as WindowDays) : 30;
  });
  const [scope, setScope] = useState<Scope>(() => {
    const s = safeStr(sp?.get("scope"));
    return (["window", "overdue", "all"] as Scope[]).includes(s as any) ? (s as Scope) : "window";
  });
  const [statusF, setStatusF] = useState<StatusFilter>(() => {
    const s = safeStr(sp?.get("status"));
    return (["", "planned", "in_progress", "at_risk", "completed", "overdue"] as StatusFilter[]).includes(s as any)
      ? (s as StatusFilter)
      : "";
  });
  ...
  const items = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter((m) =>
        `${m.milestone_name} ${m.project_title}`.toLowerCase().includes(q),
      );
    }
  ...
  const topMessage = useMemo(
    () => inferTopMessage(data, items, scope, days, statusF, search),
    [data, items, scope, days, statusF, search],
  );
      ...
      >
        <div style={{ maxWidth: 1320, margin: "0 auto", padding: "40px 40px 100px" }}>
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <Link href="/" style={{ fontFamily: T.mono, fontSize: 10, color: T.ink4, textDecoration: "none", letterSpacing: "0.08em" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 32 }}>
              <div>
                ...
                >
                  Schedule delivery intelligence across active projects — due dates, slippage, AI delay probability, and risk scoring in one operating view.
                </p>
              </div>

              {data?.ok && (
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  <Mono size={10} color={T.ink4} upper>
                    {items.length} visible milestone{items.length !== 1 ? "s" : ""}
                  </Mono>
                  <div style={{ marginTop: 4 }}>
                    <Mono size={10} color={T.ink5} upper>
                      Org scope · {deriveScopeCaption(scope, days)}
                    </Mono>
                  </div>
                </div>
              )}
          ...
          {data?.ok && (
            ...
            >
              <KpiCell label="In Scope" value={data.count} />
              <KpiCell label="Planned" value={data.kpis.planned} />
              <KpiCell
                label="At Risk"
                value={data.kpis.at_risk}
                alert={data.kpis.at_risk > 0}
                sub="needs attention"
              />
              <KpiCell
                label="Overdue"
              ...
              <div style={{ padding: "20px 24px" }}>
                ...
                <div
                  style={{
                    ...
                    marginTop: 8,
                    marginBottom: 4,
                    color: data.kpis.slip_avg_days > 0 ? RAG.A.fg : T.ink4,
                  }}
                >
          ...
          >
            <div style={{ display: "flex", gap: 3 }}>
              {SCOPES.map((s) => (
                <Pill key={s.v} label={s.l} active={scope === s.v} onClick={() => setScope(s.v)} />
              ))}
            </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10, animation: "fadeUp 0.4s 0.12s ease both" }}>
            <div>
              {!loading && topMessage ? (
                <Mono size={10} color={T.ink4}>{topMessage}</Mono>
              ) : (
                <span />
              )}
            </div>
            ...
            <div style={{ background: T.surface, border: `1px solid ${T.hr}`, overflow: "hidden", animation: "fadeUp 0.4s 0.14s ease both" }}>
              {items.length === 0 ? (
                <div style={{ padding: "80px", textAlign: "center" }}>
                  <Mono size={12} color={T.ink4}>
                    {topMessage || "No milestones match the current filters."}
                  </Mono>
                </div>
              ) : (
          ...
          {!loading && data?.ok && (
            <div style={{ marginTop: 24, display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 16, borderTop: `1px solid ${T.hr}` }}>
              <Mono size={10} color={T.ink5} upper>
                Org scope · active projects only · {deriveScopeCaption(scope, days)} · {items.length} visible item{items.length !== 1 ? "s" : ""}
              </Mono>
              <Mono size={10} color={T.ink5}>{nowUK()}</Mono>
            </div>
          )}
