...
export default async function ProjectsPage({
  ...
  const roleMap    = Object.fromEntries((memberRows ?? []).map((r: any) => [r.project_id, r.role]));
  ...
  if (projectIds.length > 0) {
    const { data: pData, error: pErr } = await supabase
      .from("projects")
      .select("id, title, project_code, colour, status, resource_status, start_date, finish_date, created_at")
      .in("id", projectIds)
    ...
    projects = (pData ?? []).map((p: any) => ({
      ...p,
      pm_name: null, // extend later if you have a PM column
    }));
  }
  ...
  const filtered = projects
    .filter(p => {
      if (filter === "Active") return (p.status ?? "active").toLowerCase() !== "closed";
      if (filter === "Closed") return (p.status ?? "").toLowerCase() === "closed";
      return true;
    })
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 32px 60px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12, flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              {["Active", "Closed", "All"].map(f => (
                <Link key={f} href={`/projects?filter=${f}&sort=${sortMode}&q=${query}`}
            ...
            <form method="get" action="/projects" style={{ position: "relative", flex: 1, maxWidth: 300 }}>
              ...
              <input
                ...
                className="pl-input"
                style={{ width: "100%", border: "1px solid #e2e8f0", borderRadius: 8, padding: "8px 10px 8px 30px", fontSize: 13, background: "white", fontFamily: "inherit" }}
              />
            </form>
            ...
            <div style={{ display: "flex", gap: 4, marginLeft: "auto" }}>
              {["Newest", "A-Z"].map(s => (
                <Link key={s} href={`/projects?filter=${filter}&sort=${s}&q=${query}`}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {filtered.map(p => {
              ...
              const dateRange = p.start_date && p.finish_date
              ...
              return (
                <div key={p.id} className="pl-row" style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: 14, padding: "16px 20px", display: "flex", alignItems: "center", gap: 14, transition: "box-shadow 0.15s, border-color 0.15s" }}>
                  {/* Colour dot */}
                  ...
                  <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 5, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                      <Link href={`/projects/${ref}`}
                        style={{ fontSize: 15, fontWeight: 700, color: "#0f172a", textDecoration: "none" }}
                        className="pl-action">
                        {p.title}
                      </Link>
                      {p.project_code && (
                        <span style={{ fontSize: 11, fontWeight: 700, background: "#f1f5f9", color: "#64748b", borderRadius: 6, padding: "2px 7px", border: "1px solid #e2e8f0", fontFamily: "'DM Mono', monospace" }}>
                          {p.project_code}
                        </span>
                      )}
                    ...
                    <div style={{ fontSize: 12, color: "#94a3b8", display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                      <span>PM: <span style={{ color: p.pm_name ? "#0f172a" : "#06b6d4" }}>{p.pm_name ?? "Unassigned"}</span></span>
                      <span style={{ width: 3, height: 3, borderRadius: "50%", background: "#cbd5e1", display: "inline-block" }} />
                      <span>Created {formatDate(p.created_at)}</span>
                      {roleMap[p.id] && (
                        <>
                          <span style={{ width: 3, height: 3, borderRadius: "50%", background: "#cbd5e1", display: "inline-block" }} />
                          <span style={{ textTransform: "capitalize" }}>{roleMap[p.id]}</span>
                        </>
                      )}
                    </div>
                    ...
                    {dateRange && (
                      <div style={{ fontSize: 12, color: "#94a3b8", display: "flex", alignItems: "center" }}>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" style={{ marginRight: 4, opacity: .5, flexShrink: 0 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0, flexWrap: "wrap" }}>
                    ...
                    <Link href={`/projects/${ref}/artifacts`}
                      className="pl-action"
                      style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 11px", border: "1px solid #e2e8f0", borderRadius: 8, fontSize: 12, fontWeight: 500, color: "#475569", textDecoration: "none", background: "white" }}>
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
                    ...
                    <Link href={`/projects/${ref}/members`}
                      className="pl-action"
                      style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 11px", border: "1px solid #e2e8f0", borderRadius: 8, fontSize: 12, fontWeight: 500, color: "#475569", textDecoration: "none", background: "white" }}>
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
                    ...
                    <Link href={`/projects/${ref}/approvals`}
                      className="pl-action"
                      style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 11px", border: "1px solid #e2e8f0", borderRadius: 8, fontSize: 12, fontWeight: 500, color: "#475569", textDecoration: "none", background: "white" }}>
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
