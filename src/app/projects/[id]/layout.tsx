  ...
  return (
    <>
      <style>{`
        ...
        .project-shell-crumb-link:hover {
          color: var(--text-2);
        }
      ...
      <script
        dangerouslySetInnerHTML={{
          __html: `
            (function(){
              function init(){
                var input = document.getElementById('project-sw-input');
                if(!input) return;
                input.addEventListener('input', function(){
                  var q = this.value.toLowerCase();
                  document.querySelectorAll('.project-sw-item').forEach(function(el){
          ...
          <div className="project-sw-wrap" tabIndex={0} style={{ outline: "none" }}>
            ...
            <div className="project-sw-dropdown">
              <div className="project-sw-search-row">
                ...
                <input id="project-sw-input" placeholder="Search projects" autoComplete="off" />
              </div>
            ...
            {tabs.map((t) => {
              ...
              return (
                <Link
                  key={t.id}
                  href={t.href}
                  className={`project-tab-link${active ? " active" : ""}`}
                >
                  {t.label}
                </Link>
              );
