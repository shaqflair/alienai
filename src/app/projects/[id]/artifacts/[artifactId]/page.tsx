...
function getArtifactReadOnlyBanner(args: {
  ...
  if (isViewer) {
    return { tone: "muted", title: "Read-only access", message: "You have view-only access to this artifact." };
  }
  if (approvalReadOnly) {
    return { tone: "warning", title: "Read-only: approval in progress", message: "This artifact cannot be edited in its current approval state." };
  }
  if (args.artifactLocked && status !== "approved") {
    return { tone: "warning", title: "Read-only: artifact locked", message: "This artifact is currently locked from editing." };
  }
  if (isLockedByAnotherUser && status !== "approved") {
    return { tone: "danger", title: "Read-only: locked by another user", message: `Another editor currently owns the edit lock (${lockedEditor}). You can take over automatically once the lock expires.` };
  }
  return { tone: null, title: "", message: null };
}

async function getProjectManagerNameBestEffort(supabase: any, projectId: string): Promise<string | null> {
  if (!projectId) return null;
  const pmRoleCandidates = ["project_manager","project manager","pm","programme_manager","program_manager","programme manager","program manager","delivery_manager","delivery manager"];

  async function readProfileName(userId: string): Promise<string | null> {
    const uid = safeStr(userId).trim();
    if (!uid) return null;
    const { data: prof } = await supabase.from("profiles").select("full_name, name, email, user_id, id").or(`user_id.eq.${uid},id.eq.${uid}`).maybeSingle();
  ...
  for (const role of pmRoleCandidates) {
    const { data, error } = await supabase.from("project_members").select("user_id, role").eq("project_id", projectId).eq("is_active", true).ilike("role", role).limit(1);
    if (error) break;
  ...
  const { data: mems } = await supabase.from("project_members").select("user_id, role").eq("project_id", projectId).eq("is_active", true).in("role", ["owner", "editor"]).limit(1);
...
export default async function ArtifactDetailPage({
  ...
  const {
    projectUuid, projectHumanId, projectTitle, projectStartDate, projectFinishDate,
    myRole, artifactId, artifact, approvalEnabled, status, isAuthor, isApprover,
    isEditable: loaderIsEditable, canDecide, canRenameTitle, canCreateRevision,
    mode, aiTargetType, aiTitle, charterInitial, typedInitialJson, latestWbsJson,
    wbsArtifactId, changeRequestsMode, charterMode, stakeholderMode, wbsMode,
    scheduleMode, closureMode, financialPlanMode, legacyExports, weeklyMode, collaboration,
  } = vm as any;
  ...
  const effectiveLockLayout = !!approvalEnabled && (financialPlanMode ? statusLower === "rejected" : isSubmitted || statusLower === "approved" || statusLower === "rejected");
  const effectiveIsEditable = approvalEnabled && financialPlanMode
    ? canEditByRole && isCurrent && !effectiveLockLayout && (isDraftOrCR || statusLower === "submitted" || statusLower === "approved")
    : approvalEnabled && (charterMode || closureMode)
      ? canEditByRole && isDraftOrCR && !effectiveLockLayout && isCurrent
      : !!loaderIsEditable;

  const canSubmitFromServer = !!approvalEnabled && !!(charterMode || closureMode || financialPlanMode) && canEditByRole && isCurrent && isDraftOrCR && !effectiveLockLayout;
  const canSubmitNonCharter = !!approvalEnabled && !(charterMode || closureMode || financialPlanMode) && !!loaderIsEditable && !!isCurrent && !effectiveLockLayout;
  const isApproverViewingSubmitted = !!approvalEnabled && !!isApprover && !isAuthor && !!canDecide && isSubmitted && !!(charterMode || closureMode || financialPlanMode);
  ...
  async function submitAction() {
    ...
    try { await submitArtifactForApproval(projectUuid, artifactId); } catch (e: any) { errMsg = String(e?.message ?? "Submit failed"); }
    if (errMsg) redirect(`${artifactPath}?action_error=${encodeURIComponent(errMsg)}`);
    revalidatePath(artifactPath); revalidatePath(artifactsPath); redirect(artifactPath);
  }
  ...
  async function approveAction() {
    ...
    if (!approvalEnabled || !projectUuid) return;
    let errMsg: string | null = null;
    try { await approveArtifact(projectUuid, artifactId); } catch (e: any) { errMsg = String(e?.message ?? "Approve failed"); }
    if (errMsg) redirect(`${artifactPath}?action_error=${encodeURIComponent(errMsg)}`);
    revalidatePath(artifactPath); revalidatePath(artifactsPath); redirect(artifactPath);
  }
  ...
  async function requestChangesAction(formData: FormData) {
    ...
    let errMsg: string | null = null;
    try { await requestChangesArtifact(projectUuid, artifactId, reason); } catch (e: any) { errMsg = String(e?.message ?? "Request changes failed"); }
    if (errMsg) redirect(`${artifactPath}?action_error=${encodeURIComponent(errMsg)}`);
    revalidatePath(artifactPath); revalidatePath(artifactsPath); redirect(artifactPath);
  }
  ...
  async function requestChangesWithCommentsAction(formData: FormData) {
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        comments = parsed.filter((c: any) => c && typeof c === "object").map((c: any) => ({ sectionTitle: String(c.sectionTitle ?? "").trim(), text: String(c.text ?? "").trim() })).filter((c) => c.text);
      }
    } catch {}
  ...
  async function rejectFinalAction(formData: FormData) {
    ...
    const reason = String(formData.get("reason") ?? "").trim() || undefined;
    let errMsg: string | null = null;
    try { await rejectFinalArtifact(projectUuid, artifactId, reason); } catch (e: any) { errMsg = String(e?.message ?? "Reject failed"); }
    if (errMsg) redirect(`${artifactPath}?action_error=${encodeURIComponent(errMsg)}`);
    revalidatePath(artifactPath); revalidatePath(artifactsPath); redirect(artifactPath);
  }
  ...
  async function renameTitleAction(formData: FormData) {
    "use server";
    let errMsg: string | null = null;
    try { await renameArtifactTitle(formData); } catch (e: any) { errMsg = String(e?.message ?? "Rename failed"); }
    if (errMsg) redirect(`${artifactPath}?action_error=${encodeURIComponent(errMsg)}`);
    revalidatePath(artifactPath); revalidatePath(artifactsPath); redirect(artifactPath);
  }
  ...
  async function createRevisionAction() {
    ...
    let errMsg: string | null = null;
    try {
      const res = await createArtifactRevision({ projectId: projectUuid, artifactId, revisionReason: "Revision created", revisionType: "material" });
      newArtifactId = safeStr((res as any).newArtifactId).trim();
    } catch (e: any) { errMsg = String(e?.message ?? "Create revision failed"); }
    if (errMsg) redirect(`${artifactPath}?action_error=${encodeURIComponent(errMsg)}`);
    revalidatePath(artifactsPath); revalidatePath(artifactPath);
    if (newArtifactId) { revalidatePath(`/projects/${projectRefForPaths}/artifacts/${newArtifactId}`); redirect(`/projects/${projectRefForPaths}/artifacts/${newArtifactId}`); }
  ...
  async function makeCurrentAction() {
    ...
    if (blocked) return;
    let errMsg: string | null = null;
    try { await setArtifactCurrent({ projectId: projectUuid, artifactId }); } catch (e: any) { errMsg = String(e?.message ?? "Make current failed"); }
    if (errMsg) redirect(`${artifactPath}?action_error=${encodeURIComponent(errMsg)}`);
    revalidatePath(artifactsPath); revalidatePath(artifactPath); redirect(artifactPath);
  }
  ...
  const ArtifactPageHeader = () => (
    <>
      <style>{`
        .af-header { background: #ffffff; border: 1px solid #e8ecf0; border-radius: 12px; margin-bottom: 20px; overflow: hidden; }
        .af-header-top { display: flex; align-items: flex-start; justify-content: space-between; gap: 16px; padding: 20px 24px 16px; border-bottom: 1px solid #e8ecf0; flex-wrap: wrap; }
        .af-title-area { flex: 1; min-width: 0; }
        .af-breadcrumb { display: flex; align-items: center; gap: 6px; font-size: 12px; color: #57606a; margin-bottom: 8px; flex-wrap: wrap; }
        ...
        .af-project-code { display: inline-flex; align-items: center; padding: 3px 8px; border-radius: 999px; font-size: 11px; font-weight: 700; letter-spacing: 0.03em; background: #eef2ff; color: #4338ca; border: 1px solid #c7d2fe; white-space: nowrap; }
        .af-title-row { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; }
        .af-title { font-size: 22px; font-weight: 700; color: #0d1117; letter-spacing: -0.4px; line-height: 1.2; margin: 0; }
        .af-type-badge { display: inline-flex; align-items: center; padding: 3px 8px; border-radius: 6px; font-size: 10px; font-weight: 600; letter-spacing: 0.05em; text-transform: uppercase; font-family: ui-monospace, monospace; background: #f6f8fa; color: #57606a; border: 1px solid #e8ecf0; white-space: nowrap; }
        .af-status-badge { display: inline-flex; align-items: center; gap: 5px; padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 600; white-space: nowrap; }
        .af-status-dot { width: 6px; height: 6px; border-radius: 50%; }
        .af-meta-row { display: flex; align-items: center; gap: 14px; margin-top: 10px; font-size: 12px; color: #57606a; flex-wrap: wrap; }
        .af-meta-item { display: flex; align-items: center; gap: 4px; color: #57606a; }
        .af-meta-sep { opacity: 0.4; color: #57606a; }
        .af-tag { display: inline-flex; align-items: center; gap: 4px; padding: 2px 8px; border-radius: 20px; font-size: 10px; font-weight: 600; }
        .af-actions { display: flex; align-items: center; gap: 8px; padding: 12px 24px; flex-wrap: wrap; background: #fafbfc; }
        .af-btn { display: inline-flex; align-items: center; gap: 5px; padding: 6px 12px; border-radius: 8px; font-size: 12px; font-weight: 600; border: 1px solid #e8ecf0; background: #ffffff; color: #57606a; cursor: pointer; text-decoration: none; font-family: inherit; transition: all 0.15s; white-space: nowrap; }
        .af-btn:hover { border-color: #d0d7de; background: #f6f8fa; color: #0d1117; }
        .af-btn-primary { background: #0d1117; border-color: #0d1117; color: #ffffff; }
        .af-btn-primary:hover { opacity: 0.88; }
        .af-btn-success { background: #16a34a; border-color: #16a34a; color: #ffffff; }
        .af-btn-success:hover { opacity: 0.88; }
        .af-btn-amber { background: #fff8e1; border-color: #f59e0b; color: #b45309; }
        .af-btn-amber:hover { background: #fef3c7; }
        .af-btn-danger { background: #fff5f5; border-color: #fecaca; color: #b91c1c; }
        .af-btn-danger:hover { background: #fee2e2; }
        .af-title-input { font-size: 22px; font-weight: 700; color: #0d1117; letter-spacing: -0.4px; border: none; outline: none; background: transparent; width: 100%; min-width: 200px; font-family: inherit; padding: 0; }
        .af-title-input:focus { background: #f6f8fa; border-radius: 6px; padding: 2px 8px; outline: 2px solid #3b82f6; outline-offset: 2px; }
        .af-decide-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; padding: 16px 24px; border-top: 1px solid #e8ecf0; }
        .af-decide-card { border: 1px solid #e8ecf0; border-radius: 10px; padding: 14px; background: #ffffff; display: flex; flex-direction: column; gap: 8px; }
        .af-decide-label { font-size: 13px; font-weight: 600; color: #0d1117; }
        .af-decide-hint { font-size: 11px; color: #8b949e; }
        textarea.af-reason { width: 100%; border: 1px solid #e8ecf0; border-radius: 8px; padding: 8px 10px; font-size: 12px; color: #0d1117; resize: none; font-family: inherit; background: #fafbfc; }
        textarea.af-reason:focus { outline: 2px solid #3b82f6; outline-offset: 1px; }
        .af-decide-grid-hidden { display: none !important; }
        @media (max-width: 700px) { .af-decide-grid { grid-template-columns: 1fr; } .af-header-top { flex-direction: column; } }
      `}</style>
      ...
      <div className="af-header">
        <div className="af-header-top">
          <div className="af-title-area">
            ...
            <div className="af-title-row">
              {canRenameTitle && !isFinancialPlan ? (
                <form action={renameTitleAction} style={{ display: "flex", alignItems: "center", gap: 10, flex: 1 }}>
                  ...
                  <input name="title" defaultValue={artifactTitle} className="af-title-input" placeholder="Artifact title..." />
                  <button type="submit" className="af-btn af-btn-primary" style={{ flexShrink: 0 }}>Save</button>
                </form>
              ) : (
            ...
            <div className="af-meta-row">
              ...
              <span className="af-tag" style={{ background: "#eef2ff", color: "#4338ca" }}>Project code: {projectCodeLabel}</span>
              <span className="af-meta-sep">•</span>
              {isCurrent ? (
              ...
              {approvalEnabled && isApprover && (<><span className="af-meta-sep">•</span><span className="af-tag" style={{ background: "#ede9fe", color: "#7c3aed" }}>Approver</span></>)}
              <span className="af-meta-sep">•</span>
              <span className="af-meta-item">Role:<strong style={{ color: "#0d1117", marginLeft: 3 }}>{myRole}</strong></span>
              <span className="af-meta-sep">•</span>
              <span className="af-meta-item">Updated:<ClientDateTime value={(artifact as any)?.updated_at ?? (artifact as any)?.created_at} /></span>
              {mode === "schedule" && (projectStartDate || projectFinishDate) && (<><span className="af-meta-sep">•</span><span className="af-meta-item">{projectStartDate || "---"} to {projectFinishDate || "---"}</span></>)}
            </div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 6, alignItems: "flex-end", flexShrink: 0 }}>
            {canSubmitFromServer && (<form action={submitAction}><button type="submit" className="af-btn af-btn-primary">{statusLower === "changes_requested" ? "Resubmit for approval" : "Submit for approval"}</button></form>)}
            {approvalEnabled && canSubmitNonCharter && !(charterMode || closureMode || financialPlanMode) && (<form action={submitAction}><button type="submit" className="af-btn af-btn-primary">{statusLower === "changes_requested" ? "Resubmit for approval" : "Submit for approval"}</button></form>)}
            {canCreateRevision && (<form action={createRevisionAction}><button type="submit" className="af-btn">Create revision</button></form>)}
            {!isCurrent && canEditByRole && (charterMode || closureMode) && (<form action={makeCurrentAction}><button type="submit" className="af-btn">Make current</button></form>)}
          </div>
        </div>
        <div className="af-actions">
          ...
          {approvalEnabled && (
            <span style={{ marginLeft: "auto", fontSize: 12, color: "#8b949e" }}>
              ...
              {statusLower === "submitted" && !isAuthor && isApprover && (isApproverViewingSubmitted ? "Approver review mode — use inline comments below." : "Submitted - you can decide below.")}
        ...
        {approvalEnabled && canDecide && statusLower === "submitted" && (
          <div className={`af-decide-grid${isApproverViewingSubmitted ? " af-decide-grid-hidden" : ""}`}>
            <div className="af-decide-card">
              ...
              <form action={approveAction}><button type="submit" className="af-btn af-btn-success" style={{ width: "100%" }}>Approve</button></form>
            </div>
            <div className="af-decide-card">
            ...
            <div className="af-decide-card">
              ...
              <form action={rejectFinalAction} style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                ...
                <input name="confirm" className="af-reason" style={{ resize: "none" }} placeholder='Type "REJECT" to confirm' required />
        ...
        {isApproverViewingSubmitted && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, padding: "16px 24px", borderTop: "1px solid #e8ecf0", background: "#fafbfc" }}>
            <div style={{ border: "1px solid #bbf7d0", borderRadius: 10, padding: "14px 16px", background: "#f0fdf4", display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#15803d" }}>Approve</div>
              <div style={{ fontSize: 11, color: "#166534" }}>Promotes to approved baseline. Use inline comments below to request changes instead.</div>
              <form action={approveAction} style={{ marginTop: 4 }}>
                <button type="submit" style={{ width: "100%", padding: "8px 0", borderRadius: 8, background: "#16a34a", border: "none", color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>✓ Approve</button>
              </form>
            </div>
            <div style={{ border: "1px solid #fecaca", borderRadius: 10, padding: "14px 16px", background: "#fff5f5", display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#b91c1c" }}>Reject (Final)</div>
              <div style={{ fontSize: 11, color: "#7f1d1d" }}>Permanently rejects this submission. This cannot be undone.</div>
              <form action={rejectFinalAction} style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 4 }}>
                <input name="reason" style={{ padding: "7px 10px", fontSize: 12, borderRadius: 8, border: "1px solid #fecaca", fontFamily: "inherit", background: "#fff", color: "#0d1117", outline: "none", width: "100%", boxSizing: "border-box" }} placeholder="Rejection reason (optional)" />
                <input name="confirm" style={{ padding: "7px 10px", fontSize: 12, borderRadius: 8, border: "1px solid #f87171", fontFamily: "inherit", background: "#fff", color: "#0d1117", outline: "none", width: "100%", boxSizing: "border-box" }} placeholder="Type REJECT to confirm" required />
                <button type="submit" style={{ padding: "8px 0", borderRadius: 8, background: "#b91c1c", border: "none", color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>✗ Reject final</button>
              </form>
            </div>
          </div>
        )}
  ...
  return (
    <div className="artifact-page-shell">
      ...
      {!isWeeklyReport && !isFinancialPlan && !changeRequestsMode && !charterMode && !stakeholderMode && !wbsMode && !scheduleMode && !closureMode && effectiveIsEditable ? (
        <div style={{ marginTop: 16, background: "#ffffff", border: "1px solid #e8ecf0", borderRadius: 12, padding: 24 }}>
          ...
          <form action={updateArtifact} style={{ display: "grid", gap: 14 }}>
            ...
            <label style={{ display: "grid", gap: 6 }}>
              ...
              <input name="title" defaultValue={String((artifact as any)?.title ?? "")} style={{ border: "1px solid #e8ecf0", borderRadius: 8, padding: "8px 12px", fontSize: 13, color: "#0d1117", fontFamily: "inherit" }} />
            </label>
            <label style={{ display: "grid", gap: 6 }}>
              <span style={{ fontSize: 13, fontWeight: 500, color: "#0d1117" }}>Content</span>
              <textarea name="content" rows={14} defaultValue={String((artifact as any)?.content ?? "")} style={{ border: "1px solid #e8ecf0", borderRadius: 8, padding: "8px 12px", fontFamily: "ui-monospace, monospace", fontSize: 12, color: "#0d1117", resize: "vertical" }} />
            </label>
            <button type="submit" style={{ width: "fit-content", padding: "8px 16px", borderRadius: 8, background: "#0d1117", color: "#ffffff", border: "none", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Save changes</button>
          </form>
        </div>
      ) : null}
    </div>
