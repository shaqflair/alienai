...
export default async function ArtifactDetailPage({
  ...
  const isDraftOrCR =
    statusLower === "draft" || statusLower === "changes_requested";
  ...
  try {
    ...
    if (projectUuid) {
      projectManagerName = await getProjectManagerNameBestEffort(
        supabase,
        String(projectUuid)
      );
    }
  ...
  async function submitAction() {
    ...
    const ok =
      charterMode || closureMode ? canSubmitFromServer : canSubmitNonCharter;
  ...
  async function createRevisionAction() {
    ...
    revalidatePath(`/projects/${projectRefForPaths}/artifacts/${artifactId}`);
    revalidatePath(
      `/projects/${projectRefForPaths}/artifacts/${(res as any).newArtifactId}`
    );
    ...
    <main className="mx-auto w-full max-w-[1600px] px-6 py-6 space-y-6 bg-white text-gray-950">
      <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-gray-700">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {approvalEnabled ? (
            <span
              className={`inline-flex items-center rounded-full border px-3 py-1 text-xs ${pill.cls}`}
            >
              {pill.label}
            </span>
          ) : (
      ...
      <header className="space-y-3">
        {canRenameTitle ? (
          <form action={renameTitleAction} className="flex flex-wrap gap-3 items-center">
            ...
            <button
              type="submit"
              className="px-5 py-3 rounded-2xl bg-black text-white text-sm font-medium hover:opacity-90"
            >
              Save name
            </button>
          </form>
        ...
        <div className="text-sm text-gray-700 flex flex-wrap items-center gap-2">
          ...
          <span>
            Updated:{" "}
            <ClientDateTime
              value={(artifact as any).updated_at ?? (artifact as any).created_at}
            />
          </span>
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <form action={approveAction} className="border border-gray-300 rounded-2xl p-4 space-y-2 bg-white">
              ...
              <div className="text-xs text-gray-600">
                Final approval promotes baseline.
              </div>
              <button
            ...
            <form
              ...
              className="border border-gray-300 rounded-2xl p-4 space-y-2 bg-white"
            >
              <div className="font-medium text-gray-950">Request Changes (CR)</div>
              <textarea
                name="reason"
                rows={3}
                className="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm text-gray-950"
                required
              />
              <button
                className="px-4 py-2 rounded-xl border border-gray-300 text-gray-900 text-sm hover:bg-gray-50"
                type="submit"
              >
                Request changes
              </button>
          ...
          <form action={updateArtifact} className="grid gap-4">
            ...
            <label className="grid gap-2">
              ...
              <input
                name="title"
                defaultValue={String((artifact as any).title ?? "")}
                className="border border-gray-300 rounded-xl px-3 py-2 text-gray-950"
              />
            </label>
            ...
            <label className="grid gap-2">
              ...
              <textarea
                ...
                className="border border-gray-300 rounded-xl px-3 py-2 font-mono text-sm text-gray-950"
              />
            </label>
