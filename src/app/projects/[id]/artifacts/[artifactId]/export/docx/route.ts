...
function milestonesTable(rows: Array<{ milestone: string; targetDate: string; actualDate?: string; notes?: string }>) {
  const safe = (rows ?? []).length ? rows : [{ milestone: "â€”", targetDate: "â€”", actualDate: "â€”", notes: "â€”" }];
...
function docHeaderFooter(args: {
  ...
  const footer = new Paragraph({
    ...
    children: [
      new TextRun({ text: "Confidential â€“ Generated by AlienAI â€“ ", color: "666666", size: 18 }),
      ...
      sections: [
        {
          ...
          children: [
            ...
            new Paragraph({
              ...
              spacing: { after: 220 },
              children: [
                new TextRun({ text: `Project: ${project.title}  â€¢  Version: ${artifact.version}  â€¢  Status: ${niceStatus(status)}`, size: 20, color: "444444" }),
              ],
            }),
            watermark
              ? new Paragraph({
