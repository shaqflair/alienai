...
export default function CreateProjectModal({ activeOrgId, userId }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/org/members?orgId=${encodeURIComponent(activeOrgId)}`)
      .then((r) => r.json())
      .then((d) => {
        if (!cancelled && Array.isArray(d.members)) setMembers(d.members);
      })
      .catch(() => {});
  ...
  function reset() {
    setStep(1);
    setName(""); setPm(""); setPmUserId(null); setPmSearch(""); setPmOpen(false);
    setSponsor(""); setSponsorId(null); setSponsorSearch(""); setSponsorOpen(false);
    setProjectType(""); setStartDate(""); setEndDate(""); setCode("");
    setDept(""); setResStatus("confirmed"); setColour(COLOURS[0]); setError(null);
  }
  ...
  return (
    <>
      <button
        ...
        style={{
          ...buttonBase({
            background: UI.ink, color: UI.white,
            border: `1px solid ${UI.ink}`,
            display: "inline-flex", alignItems: "center",
            gap: 7, padding: "8px 14px",
          }),
        }}
      >
          ...
          >
            ...
            >
              <div style={{ maxWidth: 420 }}>
                <div style={{
                  ...
                  textTransform: "uppercase", color: UI.ink4, marginBottom: 10,
                }}>
                  Portfolio setup
                </div>
                <h2 style={{
                  fontSize: 24, fontWeight: 750, margin: 0,
                  letterSpacing: "-.03em", color: UI.ink, lineHeight: 1,
                }}>
                  Create project
                </h2>
                <p style={{ fontSize: 13, color: UI.ink3, margin: "8px 0 0", lineHeight: 1.6 }}>
              ...
              <button
                ...
                onClick={close}
                style={{
                  background: UI.white, border: `1px solid ${UI.rule}`,
              ...
              >
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                  <path d="M18 6 6 18M6 6l12 12" stroke={UI.ink3} strokeWidth="2" strokeLinecap="round" />
                </svg>
              </button>
            </div>
            ...
            >
              <div style={{
                ...
                background: UI.off2, border: `1px solid ${UI.rule}`,
                borderRadius: 999, padding: "5px 10px",
                fontSize: 11, fontWeight: 650, color: UI.ink3, width: "fit-content",
              }}>
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none">
                  <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" stroke={UI.ink3} strokeWidth="2" />
                </svg>
                Active organisation
              </div>
              ...
              {error && (
                <div style={{
                  ...
                  background: UI.redBg, border: `1px solid ${UI.redRule}`,
                  fontSize: 13, color: UI.red, fontWeight: 650,
                }}>
                  {error}
                </div>
              )}
              ...
              {step === 1 ? (
                <>
                  ...
                  <Field label="Project manager">
                    <div style={{ position: "relative" }}>
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setPmUserId(null); setPm("");
                        }}
                        onFocus={() => { if (pmSearch.trim().length > 0) setPmOpen(true); }}
                        onBlur={() => { window.setTimeout(() => setPmOpen(false), 150); }}
                        autoComplete="off"
                      />
                            ...
                            filteredPmMembers.map((m) => (
                              <div
                                ...
                                onClick={() => {
                                  ...
                                  setPm(selected); setPmUserId(m.user_id);
                                  setPmSearch(selected); setPmOpen(false);
                                }}
                                style={{
                                  padding: "10px 14px", cursor: "pointer", fontSize: 13,
                                  display: "flex", flexDirection: "column", gap: 2,
                                  borderBottom: `1px solid ${UI.off}`, background: UI.white,
                                }}
                                onMouseEnter={(e) => (e.currentTarget.style.background = UI.off2)}
                                onMouseLeave={(e) => (e.currentTarget.style.background = UI.white)}
                              >
                                <span style={{ fontWeight: 650, color: UI.ink }}>{m.name || "—"}</span>
                                <span style={{ fontSize: 11, color: UI.ink4 }}>{m.email}</span>
                              </div>
                            ))
                          ) : (
                            <div style={{ padding: "10px 14px", fontSize: 13, color: UI.ink4 }}>
                              No members found
                            </div>
                          )}
                        </div>
                    ...
                    <p style={hintStyle}>Assign now or later for delivery accountability.</p>
                  </Field>
                  ...
                  <Field label="Sponsor">
                    <div style={{ position: "relative" }}>
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setSponsorId(null); setSponsor("");
                        }}
                        onFocus={() => { if (sponsorSearch.trim().length > 0) setSponsorOpen(true); }}
                        onBlur={() => { window.setTimeout(() => setSponsorOpen(false), 150); }}
                        autoComplete="off"
                      />
                          ...
                          {filteredSponsorMembers.length > 0 ? (
                            filteredSponsorMembers.map((m) => (
                              <div
                                ...
                                onClick={() => {
                                  const selected = m.name || m.email;
                                  setSponsor(selected); setSponsorId(m.user_id);
                                  setSponsorSearch(selected); setSponsorOpen(false);
                                }}
                                style={{
                                  padding: "10px 14px", cursor: "pointer", fontSize: 13,
                                  display: "flex", flexDirection: "column", gap: 2,
                    ...
                    <p style={hintStyle}>Executive accountable for budget and decision control.</p>
                  </Field>
                  ...
                  <Field label="Project type">
                    ...
                    <p style={hintStyle}>Used for reporting, filtering and portfolio segmentation.</p>
                  </Field>
                ...
                <>
                  <div style={{
                    ...
                    padding: "10px 12px", background: UI.off2,
                    borderRadius: 12, border: `1px solid ${UI.rule}`,
                  }}>
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none">
                      <rect x="3" y="3" width="18" height="18" rx="2" stroke={UI.ink3} strokeWidth="2" />
                      <path d="M3 9h18M9 9v12M15 9v12" stroke={UI.ink3} strokeWidth="2" strokeLinecap="round" />
                    </svg>
                    <span style={{
                      fontSize: 11, fontWeight: 700, letterSpacing: ".1em",
                      color: UI.ink3, textTransform: "uppercase",
                    }}>
                  ...
                  <Field label="Resource status">
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                      {(["confirmed", "pipeline"] as const).map((s) => {
                        ...
                        return (
                          <button
                            key={s} type="button" onClick={() => setResStatus(s)}
                            style={{
                  ...
                  <Field label="Project colour">
                    <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                      {COLOURS.map((c) => (
                        <button
                          key={c} type="button" onClick={() => setColour(c)} title={c}
                          style={{
                            width: 28, height: 28, borderRadius: "50%", background: c,
                            border: colour === c ? "2px solid #111111" : "1px solid rgba(0,0,0,.08)",
                            cursor: "pointer",
                      ...
                      <span style={{
                        fontSize: 11, fontWeight: 700, background: UI.off2,
                        color: UI.ink3, borderRadius: 8, padding: "4px 8px",
                        border: `1px solid ${UI.rule}`, marginLeft: 4,
                        fontFamily: "'DM Mono', monospace",
                      }}>
                        {code || "PRJ"}
                      </span>
                    </div>
                    <p style={hintStyle}>Used to identify the project visually in portfolio views.</p>
                  </Field>
                </>
              )}
            </div>
              ...
              {step === 1 ? (
                <>
                  <button type="button" onClick={close}
                    style={buttonBase({ background: UI.white, border: `1px solid ${UI.ruleStrong}`, color: UI.ink3 })}>
                    Cancel
                  </button>
                  <button type="button" onClick={() => name.trim() && setStep(2)}
                    style={buttonBase({
                      background: UI.ink, color: UI.white,
                      border: `1px solid ${UI.ink}`,
