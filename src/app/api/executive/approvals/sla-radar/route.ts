...
function riskFromDue(nowMs: number, dueIso: string | null) {
  if (!dueIso)
    return {
      ...
      at_risk: false,
      overdue_days: 0,
      hours_to_due: null as number | null,
    };
  ...
  // “at risk” window: due within 48 hours
  if (diffHrs <= 48) {
    return {
      breached: false,
      at_risk: true,
...
function toStageKey(type: string, status: string | null, dueIso: string | null) {
  ...
  const dueLabel =
    due && Number.isFinite(due.getTime())
      ...
      : "";
  const parts = [t, s, dueLabel ? `due ${dueLabel}` : ""].filter(Boolean);
  return parts.join(" · ");
}
...
export async function GET() {
  try {
    ...
    // ✅ FIX-SLA2: pass supabase (matches rest of your executive routes pattern)
     ...
     * - tasks (due_at) in org
     *
     * Response: { items: [...] } where each item includes:
     *  breached:boolean, at_risk:boolean (contract for cockpit)
     *  project_title + stage_key for MicroList rendering
     */
    ...
    if (!cachedErr && Array.isArray(cached) && cached.length) {
      items = cached
        .map((r: any) => {
          ...
          const projectTitle =
            safeStr(r?.project_title || r?.project_name || r?.project || "") ||
            null;
          ...
          return {
            ...
            overdue_days: Number.isFinite(Number(r?.overdue_days))
              ? Number(r?.overdue_days)
              : risk.overdue_days,
            hours_to_due: Number.isFinite(Number(r?.hours_to_due))
              ? Number(r?.hours_to_due)
              : risk.hours_to_due,
            ...
            // ✅ FIX-SLA3: cockpit expects these
      ...
      if (!ticketsErr && Array.isArray(tickets) && tickets.length) {
        items = tickets
          .map((t: any) => {
            ...
            const closed = ["done", "closed", "completed", "resolved"].includes(
              st
            );
            ...
            const projectTitle = safeStr(t?.projects?.name) || null;
            ...
            return {
              ...
              // ✅ FIX-SLA3: cockpit MicroList keys
              project_title: projectTitle,
    ...
    // Member scope: only projects they belong to
