...
function riskFromDue(nowMs: number, dueIso: string | null) {
  ...
  if (diffMs < 0) return { breached: true, at_risk: false, overdue_days: Math.max(1, Math.floor(Math.abs(diffMs) / 864e5)), hours_to_due: diffHrs };
  if (diffHrs <= 48) return { breached: false, at_risk: true, overdue_days: 0, hours_to_due: diffHrs };
  return { breached: false, at_risk: false, overdue_days: 0, hours_to_due: diffHrs };
}

function toStageKey(type: string, status: string | null, dueIso: string | null) {
  const t = safeStr(type || "item");
  const s = safeStr(status || "").trim();
  ...
  const dueLabel = due && Number.isFinite(due.getTime())
    ...
    : "";
  return [t, s, dueLabel ? `due ${dueLabel}` : ""].filter(Boolean).join(" · ");
}
...
export async function GET() {
  try {
    ...
    if (!cachedErr && Array.isArray(cached) && cached.length) {
      items = cached
        .map((r: any) => {
          ...
          const type = safeStr(r?.type || "item");
          const status = safeStr(r?.status || "") || null;
          return {
            type, id: r?.id ?? r?.item_id ?? null,
            title: safeStr(r?.title || r?.label || "Untitled"),
            status, priority: r?.priority ?? null,
            due_at: dueIso,
            ...
            project_title: safeStr(r?.project_title || r?.project_name || r?.project || "") || null,
            stage_key: safeStr(r?.stage_key) || toStageKey(type, status, dueIso),
            assignee_id: r?.assignee_id ?? null,
          };
        })
        .filter((x: any) => x.breached || x.at_risk);
    } else {
      // 2) Live: tickets across all org projects
      const { data: tickets, error: ticketsErr } = await supabase
        .from("tickets")
        .select("id, title, status, priority, due_at, sla_due_at, updated_at, project_id, assignee_id, projects!inner(id, organisation_id, name)")
      ...
      if (!ticketsErr && Array.isArray(tickets) && tickets.length) {
        items = tickets.map((t: any) => {
          ...
          if (["done", "closed", "completed", "resolved"].includes(st)) return null;
          if (!risk.breached && !risk.at_risk) return null;
          const status = t?.status ?? null;
          return {
            type: "ticket", id: t.id, title: safeStr(t?.title || "Untitled"),
            status, priority: t?.priority ?? null,
            due_at: dueIso, breached: risk.breached, at_risk: risk.at_risk,
            overdue_days: risk.overdue_days, hours_to_due: risk.hours_to_due,
            updated_at: safeIso(t?.updated_at),
            project_id: safeStr(t?.project_id) || null,
            project_title: safeStr(t?.projects?.name) || null,
            stage_key: toStageKey("ticket", status, dueIso),
        ...
        const { data: tasks, error: tasksErr } = await supabase
          .from("tasks")
          .select("id, title, status, due_at, updated_at, project_id, assignee_id, projects!inner(id, organisation_id, name)")
          .eq("projects.organisation_id", orgId)
          .limit(500);
        ...
        if (!tasksErr && Array.isArray(tasks)) {
          items = tasks.map((t: any) => {
            ...
            const status = t?.status ?? null;
            return {
              type: "task", id: t.id, title: safeStr(t?.title || "Untitled"),
              status, due_at: dueIso, breached: risk.breached, at_risk: risk.at_risk,
              overdue_days: risk.overdue_days, hours_to_due: risk.hours_to_due,
              updated_at: safeIso(t?.updated_at),
              project_id: safeStr(t?.project_id) || null,
              project_title: safeStr(t?.projects?.name) || null,
              stage_key: toStageKey("task", status, dueIso),
              assignee_id: t?.assignee_id ?? null,
            };
          }).filter(Boolean) as any[];
        }
    ...
    // ✅ Always org-scoped — isExecutiveForOrg check removed
