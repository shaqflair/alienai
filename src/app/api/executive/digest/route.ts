// src/app/api/executive/digest/route.ts
import "server-only";
...
export async function GET(req: Request) {
  try {
    ...
    // 3) Decisions within window (active projects only)
    let decisions: any[] = [];
    ...
    // 4) PMs (NO nested join) — org members + profiles
    ...
    const atRiskProjects = activeProjects
      .map((p: any) => {
        ...
        const daysSince = p?.updated_at
          ? Math.round((Date.now() - new Date(p.updated_at).getTime()) / 86400000)
          : null;
        ...
        const score = Math.min(
          ...
          (overdueCount > 0 ? 40 : 0) +
            (daysSince != null && daysSince > 14
              ? Math.min(35, 20 + Math.floor((daysSince - 14) / 7) * 5)
              : 0)
        );
    ...
    // ✅ Build “pending approvals” payload once
    ...
    const digest = {
      ...
      sections: {
        // ✅ KEEP: old key (what the UI likely expects)
        pending_approvals: pendingPayload,

        // ✅ NEW: keep this too (fine for future UI)
        sla_breaches: {
