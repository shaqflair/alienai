...
async function resolveOrgName(supabase: any, userId: string) {
  // Best-effort: first active org for this user
  try {
...
async function buildExecSummary(args: {
  ...
  if (!projectIds.length) return { ok: false, error: "No accessible projects found." };
...
function renderPdfHtml(summary: ExecSummary) {
  ...
  const wowBlock =
    wow && wow.prev_week_start
      ...
      <div class="card soft">
        <div class="card-title-row">
          <div class="card-title">Week-on-week</div>
          <div class="muted">${esc(fmtUkDate(wow.week_start))} vs ${esc(fmtUkDate(wow.prev_week_start))}</div>
        </div>
        <div class="list">
          ${
            (wow.narrative || []).map((t) => `<div class="li">• ${esc(t)}</div>`).join("") ||
    ...
    `
      : `
      <div class="card soft">
        <div class="muted">Week-on-week will appear after at least <b>2 weekly snapshots</b> exist.</div>
      </div>
    `;
          ...
          : items
              .map((x) => {
                ...
                if (x.exposure_total_fmt)
                  pills.push(`<span class="pill pill-neutral">Exposure: ${esc(x.exposure_total_fmt)}</span>`);
                ...
                return `
                  <div class="item">
                    <div class="item-meta">${esc(x.project_title || "Project")} • ${esc(x.type || "RAID")}</div>
...
<head>
  ...
  <title>Portfolio RAID Brief</title>
  <style>
    :root{
      --ink:#0f172a;
      --muted:#64748b;
    ...
    *{box-sizing:border-box}
    ...
    .li{ font-size:12px; color:#334155; margin:6px 0; }
...
<body>
  <div class="page">
          ...
          <div class="logo">
            ${
              logoUrl
                ...
                : `<div class="wordmark">ΛLIENΛ</div>`
            }
          </div>

          <div class="orgblock">
            <div class="clientname">${esc(clientPrimary)}</div>
            <div class="orgline"><b>Organisation:</b> ${esc(orgSecondary)}</div>
          </div>
        </div>

        <div class="kicker">Executive Summary • next ${esc(summary.days)} days • scope: ${esc(summary.scope)}</div>
      </div>

      <div class="title">Portfolio RAID Brief</div>
      <div class="headline">${esc(summary.summary.headline)}</div>
    ...
    <div class="footer">
      <div>Aliena AI • Portfolio Insights</div>
      <div>${esc(fmtUkDateTime(new Date().toISOString()))}</div>
    </div>
  </div>
</body>
</html>
...
/* ---------------- PPTX renderer (LIGHT dashboard + logo) ---------------- */
...
async function renderPptxFromSummary(summary: ExecSummary) {
  ...
  // Slide size in inches (pptxgenjs wide is typically 13.333 x 7.5)
  ...
  // resolve logo as data URI for pptx
  const logoUrl =
    safeStr(process.env.BRANDING_LOGO_URL || process.env.NEXT_PUBLIC_BRANDING_LOGO_URL).trim() || BRAND_LOGO_URL;
  ...
  slide.addText("Portfolio RAID Brief", {
    x: leftX + 0.75,
    y: topY - 0.02,
    w: 5.0,
    h: 0.4,
  ...
  // top-right analysis window pill
  addCard(rightX + rightW - 2.75, topY - 0.02, 2.75, 0.65, { fill: C.card });
  slide.addText("ANALYSIS WINDOW", {
    x: rightX + rightW - 2.65,
    y: topY + 0.06,
    w: 2.55,
    h: 0.2,
    fontSize: 8.5,
  ...
  slide.addText("PORTFOLIO STATUS", {
    x: leftX + 0.3,
    y: statusY + 0.18,
    w: leftW - 0.6,
    h: 0.2,
  ...
  slide.addText(safe(summary.summary?.headline || ""), {
    x: leftX + 0.3,
    y: statusY + 0.45,
    w: leftW - 0.6,
    h: 0.9,
  ...
  slide.addText(`Exposure: ${kExposureFmt}`, {
    x: leftX + 0.75,
    y: exposureY + 0.18,
    w: leftW - 1.0,
    h: 0.25,
  ...
  const bottomTileH = 1.25;
  const bottomTileW = (rightW - gutter) / 2;
  ...
  slide.addText("FINANCIAL HOTSPOTS", {
    ...
    if (format === "md") {
      ...
      for (const sec of summary.sections) {
        ...
        for (const it of sec.items) {
          lines.push(
            `- ${it.project_title || "Project"} • ${it.type || "RAID"} • ${it.title || "Untitled"} • Due ${
