...
function hasAnyFilters(f: PortfolioFilters) {
  return (f.projectName && f.projectName.length) || (f.projectCode && f.projectCode.length) || (f.projectManagerId && f.projectManagerId.length) || (f.department && f.department.length);
}
...
async function applyProjectFilters(supabase: any, scopedProjectIds: string[], filters: PortfolioFilters) {
  ...
  const filtered = rows.filter(p => {
    ...
    if (codeNeedles.length && !codeNeedles.some(n => code.includes(n))) return false;
    if (pmSet.size) { const pm = safeStr(p?.project_manager_id).trim(); if (!pm || !pmSet.has(pm)) return false; }
    if (deptNeedles.length) { const dept = safeStr(p?.department).toLowerCase().trim(); if (!dept || !deptNeedles.some(n => dept.includes(n))) return false; }
    return true;
  });
...
async function handle(req: Request, opts: { scope: string; windowDays: 7 | 14 | 30 | 60; type: string; status: string; filters: PortfolioFilters }) {
  ...
  // ── Exclude pipeline projects ──────────────────────────────────────────
  // Uses resource_status column — same logic as project page:
  // isPipeline = resource_status.toLowerCase() === "pipeline"
  let nonPipelineIds = scopedActive;
  if (scopedActive.length > 0) {
    const { data: pipelineRows } = await supabase
      .from("projects")
      .select("id")
      .in("id", scopedActive)
      .ilike("resource_status", "%pipeline%");
    const pipelineSet = new Set((pipelineRows ?? []).map((p: any) => String(p.id)));
    if (pipelineSet.size > 0) {
      nonPipelineIds = scopedActive.filter(id => !pipelineSet.has(id));
    }
  }
  // ───────────────────────────────────────────────────────────────────────

  const filtered   = await applyProjectFilters(supabase, nonPipelineIds, opts.filters);
  const projectIds = filtered.projectIds;

  if (!projectIds.length) {
    return ok({
      scope: opts.scope, windowDays: opts.windowDays, type: opts.type, status: opts.status,
      items: [],
      meta: {
        scope: "org", organisationId, projectCount: 0, active_only: true, scopeMeta,
        scopeCounts: { scopedIdsRaw: scopedRaw.length, scopedIdsActive: scopedActive.length, scopedIdsFiltered: 0 },
        active_filter_ok: active.ok, active_filter_error: active.error, filters: filtered.meta,
      },
    });
  }
  ...
  if (ids.length) {
    ...
    const [scoresRes, predsRes, finsRes] = await Promise.all([
      supabase.from("raid_item_scores").select("raid_item_id, score, components, model_version, scored_at").in("raid_item_id", ids).order("scored_at", { ascending: false }).limit(cap),
      supabase.from("raid_sla_predictions").select("raid_item_id, breach_probability, days_to_breach, confidence, predicted_at").in("raid_item_id", ids).order("predicted_at", { ascending: false }).limit(cap),
      supabase.from("raid_financials").select("raid_item_id, currency, est_cost_impact, est_revenue_at_risk, est_penalties").in("raid_item_id", ids).limit(ids.length),
    ]);

    for (const s of scoresRes.data || []) { const id = String((s as any).raid_item_id || ""); if (id && !scoreByItem.has(id)) scoreByItem.set(id, s); }
    for (const p of predsRes.data  || []) { const id = String((p as any).raid_item_id || ""); if (id && !predByItem.has(id))  predByItem.set(id, p);  }
    for (const f of finsRes.data   || []) { const id = String((f as any).raid_item_id || ""); if (id) finByItem.set(id, f);                            }
  }
  ...
  const items = rows.map((r: any) => {
    ...
    return {
      id: r.id, public_id: publicId,
      project_id: r.project_id, project_title: r?.projects?.title || "Project",
      project_code: r?.projects?.project_code ?? null, project_code_label: codeLabel,
      ...
      score, score_source: ai ? "ai" : basicScore != null ? "basic" : null,
  ...
  return ok({
    scope: opts.scope, windowDays: opts.windowDays, type: opts.type, status: opts.status, items,
    meta: {
      scope: "org", organisationId, active_only: true, scopeMeta,
