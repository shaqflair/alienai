// src/app/api/portfolio/financial-plan-summary/route.ts — v8
// DEBUG: Added console logging to identify why totalBudget = 0
// FIX:   Broaden content field name scanning (approved_budget, total_approved etc.)
//        Also scan artifacts table columns more defensively
//
import "server-only";
...
function uniqStrings(xs: any): string[] {
  ...
  for (const v of arr) { const s = safeStr(v).trim(); if (!s) continue; const k = s.toLowerCase(); if (seen.has(k)) continue; seen.add(k); out.push(s); }
  return out;
}
...
function ragFromVariancePct(pct: number | null): "G" | "A" | "R" {
  if (pct === null) return "G"; if (pct > 5) return "R"; if (pct > 0) return "A"; return "G";
}
...
async function normalizeActiveIds(supabase: any, rawIds: string[]) {
  ...
  try {
    ...
    if (Array.isArray(r)) { const ids = r.filter(Boolean); if (!ids.length && rawIds.length) return failOpen("active filter 0"); return { ids, ok: true, error: null as string | null }; }
  ...
  } catch (e: any) { return failOpen(safeStr(e?.message || e || "active filter failed")); }
}

type PortfolioFilters = { projectName?: string[]; projectCode?: string[]; projectManagerId?: string[]; department?: string[]; };
function hasAnyFilters(f: PortfolioFilters) { return (f.projectName && f.projectName.length)||(f.projectCode && f.projectCode.length)||(f.projectManagerId && f.projectManagerId.length)||(f.department && f.department.length); }
function parseFiltersFromUrl(url: URL): PortfolioFilters {
  ...
  if (name.length) out.projectName = name; if (code.length) out.projectCode = code;
  if (pm.length) out.projectManagerId = pm; if (dept.length) out.department = dept;
  return out;
}
function parseFiltersFromBody(body: any): PortfolioFilters {
  ...
  if (names.length) out.projectName = names; if (codes.length) out.projectCode = codes;
  if (pms.length) out.projectManagerId = pms; if (depts.length) out.department = depts;
  return out;
}
function looksMissingRelation(err: any) { const msg = String(err?.message||err||"").toLowerCase(); return msg.includes("does not exist")||msg.includes("relation")||msg.includes("42p01"); }
function looksMissingColumn(err: any) { const msg = String(err?.message||err||"").toLowerCase(); return msg.includes("column")&&msg.includes("does not exist"); }
...
async function applyProjectFilters(supabase: any, scopedProjectIds: string[], filters: PortfolioFilters) {
  ...
  let rows: any[] = []; let lastErr: any = null;
  ...
  if (!rows.length) { meta.applied = true; meta.notes.push("filter fallback"); if (lastErr?.message) meta.notes.push(lastErr.message); return { projectIds: scopedProjectIds, meta }; }
  ...
  const filtered = rows.filter((p) => {
    const title = safeStr(p?.title).toLowerCase(); const code = projectCodeLabel(p?.project_code).toLowerCase();
  ...
  meta.applied = true; meta.counts = { before: scopedProjectIds.length, after: outIds.length };
  return { projectIds: outIds, meta, projectRows: rows };
}

/* ---- extract budget from any content shape ---- */
function extractBudgetFromContent(content: any): {
  totalApprovedBudget: number; totalBudgeted: number;
  totalForecast: number; totalActual: number; currency: string;
} {
  let totalApprovedBudget = 0, totalBudgeted = 0, totalForecast = 0, totalActual = 0;
  let currency = "GBP";
  ...
  // ── Approved budget: scan every plausible field name ──
  const approvedRaw =
  ...
  // ── Cost lines: snake_case and camelCase variants ──
  const costLines: any[] =
    Array.isArray(content.cost_lines) ? content.cost_lines :
  ...
  for (const line of costLines) {
    const budgeted = num(line?.budgeted ?? line?.budget ?? line?.planned ?? line?.amount, 0);
    const forecast = num(line?.forecast ?? line?.forecasted ?? line?.projected, 0);
  ...
  // ── If still 0, try top-level totals directly ──
...
async function handle(req: Request, filters: PortfolioFilters) {
  ...
  if (!projectIds.length) {
    return noStoreJson({
      ok: true, total_approved_budget: 0, total_spent: 0, variance_pct: null,
      rag: "G" as const, project_count: 0, currency: "GBP", project_ref: null, artifact_id: null,
      portfolio: { totalBudget: 0, totalForecast: 0, totalActual: 0, projectCount: 0, withPlanCount: 0 },
      projects: [],
      meta: { organisationId, scope: { ...scopeMeta, scopedIdsRaw: scopedProjectIdsRaw.length, scopedIdsActive: scopedProjectIds.length, active_filter_ok: active.ok, active_filter_error: active.error }, filters: filtered.meta },
    });
  }
  ...
  if (Array.isArray((filtered as any).projectRows) && (filtered as any).projectRows.length) {
    ...
    const { data: projRows, error: projErr } = await supabase
      ...
      .in("id", projectIds).order("title", { ascending: true });
  ...
  console.log(`[fps] projectIds=${projectIds.length}, artifactsFound=${artifacts.length}, plansMatched=${planByProject.size}`);

  // 5) Role lookup
  let roleByProject = new Map<string, string>();
  try {
    const { data: memberships } = await supabase
      .from("project_members").select("project_id, role, is_active, removed_at")
      .eq("user_id", user.id).is("removed_at", null).in("project_id", projectIds).limit(20000);
    for (const m of memberships ?? []) {
      const pid = String((m as any)?.project_id||"").trim();
      if (pid && !roleByProject.has(pid)) roleByProject.set(pid, safeStr((m as any)?.role).trim()||"viewer");
    }
  } catch { roleByProject = new Map(); }
  ...
  const summaries = (projects ?? []).map((project: any) => {
    ...
    console.log(`[fps] project=${pid} artifact=${artifact?.id ?? "NONE"} contentNull=${content === null}`);
    if (content) {
      console.log(`[fps] project=${pid} content keys:`, Object.keys(content).slice(0, 15));
    }

    let hasFinancialPlan = false;
    let extracted = { totalApprovedBudget: 0, totalBudgeted: 0, totalForecast: 0, totalActual: 0, currency: "GBP" };
    ...
    const { totalApprovedBudget, totalBudgeted, totalForecast, totalActual, currency } = extracted;
    const effectiveBudget = totalApprovedBudget > 0 ? totalApprovedBudget : totalBudgeted;
    const variancePct = effectiveBudget > 0 ? Math.round(((totalForecast - effectiveBudget) / effectiveBudget) * 1000) / 10 : null;
    const burnPct = effectiveBudget > 0 ? Math.round((totalActual / effectiveBudget) * 100) : 0;
    ...
    return {
      projectId: pid, projectCode: project?.project_code ?? null,
      projectCodeLabel: projectCodeLabel(project?.project_code) || null,
      title: project?.title ?? "Project", colour: project?.colour ?? "#00b8db",
      status: project?.resource_status ?? "confirmed",
      startDate: project?.start_date ?? null, finishDate: project?.finish_date ?? null,
      role: roleByProject.get(pid) || "viewer",
      hasFinancialPlan, artifactId: artifact?.id ?? null, lastUpdated: artifact?.updated_at ?? null,
      currency,
      totals: { approvedBudget: totalApprovedBudget, budget: effectiveBudget, forecast: totalForecast, actual: totalActual, variance: totalForecast - effectiveBudget, variancePct, burnPct },
      monthlyBreakdown,
    };
  });

  // 7) Portfolio aggregates
  ...
  const portfolioVariancePct = effectivePortBudget > 0 ? Math.round(((portTotalForecast - effectivePortBudget) / effectivePortBudget) * 1000) / 10 : null;
  ...
  console.log(`[fps] withPlan=${withPlan.length} effectivePortBudget=${effectivePortBudget} rag=${portfolioRag}`);

  const portfolio = { totalBudget: effectivePortBudget, totalForecast: portTotalForecast, totalActual: portTotalActual, projectCount: summaries.length, withPlanCount: withPlan.length, variancePct: portfolioVariancePct, rag: portfolioRag };

  return noStoreJson({
    ok: true,
    total_approved_budget: effectivePortBudget > 0 ? effectivePortBudget : null,
    total_spent: portTotalActual > 0 ? portTotalActual : null,
    variance_pct: portfolioVariancePct, pending_exposure_pct: null,
    rag: portfolioRag, currency: portfolioCurrency,
    ...
    meta: {
      organisationId,
      scope: { ...scopeMeta, scopedIdsRaw: scopedProjectIdsRaw.length, scopedIdsActive: scopedProjectIds.length, active_filter_ok: active.ok, active_filter_error: active.error },
...
export async function GET(req: Request) {
  try { const url = new URL(req.url); return await handle(req, parseFiltersFromUrl(url)); }
  catch (e: any) { console.error("[fps][GET]", e); return noStoreJson({ ok: false, error: safeStr(e?.message||e) }, 500); }
}

export async function POST(req: Request) {
  try { const body = await req.json().catch(() => ({})); return await handle(req, parseFiltersFromBody(body)); }
  catch (e: any) { console.error("[fps][POST]", e); return noStoreJson({ ok: false, error: safeStr(e?.message||e) }, 500); }
}