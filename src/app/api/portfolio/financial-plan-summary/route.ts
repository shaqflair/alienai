...
/* ─── SINGLE clean extractBudgetFromContent ─────────────────────────────── */
...
async function handle(req: Request, filters: PortfolioFilters) {
  ...
  for (const artType of artTypes) {
    const { data, error } = await supabase
      .from("artifacts").select(artSel)
      .in("project_id", projectIds).eq("type", artType)
      .order("updated_at", { ascending: false }).limit(20000);
    if (!error && Array.isArray(data) && data.length > 0) { artifacts = data; break; }
  }
  ...
  const summaries = (projects ?? []).map((project: any) => {
    ...
    const variancePct = effectiveBudget > 0 ? Math.round(((variance / effectiveBudget) * 100) * 10) / 10 : null;
    ...
    return {
      ...
      currency,
      totals: { approvedBudget: totalApprovedBudget, budget: effectiveBudget, forecast: totalForecast, actual: totalActual, variance, variancePct, burnPct },
      monthlyBreakdown,
    };
  });
  ...
  const portfolio = {
    ...
    rag: portfolioRag,
    meta: { totalApprovedBudget: portTotalApproved, totalEffectiveBudget: portTotalBudget },
  };
