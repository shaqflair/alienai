...
function computeScheduleScore(k: ScheduleAgg | null) {
  ...
  if (due === 0 && overdue === 0) {
    return { score: 100, note: "No milestones due or overdue." };
  }
...
function computeRaidScore(stats: { open_high: number; overdue: number; sla_high: number }) {
  let score = 100;
  score -= Math.min(40, stats.open_high * 8);
  score -= Math.min(25, stats.overdue * 6);
  ...
  try {
    ...
    if (error) {
      if (looksMissingRelation(error)) {
        return {
          ok: false,
          error: "schedule_milestones missing",
          data: null as ScheduleAgg | null,
        };
      }
        ...
        const slip = Math.max(
          ...
          Math.round(
            (new Date(`${end}T00:00:00.000Z`).getTime() -
              new Date(`${base}T00:00:00.000Z`).getTime()) /
              86400000,
          ),
        );
        slipSum += slip;
    ...
    return {
      ok: false,
      error: String(e?.message || "schedule query failed"),
      data: null as ScheduleAgg | null,
    };
  }
}
...
async function fetchRaidStats(supabase: any, projectIds: string[]) {
  ...
  for (const r of rows) {
    ...
    const score =
      (r as any)?.probability == null || (r as any)?.severity == null
        ? null
        : Math.round((p * s) / 100);
...
async function fetchStaleActivity(supabase: any, projectIds: string[]) {
  ...
  for (const p of rows) {
    ...
    if (now - createdAt < sevenDaysMs) continue;
...
async function handle(req: Request, method: "GET" | "POST") {
  try {
    ...
    const raidScore = computeRaidScore({
      open_high: raid.open_high,
      overdue: raid.overdue,
      sla_high: raid.sla_high,
    });
    ...
    return jsonOk({
      ...
      raid: raid.ok
        ? { open_high: raid.open_high, overdue: raid.overdue, sla_high: raid.sla_high }
        : null,
      approvals: {
        pending: approvals.pending,
        source: approvals.source,
        ok: approvals.ok,
        error: (approvals as any).error ?? null,
      },
      activity: {
        stale7d: activity.stale7d,
        ok: activity.ok,
        meta: activity.meta,
      },
      meta: {
        ...
        scope: {
          ...
          orgScope: scopeMeta?.orgScope ?? null,
        },
        notes:
          finalProjectIds.length === 0
            ? ["No active projects in scope after filtering."]
            : [],
      },
    });
  } catch (e: any) {
