...
export async function POST(req: Request) {
  try {
    ...
    // 2. Check email matches invite (if invite has a specific email)
    ...
    // 4. Create the user via admin API (bypasses signup restriction)
    ...
    if (accErr) {
      // Non-fatal -- membership was created, just log it
      console.error("Failed to mark invite accepted:", accErr.message);
    }

    // 7. Create a session for the new user so they are signed in immediately
    ...
    if (sessionErr || !session?.properties?.hashed_token) {
      // Account created but can't auto-sign-in -- send them to login
      return ok({
        created:         true,
        organisation_id: organisationId,
        role,
        redirect:        `/login?next=/onboarding&info=account_created`,
        auto_signin:     false,
      });
    }
    ...
    return ok({
      ...
      organisation_id: organisationId,
      role,
      redirect:        session.properties.action_link,
      auto_signin:     true,
    });
