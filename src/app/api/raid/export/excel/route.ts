...
function addRaidSheet(wb: ExcelJS.Workbook, name: string, rows: RaidRow[]) {
  ...
  rows.forEach((row, idx) => {
    ...
    if (scoreColIndex > 0) {
      tintScoreCell(excelRow.getCell(scoreColIndex), sc);
    }
  });
...
export async function GET(req: NextRequest) {
  try {
    ...
    const projectId = safeStr(searchParams.get("projectId")).trim();
    ...
    const { data: items, error } = await supabase
      .from("raid_items")
      .select(
        "public_id,type,status,priority,probability,severity,impact,owner_label,title,description,response_plan,next_steps,notes,ai_rollup,due_date,updated_at,created_at"
      )
      .eq("project_id", projectId)
    ...
    // Group into tabs (exact names requested)
    const TYPES = [
      { db: "Risk", sheet: "Risks" },
    ...
    const byType: Record<string, RaidRow[]> = {};
    for (const t of TYPES) byType[t.db] = [];
    for (const row of rowsAll) {
      const key = safeStr(row.type).trim();
      if (byType[key]) byType[key].push(row);
      // if any weird type sneaks in, ignore or you can route to "Other"
    }
    ...
    const dueSoonCount = (items ?? []).filter((it: any) => {
      if (!it?.due_date) return false;
      const d = new Date(String(it.due_date));
      if (Number.isNaN(d.getTime())) return false;
      const diffDays = (d.getTime() - now.getTime()) / (1000 * 60 * 60 * 24);
      const st = safeStr(it.status).toLowerCase();
      return diffDays >= 0 && diffDays <= 14 && st !== "closed";
    }).length;
    ...
    if (organisationName) addKV("Organisation", organisationName);
    ...
    [
      ["Open", openCount],
      ["High Exposure (Score ≥ 61)", highExposure],
      ["Due Soon (14d)", dueSoonCount],
      ["Closed", closedCount],
      ["Total", total],
    ].forEach(([k, v]) => {
      summary.getCell(r, 1).value = k;
      summary.getCell(r, 2).value = v as any;
      summary.getCell(r, 2).font = { bold: true };
      r++;
    });

    r += 2;
    summary.getCell(r, 1).value = "Counts by Type";
    summary.getCell(r, 1).font = { bold: true, size: 12 };
    r++;

    for (const t of TYPES) {
      summary.getCell(r, 1).value = t.sheet;
      summary.getCell(r, 2).value = byType[t.db]?.length ?? 0;
      r++;
    }
    ...
    // Type tabs (what you asked for)
    for (const t of TYPES) {
      addRaidSheet(wb, t.sheet, byType[t.db] ?? []);
    }
