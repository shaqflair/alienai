...
export async function GET(req: Request) {
  ...
  if (!stripeKey) return bad("Stripe not configured", 500);
  ...
  if (!PLAN_PRICES[plan])   return bad(`Unknown plan or missing STRIPE_PRICE_${plan.toUpperCase()} env var`);
  ...
  if (!customerId) {
    const customer = await stripe.customers.create({
      ...
      name:     safeStr(org?.name) || undefined,
