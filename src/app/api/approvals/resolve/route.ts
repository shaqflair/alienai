...
export async function POST(req: Request) {
  try {
    ...
    const applicable = (rules ?? []).filter((r: any) =>
      inBand(amount, r.min_amount, r.max_amount)
    );
    ...
    const groupIds = Array.from(
      new Set(
        applicable
          .map((r: any) => String(r.approval_group_id ?? ""))
          .filter(Boolean)
      )
    );
    const directUserIds = Array.from(
      new Set(
        applicable
          .map((r: any) => String(r.approver_user_id ?? ""))
          .filter(Boolean)
      )
    );
    ...
    if (groupIds.length) {
      ...
      groupMembers = (gm.data ?? []).filter((m: any) =>
        "is_active" in m ? m.is_active !== false : true
      );
    }

    const groupUserIds = Array.from(
      new Set(groupMembers.map((m: any) => String(m.user_id ?? "")).filter(Boolean))
    );
    const allUserIds = Array.from(new Set([...directUserIds, ...groupUserIds]));

    // IMPORTANT: loadProfilesByUserIds returns an object-like map in your helpers
    const profByUser = (await loadProfilesByUserIds(supabase, allUserIds)) as Record<
      string,
      any
    >;
    ...
    const steps = Array.from(byStep.keys())
      ...
      .map((step) => {
        ...
        for (const r of stepRules) {
          ...
          if (uid) {
            ...
            const members = groupMembers.filter(
              (m: any) => String(m.group_id) === gid
            );
            for (const m of members) {
              const mu = String(m.user_id ?? "").trim();
              if (!mu) continue;
              const p = profByUser[mu];
              people.push({
                user_id: mu,
                full_name: safeStr(p?.full_name).trim() || null,
                email: safeStr(p?.email).trim() || null,
        ...
        const uniq = people.filter((x) =>
          seen.has(x.user_id) ? false : (seen.add(x.user_id), true)
        );
