...
// WBS items include ALL org projects (including pipeline) — no active-only filter.
import "server-only";
...
type Item = {
  project_id: string; project_title: string; project_code?: string | null;
  artifact_id: string; artifact_updated_at: string | null; artifact_updated_at_iso?: string | null;
  due_date: string | null; due_date_iso?: string | null;
  wbs_row_id: string; wbs_row_index: number; title: string;
  owner_label?: string | null; is_done: boolean; is_overdue: boolean;
  effort_missing: boolean; href: string;
};
...
export async function GET(req: Request) {
  try {
    ...
    if (!projectIds.length) {
      return jsonOk({ items: [], nextCursor: null, meta: { projectCount: 0, scope: "portfolio", active_only: false } });
    }
    ...
    for (const a of artRows || []) {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        itemsAll.push({
          project_id: projectId, project_title, project_code,
          ...
          due_date_iso: dueIso,
          wbs_row_id: rowId, wbs_row_index: i, title,
          owner_label: owner || null,
          is_done: isDone, is_overdue: isOverdue, effort_missing: effortMissing, href,
        });
      }
    }
    ...
    return jsonOk({
      ...
      meta: {
        ...
        scope: "portfolio", active_only: false,
      },
    });
  } catch (e: any) {
