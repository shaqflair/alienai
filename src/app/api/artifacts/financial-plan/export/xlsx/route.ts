...
export async function GET(req: NextRequest) {
  try {
    ...
    if (projectErr) {
      console.error("[xlsx] Project lookup failed:", projectErr.message);
    }
    ...
    if (costLines.length > 0) {
      ...
      tRow.getCell(7).border = NAVY_BORDER;
      tRow.getCell(7).fill = FILLS.sub;
      tRow.getCell(8).border = NAVY_BORDER;
      tRow.getCell(8).fill = FILLS.sub;
    }
    ...
    if (hasMonthly && costLines.length > 0) {
      ...
      costLines.forEach((line: any, i: number) => {
        ...
        const rowVals: any[] = [capWords(safeStr(line.category)), safeStr(line.description || line.category)];
    ...
    if (changeExp.length > 0) {
      ...
      wsCE.columns = [{ width: 16 }, { width: 30 }, { width: 18 }, { width: 16 }, { width: 30 }];
