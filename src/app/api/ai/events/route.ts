  ...
  try {
    ...
    if (!projectId || !artifactId || !suggestionId) {
      return NextResponse.json(
        { ok: false, error: "Missing projectId, artifactId, or suggestionId" },
        { status: 400 }
      );
    }
    ...
    // Membership gate (RLS should also enforce, but we do explicit check for clear errors)
    ...
    // Defensive: suggestion must belong to artifact if it specifies one
    ...
    // Optional fast-path: add_stakeholder suggestions can insert to canonical stakeholders table
        ...
        return NextResponse.json(
          {
            ...
            warning: `Created stakeholder but failed to mark suggestion applied: ${uErr.message}`,
          },
          { status: 200 }
        );
      }
    ...
    // Load target artifact (RLS-protected)
    ...
    // Save artifact JSON (RLS update requires editor)
    ...
    // Mark suggestion applied (RLS + trigger enforces actioned_by/decided_at)
    const nowIso = new Date().toISOString();
    ...
    // Best-effort event log (don’t fail if project_events is locked down)
    try {
