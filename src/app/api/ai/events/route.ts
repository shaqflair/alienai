...
async function loadProjectsForOrgs(supabase: any, orgIds: string[]) {
  ...
  // projects_active already includes project_code (human id) in your setup
  const { data, error } = await supabase
...
async function resolveProjectUuid(
  ...
  for (const col of HUMAN_COL_CANDIDATES) {
    ...
    if (error) {
      ...
      throw new Error(
        `${shapeSbError(error).code || "db_error"}: ${shapeSbError(error).message}`
      );
    }
...
type ProjectMeta = {
  project_human_id: string | null; // (we store project_code here)
...
async function loadProjectMeta(
  ...
  const pickMemberForRoles = async (roles: string[]) => {
    ...
    if (memErr)
      throw new Error(
        `${shapeSbError(memErr).code || "db_error"}: ${shapeSbError(memErr).message}`
      );
  ...
  const pmUserId: string | null = await pickMemberForRoles([
    "project_manager",
    "owner",
  ]);
...
/* ─────────────────────────────────────────────────────────────────────────────
   BULK ORG-SCOPE DUE LOADER (prefers work_items)
───────────────────────────────────────────────────────────────────────────── */
...
async function buildDueDigestOrgBulk(args: {
  ...
  const [artRes, msRes, wResTry, raidRes, chRes] = await Promise.all([
    ...
    // ✅ FIX-E7: prefer work_items (fallback to wbs_items below)
    supabase
      .from("work_items")
      .select(
        "id,project_id,title,type,stage,status,due_date,artifact_id,parent_id,milestone_id,created_at,started_at,completed_at,updated_at"
      )
      .in("project_id", projectIds)
    ...
    supabase
      ...
      .select(
        "id,project_id,title,seq,status,delivery_status,decision_status,updated_at,artifact_id,review_by"
      )
      .in("project_id", projectIds)
      .order("updated_at", { ascending: false })
      .limit(5000),
  ...
  if (raidRes.error && isMissingColumnError(raidRes.error.message, "source_artifact_id")) {
    const fb = await supabase
      .from("raid_items")
      .select(
        "id,project_id,public_id,item_no,type,title,description,status,due_date,owner_label,ai_status,priority"
      )
      .in("project_id", projectIds)
      .not("due_date", "is", null)
      .order("due_date", { ascending: true })
  ...
  if (wResTry.error && !workRows.length) {
    ...
    const wbsFb = await supabase
      .from("wbs_items")
      .select(
        "id,project_id,name,description,status,due_date,owner,source_artifact_id,source_row_id,sort_order,parent_id,updated_at"
      )
      .in("project_id", projectIds)
      .not("due_date", "is", null)
      .order("due_date", { ascending: true })
  ...
  // work_items OR legacy wbs_items (both normalized to itemType=work_item)
  for (const w of workRows) {
    ...
    const ownerLabel =
      ...
      safeStr((w as any)?.owner_label).trim() ||
      null;

    const title =
      safeStr((w as any)?.title).trim() ||
      safeStr((w as any)?.name).trim() ||
      "Work item";
  ...
  const dueSoon = all
    ...
    .sort((a: any, b: any) => {
      ...
      if (safeStr(a?.meta?.project_name) !== safeStr(b?.meta?.project_name))
        return safeStr(a?.meta?.project_name).localeCompare(safeStr(b?.meta?.project_name));
...
/* ─────────────────────────────────────────────────────────────────────────────
   ✅ buildDashboardStatsFromBulkRows (no new queries)
───────────────────────────────────────────────────────────────────────────── */
...
function buildDashboardStatsFromBulkRows(args: {
  ...
  projects: number;
  success: {
    work_packages_completed: number;
    milestones_done: number;
    raid_closed: number;
    changes_closed: number;
    wbs_done: number;
    lessons: number;
  };
  ...
  const wbs_done = workRows.filter((x: any) => {
    ...
    // support work_items or legacy wbs_items
...
/* ─────────────────────────────────────────────────────────────────────────────
   LEGACY: buildDashboardStatsGlobal (kept; now accepts preloads)
───────────────────────────────────────────────────────────────────────────── */
...
async function buildDashboardStatsGlobal(
  ...
  preloadedRows?: { msRows: any[]; workRows: any[]; artRows: any[]; raidRows: any[]; chRows: any[] }
) {
  ...
  const [msResult, workTry, lessonsResult, raidResult, changesResult] = await Promise.all([
    supabase
      .from("schedule_milestones")
    ...
    supabase
      .from("work_items")
      .select("id,status,project_id,completed_at")
      .in("project_id", projectIds)
      .limit(20000),
  ...
  let workRows: any[] = Array.isArray(workTry.data) ? workTry.data : [];
  if (workTry.error) {
    ...
    if (missingWorkItems) {
      const wbsFb = await supabase
        ...
        .in("project_id", projectIds)
        .limit(20000);
      workRows = Array.isArray(wbsFb.data) ? wbsFb.data : [];
    }
  }
  ...
  const milestones_due_30d = ms.filter((m: any) => {
    const st = safeLower(m?.status);
    if (st === "done" || st === "completed" || st === "closed") return false;
    const d = parseDueToUtcDate(m?.end_date ?? m?.start_date);
    return !!(d && inWindow(d, from, to));
  ...
  const lessons_count = lessons.filter((a: any) => {
    const t = safeLower(a?.artifact_type || a?.type);
    return t === "lessons_learned" && (a?.is_current === true || a?.is_current == null);
  }).length;
  ...
  const changes_closed = changes.filter((c: any) => {
    const s = safeLower(c?.status);
    const d = safeLower(c?.delivery_status);
    return s === "closed" || s === "implemented" || d === "closed" || d === "implemented";
  }).length;

  const milestones_done = ms.filter((m: any) => {
    const st = safeLower(m?.status);
    return st === "done" || st === "completed" || st === "closed";
  }).length;
...
// ✅ FIX-E7: prefer work_items; fallback to wbs_items
async function loadDueWorkItems(
  ...
  return rows
    .map((w: any) => {
      ...
      const srcArtifactId = usedLegacy
        ? safeStr(w?.source_artifact_id).trim()
        : safeStr(w?.artifact_id).trim();

      const ownerLabel = usedLegacy ? safeStr(w?.owner).trim() || null : null;

      const title = usedLegacy
        ? safeStr(w?.name).trim() || "WBS item"
        : safeStr(w?.title).trim() || "Work item";
...
function buildDeliveryHeadline(
  ...
  if (rag === "red") {
    if (overdueCount === 1) return `${proj}1 overdue item requires immediate action to protect delivery.`;
...
export async function POST(req: Request) {
  try {
    ...
    /* ---------- ✅ FIX-E5: warn on unknown eventType then fall through to draft assist ---------- */
    const knownTypes = ["artifact_due", "delivery_report", "change_ai_impact_assessment"];
    if (eventType && !knownTypes.includes(eventType)) {
      console.warn(
        `[ai/events] Unrecognised eventType "${eventType}" — falling through to draft assist. If this is intentional, add it to knownTypes.`
      );
    }
