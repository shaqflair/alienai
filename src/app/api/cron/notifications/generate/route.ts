...
// ✅ FIX-CR3: organisation_id resolved via project_id → projects table (view doesn't expose it)
// ✅ FIX-CR4: window_days = 30 set on every cache row (was missing, breaking pending route filter)
// ✅ FIX-CR5: No more artifact_approval_decisions join — view already filters to pending steps
...
async function handler(req: Request) {
  try {
    ...
    // ✅ FIX-CR1: Query the view — all joins already done, correct columns exposed
    ...
    // ✅ FIX-CR3: Load project metadata to get organisation_id (not in view)
    ...
    // ✅ Build cache rows from view data
    const cacheRows: any[] = [];
    ...
    for (const row of rows) {
      ...
      // ✅ FIX-CR3: skip if project has no org
      ...
      // ✅ FIX-CR2: use pending_email as approver label (the view's actual field)
      // Fall back to approver_role if email not set
      ...
      cacheRows.push({
        organisation_id: orgId,                                    // ✅ FIX-CR3: real org UUID
        project_id: pid,
    ...
    for (let i = 0; i < cacheRows.length; i += 500) {
      const { error } = await supabase
        .from("exec_approval_cache")
        .insert(cacheRows.slice(i, i + 500));
      if (error) throw error;
    }

    // Rebuild bottlenecks — group by approver email
    await supabase
    ...
    const byApprover = new Map<string, {
      label: string; userId: string | null; groupId: string | null;
      open: number; breached: number; atRisk: number; maxOverdue: number;
    }>();

    for (const r of cacheRows) {
      const key = r.approver_label || "Unassigned";
      const cur = byApprover.get(key) ?? {
        label: r.approver_label, userId: r.approver_user_id,
        groupId: null, open: 0, breached: 0, atRisk: 0, maxOverdue: 0,
      };
      cur.open++;
    ...
    try { await supabase.rpc("exec_refresh_intel"); } catch { /* ignore */ }

    // Log for Vercel — confirm org distribution after fix
    const orgDist: Record<string, number> = {};
    for (const r of cacheRows) {
      const o = safeStr(r.organisation_id);
    ...
    return jsonOk({
      ...
      skipped_null_org: skippedNullOrg,
      orgs: orgDist,
    });
