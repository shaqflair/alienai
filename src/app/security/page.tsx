...
const C = {
  cyan:"#00C2E8", cyanLt:"#57E7FF", green:"#22C55E",
  amber:"#EAB308", text:"#F5F8FC", muted:"#A0ACBC", muted2:"#667184",
};
const dp = "'Plus Jakarta Sans',sans-serif";
const mn = "'Fira Code',monospace";

function HoverLink({ href, children, color }: { href:string; children:React.ReactNode; color?:string }) {
  const [hov, setHov] = useState(false);
  return (
    <a href={href} style={{ color: hov ? (color || C.text) : C.muted, transition:"color 0.2s", fontSize:13 }}
...
function Card({ children, bg, border }: { children:React.ReactNode; bg?:string; border?:string }) {
  ...
  return (
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{ padding:"20px 22px", borderRadius:16, transition:"border-color .2s,transform .2s",
        background: bg || "rgba(255,255,255,0.03)",
        border: hov ? "1px solid rgba(0,194,232,0.22)" : (border || "1px solid rgba(255,255,255,0.07)"),
...
export default function SecurityPage() {
  ...
  return (
    <main style={{ minHeight:"100vh", background:"linear-gradient(180deg,#03050A 0%,#060C14 50%,#04080F 100%)", color:C.text, fontFamily:"'Inter',system-ui,sans-serif", WebkitFontSmoothing:"antialiased" as any }}>
      <link rel="preconnect" href="https://fonts.googleapis.com" />
      <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700&family=Fira+Code:wght@400;500&family=Inter:wght@400;500;600;700&display=swap" rel="stylesheet" />
      ...
      <div style={{ maxWidth:900, margin:"0 auto", padding:"80px 28px", position:"relative", zIndex:1 }}>
        ...
        <HoverLink href="/">&larr; Back to Aliena AI</HoverLink>

        <div style={{ marginBottom:64, marginTop:40 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:14, marginBottom:64 }}>
          {[{val:"AES-256",label:"Encryption standard"},{val:"TLS 1.2+",label:"In-transit security"},{val:"Row-Level",label:"Database security"}].map(s=>(
            <div key={s.val} style={{ padding:"22px 16px", borderRadius:16, background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", textAlign:"center" }}>
        ...
        {[{num:"01",title:"Data Protection",items:dataProtection},{num:"02",title:"Infrastructure",items:infrastructure},{num:"04",title:"Access Controls",items:accessControls}].map(sec=>(
          <section key={sec.title} style={{ marginBottom:56 }}>
            <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:24 }}>
              <span style={{ fontFamily:mn, fontSize:10, color:C.cyanLt, letterSpacing:"0.14em", textTransform:"uppercase" as any }}>{sec.num}</span>
              <h2 style={{ fontFamily:dp, fontSize:26, fontWeight:700, letterSpacing:"-0.03em" }}>{sec.title}</h2>
            </div>
        ...
        <section style={{ marginBottom:56 }}>
          <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:24 }}>
            <span style={{ fontFamily:mn, fontSize:10, color:C.cyanLt, letterSpacing:"0.14em", textTransform:"uppercase" as any }}>03</span>
            <h2 style={{ fontFamily:dp, fontSize:26, fontWeight:700, letterSpacing:"-0.03em" }}>Compliance Roadmap</h2>
          </div>
          <div style={{ display:"grid", gap:12 }}>
            {compliance.map(item=>(
              <Card key={item.label} bg={item.bg} border={"1px solid " + item.bc}>
                <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:10 }}>
                  <span style={{ fontFamily:dp, fontWeight:700, fontSize:15 }}>{item.label}</span>
                  <span style={{ fontSize:10, fontWeight:700, padding:"3px 10px", borderRadius:999, background:item.bg, color:item.sc, border:"1px solid " + item.bc, fontFamily:mn, letterSpacing:"0.06em", textTransform:"uppercase" as any }}>{item.status}</span>
                </div>
                <div style={{ fontSize:14, color:C.muted, lineHeight:1.7 }}>{item.body}</div>
              </Card>
            ))}
        ...
        <section style={{ marginBottom:56 }}>
          <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:24 }}>
            <span style={{ fontFamily:mn, fontSize:10, color:C.cyanLt, letterSpacing:"0.14em", textTransform:"uppercase" as any }}>05</span>
            <h2 style={{ fontFamily:dp, fontSize:26, fontWeight:700, letterSpacing:"-0.03em" }}>Responsible Disclosure</h2>
          </div>
          <div style={{ padding:"28px", background:"rgba(0,194,232,0.06)", border:"1px solid rgba(0,194,232,0.18)", borderRadius:20 }}>
