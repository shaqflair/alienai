...
export default function RisksClient({
  ...
  initialWindowDays: 7 | 14 | 30 | 60;
  initialScope: "window" | "overdue" | "all";
  initialType: "all" | "Risk" | "Issue" | "Assumption" | "Dependency";
  initialStatus: "all" | "open" | "in_progress" | "mitigated" | "closed" | "invalid";
}) {
  const router = useRouter();

  const [windowDays, setWindowDays] = useState<7 | 14 | 30 | 60>(initialWindowDays);
  const [scope, setScope] = useState<"window" | "overdue" | "all">(initialScope);
  const [type, setType] = useState<"all" | "Risk" | "Issue" | "Assumption" | "Dependency">(initialType);
  const [status, setStatus] = useState<"all" | "open" | "in_progress" | "mitigated" | "closed" | "invalid">(initialStatus);
                ...
                <tbody>
                  {loading ? (
                    ...
                    items.map((it) => {
                      ...
                      const exposure =
                        (n(it.est_cost_impact, 0) || 0) +
                        (n(it.est_revenue_at_risk, 0) || 0) +
                        (n(it.est_penalties, 0) || 0);
