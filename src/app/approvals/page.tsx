// src/app/approvals/page.tsx — Control Centre v3
// ✅ Overview: counts from /api/executive/approvals (real counts object, not derived)
// ✅ Bottlenecks: calls /api/executive/approvals/bottlenecks (not client-derived)
// ✅ PM Performance: /api/executive/pm-performance (unchanged)
// ✅ At Risk: /api/executive/projects/at-risk?active_only=true (unchanged)
// ✅ Digest: /api/executive/digest (unchanged)

"use client";

import React, { useEffect, useState, useMemo, useCallback } from "react";
import {
  ShieldCheck, Users, CheckCircle2, AlertTriangle, Clock, ChevronDown,
  Calendar, BarChart2, Flame, Activity, RefreshCw, UserCheck, Zap,
...
interface RiskSignal { key: string; label: string; detail: string; score: number; triggered: boolean }
...
interface PmItem {
  ...
  role: string; color: string; projects_managed: number; department?: string | null;
...
interface Project {
  id: string; title: string | null; project_code: string | null; project_manager_id: string | null;
}
  ...
  return (
    <div style={{ ...CARD, padding: "18px 20px", marginBottom: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14, paddingBottom: 10, borderBottom: "1px solid rgba(226,232,240,0.6)" }}>
        ...
        {count !== undefined && (
          <span style={{ background: "#6366f1", color: "white", borderRadius: 20, padding: "1px 8px", fontSize: 10, fontWeight: 700 }}>{count}</span>
        )}
      </div>
      {children}
    </div>
...
function PmCard({ pm, idx, projects, onAssignProject }: { pm: PmItem; idx: number; projects: Project[]; onAssignProject: (pmId: string, projectId: string) => void }) {
  const [expanded, setExpanded] = useState(false);
  const [assigning, setAssigning] = useState(false);
  const rate = pm.decisions.approval_rate;

  return (
    <m.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.38, delay: idx * 0.07, ease: [0.16, 1, 0.3, 1] }} style={{ ...CARD, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ padding: "16px 16px 16px 20px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {pm.department && <div style={{ fontSize: 10, color: "#94a3b8", marginTop: 2 }}>{pm.department}</div>}
          </div>
          <RagBadge rag={pm.rag} />
        </div>
...
function OverviewTab({ cacheItems, loading, counts }: { cacheItems: CacheItem[]; loading: boolean; counts: LiveCounts | null }) {
  const display = counts
    ? { total: counts.pending, breached: counts.breached, at_risk: counts.at_risk, pending: Math.max(0, counts.pending - counts.at_risk - counts.breached) }
    : (() => {
        ...
        return { total: cacheItems.length, pending, at_risk, breached };
      })();
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        {[
          { label: "Total Pending", value: display.total,    color: "#6366f1", icon: <Clock size={16} color="#6366f1" /> },
          { label: "SLA Breached",  value: display.breached, color: "#e11d48", icon: <Flame size={16} color="#e11d48" /> },
          { label: "At Risk",       value: display.at_risk,  color: "#d97706", icon: <AlertTriangle size={16} color="#d97706" /> },
          { label: "Within SLA",    value: display.pending,  color: "#10b981", icon: <CheckCircle2 size={16} color="#10b981" /> },
        ].map((s, i) => (
...
function BottlenecksTab() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch("/api/executive/approvals/bottlenecks", { credentials: "include", cache: "no-store" })
      .then((r) => r.json())
      .then((j) => {
        if (j?.ok) setItems(j.items ?? []);
        else setError(j?.error ?? "Failed to load bottlenecks");
      })
  ...
  if (error) return <div style={{ padding: "20px", borderRadius: 12, background: "rgba(255,241,242,0.8)", border: "1px solid rgba(253,164,175,0.4)", color: "#9f1239", fontSize: 13 }}>⚠ {error}</div>;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      {items.map((b, i) => {
        const pct = Math.max(8, (b.pending_count / maxCount) * 100);
        const heat: Rag = b.avg_wait_days >= 5 ? "R" : b.avg_wait_days >= 3 ? "A" : "G";
        const rc = RAG_CFG[heat];
        return (
          <m.div key={i} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: i * 0.06 }} style={{ ...CARD, position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ position: "relative", padding: "14px 16px", display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 10, color: "#94a3b8" }}>
                  {b.pending_count} pending · avg {b.avg_wait_days.toFixed(1)}d · max {b.max_wait_days}d · {b.projects_affected} project{b.projects_affected !== 1 ? "s" : ""}
                </div>
              </div>
              <RagBadge rag={heat} />
            </div>
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <div style={{ flex: 1, height: 6, borderRadius: 4, background: "rgba(241,245,249,0.9)", overflow: "hidden" }}>
        <m.div initial={{ width: 0 }} animate={{ width: `${score}%` }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }} style={{ height: "100%", borderRadius: 4, background: cfg.barColor }} />
      </div>
...
function RiskCard({ item, idx }: { item: ProjectRisk; idx: number }) {
  ...
  return (
    <m.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: idx * 0.06, ease: [0.16, 1, 0.3, 1] }} style={{ ...CARD, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ padding: "14px 16px 14px 20px" }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10, marginBottom: 10 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
              {item.project_code && <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 9, fontWeight: 700, color: "#4338ca", background: "rgba(238,242,255,0.8)", border: "1px solid rgba(199,210,254,0.6)", borderRadius: 4, padding: "1px 5px" }}>{item.project_code}</span>}
            ...
            <div style={{ fontSize: 10, color: "#94a3b8" }}>
              {[item.overdue_steps > 0 && `${item.overdue_steps} overdue step${item.overdue_steps !== 1 ? "s" : ""}`, item.days_since_activity != null && `${item.days_since_activity}d since activity`, item.rejection_rate != null && `${item.rejection_rate}% rejection rate`].filter(Boolean).join(" · ")}
            </div>
          </div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 10 }}>
          {item.signals.filter((s) => s.triggered).map((s) => (
            <span key={s.key} style={{ display: "inline-flex", alignItems: "center", gap: 4, borderRadius: 8, padding: "2px 8px", fontSize: 9, fontWeight: 600, background: "rgba(255,241,242,0.8)", border: "1px solid rgba(253,164,175,0.4)", color: "#9f1239" }}>⚠ {s.label}</span>
          ))}
          {item.signals.filter((s) => s.triggered).length === 0 && <span style={{ fontSize: 10, color: "#10b981", fontWeight: 600 }}>✓ No risk signals triggered</span>}
        </div>

        <button onClick={() => setExpanded((e) => !e)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "5px 0", background: "none", border: "none", cursor: "pointer", fontSize: 10, fontWeight: 600, color: "#6366f1" }}>
          ...
          {expanded && (
            <m.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.25 }} style={{ overflow: "hidden" }}>
              <div style={{ paddingTop: 8 }}>
                {item.signals.map((s) => (
                  <div key={s.key} style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "6px 0", borderBottom: "1px solid rgba(226,232,240,0.4)" }}>
                    <div style={{ width: 16, height: 16, borderRadius: "50%", flexShrink: 0, marginTop: 1, background: s.triggered ? "rgba(244,63,94,0.12)" : "rgba(16,185,129,0.12)", border: `1px solid ${s.triggered ? "rgba(244,63,94,0.3)" : "rgba(16,185,129,0.3)"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9 }}>
                      {s.triggered ? "!" : "✓"}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: s.triggered ? "#9f1239" : "#065f46" }}>{s.label}</div>
                      <div style={{ fontSize: 10, color: "#64748b", marginTop: 2 }}>{s.detail}</div>
                    </div>
                    {s.triggered && <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 10, fontWeight: 700, color: "#f43f5e", flexShrink: 0 }}>+{s.score}</div>}
                  </div>
                ))}
              </div>
            </m.div>
...
function AtRiskTab() {
  ...
  const filtered = useMemo(() => {
    ...
    return filter === "ALL" ? data.items : data.items.filter((i) => i.risk_level === filter);
  }, [data, filter]);
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 20 }}>
        {[{ label: "Active Projects", value: summary.total, color: "#6366f1" }, { label: "High Risk", value: summary.high, color: "#e11d48" }, { label: "Medium Risk", value: summary.medium, color: "#d97706" }, { label: "Low Risk", value: summary.low, color: "#10b981" }].map((s, i) => (
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
        {(["ALL", "HIGH", "MEDIUM", "LOW"] as const).map((f) => (
          <button key={f} onClick={() => setFilter(f)} style={{ padding: "5px 14px", borderRadius: 20, border: `1px solid ${filter === f ? "#6366f1" : "rgba(226,232,240,0.8)"}`, background: filter === f ? "rgba(238,242,255,0.9)" : "rgba(255,255,255,0.7)", color: filter === f ? "#4338ca" : "#64748b", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
            {f === "ALL" ? `All (${summary.total})` : f === "HIGH" ? `High (${summary.high})` : f === "MEDIUM" ? `Medium (${summary.medium})` : `Low (${summary.low})`}
          </button>
        ))}
      </div>
      {filtered.length === 0 ? (
        <div style={{ textAlign: "center", padding: "48px 0" }}><CheckCircle2 size={36} color="#10b981" style={{ margin: "0 auto 12px" }} /><div style={{ fontSize: 15, fontWeight: 600, color: "#374151" }}>No projects at this risk level</div></div>
      ) : (
...
function DigestTab() {
  ...
  const fetchDigest = useCallback(() => {
    ...
    fetch(`/api/executive/digest?days=${days}`, { credentials: "include", cache: "no-store" })
      .then((r) => r.json())
      .then((j) => { if (j?.ok) setDigest(j.digest); else setDigest(null); })
  ...
  const pendingApprovals = sections?.pending_approvals ?? sections?.sla_breaches ?? { total: 0, breached: 0, at_risk: 0, items: [] };
  const decisions = sections?.decisions ?? { total: 0, approved: 0, rejected: 0, approval_rate: null, recent: [] };
  const pmPerf = sections?.pm_performance ?? [];
  const atRisk = sections?.at_risk_projects ?? [];
  const newProjects = sections?.new_projects ?? [];
  const milestones = sections?.upcoming_milestones ?? [];
  const activeProjects = Number(sum?.active_projects ?? 0);
  const totalProjects = Number(sum?.total_projects ?? 0);
  const statPending = Number(sum?.pending_total ?? pendingApprovals?.total ?? 0);
  const statBreached = Number(sum?.breached_total ?? pendingApprovals?.breached ?? 0);
  const statDecisions = Number(sum?.decisions_total ?? decisions?.total ?? 0);
  const statNewProjects = Number(sum?.new_projects ?? 0);
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          ...
          {([7, 14, 30, 60] as const).map((d) => (
            <button key={d} onClick={() => setDays(d)} style={{ padding: "5px 14px", borderRadius: 20, border: `1px solid ${days === d ? "#6366f1" : "rgba(226,232,240,0.8)"}`, background: days === d ? "rgba(238,242,255,0.9)" : "rgba(255,255,255,0.7)", color: days === d ? "#4338ca" : "#64748b", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>{d}d</button>
          ))}
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={() => { setPdfLoading(true); window.open(`/api/executive/digest/pdf?days=${days}`, "_blank"); setTimeout(() => setPdfLoading(false), 2000); }} disabled={pdfLoading || !digest}
      ...
      {loading ? (
        ...
        <div style={{ padding: "60px 0", textAlign: "center" }}><FileText size={36} color="#cbd5e1" style={{ margin: "0 auto 12px" }} /><div style={{ fontSize: 14, fontWeight: 600, color: "#374151" }}>No digest data</div></div>
      ) : (
        <>
          <div style={{ ...CARD, padding: "16px 20px", marginBottom: 16, background: "linear-gradient(135deg,rgba(99,102,241,0.06),rgba(238,242,255,0.8))", border: "1px solid rgba(199,210,254,0.5)" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: "#64748b", marginTop: 3 }}>Projects: <b>{activeProjects}</b> active {totalProjects ? <span>of <b>{totalProjects}</b> total</span> : null}</div>
              </div>
              <button onClick={() => { window.open(`/api/executive/digest/pdf?days=${days}`, "_blank"); }} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", borderRadius: 10, border: "1px solid rgba(199,210,254,0.6)", background: "rgba(238,242,255,0.6)", color: "#4338ca", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginBottom: 16 }}>
            {[{ label: "Pending", value: statPending, color: "#6366f1" }, { label: "Breached", value: statBreached, color: "#e11d48" }, { label: "Decisions", value: statDecisions, color: "#0f172a" }, { label: "New Projects", value: statNewProjects, color: "#6366f1" }].map((s, i) => (
          ...
          <SectionCard title="Pending Approvals (incl. Breaches)" count={Number(pendingApprovals?.total ?? 0)}>
            {Number(pendingApprovals?.total ?? 0) === 0 ? (
              <div style={{ display: "flex", alignItems: "center", gap: 8, color: "#10b981", fontSize: 12, fontWeight: 600 }}><CheckCircle2 size={16} /> No pending approvals in this window</div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {(pendingApprovals?.items ?? []).slice(0, 20).map((r: any, i: number) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 10px", borderRadius: 8, background: "rgba(248,250,255,0.6)", border: "1px solid rgba(226,232,240,0.5)" }}>
                    <AlertTriangle size={13} color={ragColor(ss(r?.sla_status))} />
                    ...
                    <StatusBadge status={ss(r?.sla_status)} />
                  </div>
                ))}
              </div>
            )}
          ...
          <SectionCard title="Approval Decisions" count={Number(decisions?.total ?? 0)}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 14 }}>
              {[{ label: "Approved", value: decisions?.approved ?? 0, color: "#10b981" }, { label: "Rejected", value: decisions?.rejected ?? 0, color: (decisions?.rejected ?? 0) > 0 ? "#f43f5e" : "#94a3b8" }, { label: "Approval Rate", value: decisions?.approval_rate != null ? `${decisions.approval_rate}%` : "N/A", color: decisions?.approval_rate != null && decisions.approval_rate >= 70 ? "#10b981" : "#f43f5e" }].map((s, i) => (
            ...
            {(decisions?.recent ?? []).length > 0 && (
              <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                {(decisions?.recent ?? []).slice(0, 10).map((d: any, i: number) => (
          ...
          <SectionCard title="PM Performance Snapshot">
            {pmPerf.length === 0 ? <div style={{ fontSize: 12, color: "#94a3b8" }}>No PM data available.</div> : (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {pmPerf.slice(0, 12).map((pm: any, i: number) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", borderRadius: 10, background: "rgba(248,250,255,0.8)", border: "1px solid rgba(226,232,240,0.5)" }}>
                    <div style={{ width: 32, height: 32, borderRadius: "50%", background: "linear-gradient(135deg,#6366f1,#4f46e5)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "white", flexShrink: 0 }}>{initials(ss(pm?.full_name))}</div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: "flex", gap: 12, fontSize: 11 }}>
                      ...
                      <span style={{ color: (pm?.rejected ?? 0) > 0 ? "#f43f5e" : "#94a3b8", fontWeight: 700 }}>❌ {pm?.rejected}</span>
                      {(pm?.overdue ?? 0) > 0 && <span style={{ color: "#e11d48", fontWeight: 700 }}>⚠ {pm?.overdue} overdue</span>}
          ...
          <SectionCard title="At-Risk Projects" count={atRisk.length}>
            {atRisk.length === 0 ? (
              <div style={{ display: "flex", alignItems: "center", gap: 8, color: "#10b981", fontSize: 12, fontWeight: 600 }}><CheckCircle2 size={16} /> No projects at elevated risk</div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {atRisk.slice(0, 12).map((p: any, i: number) => (
          ...
          {newProjects?.length > 0 && (
            <SectionCard title="New Projects Started" count={newProjects.length}>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {newProjects.slice(0, 12).map((p: any, i: number) => (
          ...
          {milestones?.length > 0 && (
            <SectionCard title="Upcoming Milestones" count={milestones.length}>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {milestones.slice(0, 12).map((ms: any, i: number) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 10px", borderRadius: 8, background: "rgba(248,250,255,0.6)", border: "1px solid rgba(226,232,240,0.5)" }}>
...
export default function ControlCentrePage() {
  ...
  // Live counts from the primary approvals endpoint (has real .counts object)
  useEffect(() => {
    fetch("/api/executive/approvals", { credentials: "include", cache: "no-store" })
      .then((r) => r.json())
      .then((j) => { if (j?.ok && j?.counts) setLiveCounts(j.counts); })
      .catch(() => {});
  }, []);

  // Pending items for project breakdown + bottlenecks fallback
  useEffect(() => {
  ...
  async function handleAssignProject(pmId: string, projectId: string) {
    try {
      const res = await fetch("/api/executive/projects/assign-pm", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ project_id: projectId, pm_user_id: pmId }) });
      ...
      } else { setToast("Failed to assign PM"); }
  ...
  return (
    <LazyMotion features={domAnimation}>
      ...
      <AnimatePresence>
        {toast && (
          <m.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} style={{ position: "fixed", top: 20, right: 20, zIndex: 100, background: "rgba(15,23,42,0.9)", color: "white", borderRadius: 12, padding: "10px 16px", fontSize: 13, fontWeight: 600, backdropFilter: "blur(12px)" }}>
            {toast}
          </m.div>
        )}
      </AnimatePresence>
      ...
      <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,rgba(238,242,255,0.6) 0%,rgba(255,255,255,0.8) 50%,rgba(240,253,250,0.5) 100%)", padding: "32px 24px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <m.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
            ...
            <a href="/" style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: 10, background: "rgba(238,242,255,0.8)", border: "1px solid rgba(199,210,254,0.6)", fontSize: 11, fontWeight: 700, color: "#4338ca", textDecoration: "none" }}>← Dashboard</a>
          </m.div>

          <div style={{ display: "flex", gap: 4, marginBottom: 24, background: "rgba(241,245,249,0.8)", borderRadius: 12, padding: 4, width: "fit-content" }}>
            {TABS.map((t) => (
              <button key={t.id} onClick={() => setTab(t.id)} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 9, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 700, transition: "all 0.2s", background: tab === t.id ? "white" : "transparent", color: tab === t.id ? "#0f172a" : "#64748b", boxShadow: tab === t.id ? "0 1px 4px rgba(0,0,0,0.08)" : "none" }}>
                {t.icon} {t.label}
              </button>
            ))}
          </div>
          ...
          <AnimatePresence mode="wait">
            <m.div key={tab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.25 }}>
              {tab === "overview" && <OverviewTab cacheItems={cacheItems} loading={cacheLoading} counts={liveCounts} />}
              ...
              {tab === "pm" && (
                <div>
                  ...
                  {pmLoading ? (
                    ...
                    <div style={{ textAlign: "center", padding: "60px 0" }}><Users size={40} color="#cbd5e1" style={{ margin: "0 auto 16px" }} /><div style={{ fontSize: 15, fontWeight: 600, color: "#374151" }}>No PM data available</div></div>
                  ) : (
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(320px,1fr))", gap: 16 }}>
