// src/app/approvals/page.tsx — Control Centre v2 (aligned to digest API changes)
// Tabs: Overview · PM Performance · Bottlenecks · At Risk Predictor · Digest
//
// ✅ Digest tab now supports BOTH shapes:
//    - digest.sections.pending_approvals (new + now present in API)
//    - digest.sections.sla_breaches (legacy/new)
// ✅ Summary cards show active vs total projects (digest.summary.active_projects / total_projects)
// ✅ Fix: no duplicate ragColor() shadowing + safer defaults to prevent blank UI on shape mismatch
...
interface PmItem {
  ...
  rag: Rag;
  // optional new fields that may appear (harmless if missing)
      ...
      <div style={{ padding: "16px 16px 16px 20px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {pm.department ? (
              <div style={{ fontSize: 10, color: "#94a3b8", marginTop: 2 }}>{pm.department}</div>
            ) : null}
          </div>
          <RagBadge rag={pm.rag} />
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, marginBottom: 14 }}>
          ...
          <StatPill
            label="Overdue"
            value={pm.overdue_items}
            color={pm.overdue_items > 0 ? "#e11d48" : "#10b981"}
          />
          <StatPill label="Approved" value={pm.decisions.approved} color="#10b981" />
          <StatPill
            label="Rejected"
            value={pm.decisions.rejected}
            color={pm.decisions.rejected > 0 ? "#f43f5e" : "#94a3b8"}
          />
        </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
              ...
              <span
                style={{
                  ...
                  fontSize: 11,
                  fontWeight: 700,
                  color: rate && rate >= 70 ? "#10b981" : "#f43f5e",
                }}
              >
                {rate ?? 0}%
              </span>
        ...
        {pm.pending_as_approver > 0 && (
          ...
          >
            ...
            <span style={{ fontSize: 11, fontWeight: 600, color: "#92400e" }}>
              {pm.pending_as_approver} pending approval{pm.pending_as_approver !== 1 ? "s" : ""} awaiting this user
            </span>
          </div>
        )}
          ...
          {expanded && (
            ...
            >
              <div style={{ paddingTop: 8, display: "flex", flexDirection: "column", gap: 6 }}>
                ...
                {pm.project_list.length === 0 && (
                  <div style={{ fontSize: 11, color: "#94a3b8", padding: "4px 0" }}>No projects assigned yet</div>
                )}
                    ...
                    >
                      {projects
                        ...
                        .map((p) => (
                          ...
                          >
                            {p.project_code && (
                              <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 9, fontWeight: 700, color: "#4338ca" }}>
                                {p.project_code}
                              </span>
                            )}
...
function OverviewTab({ cacheItems, loading }: { cacheItems: CacheItem[]; loading: boolean }) {
  ...
  const byProject = useMemo(() => {
    const map = new Map<
      string,
      { title: string | null; code: string | null; count: number; breached: number; at_risk: number }
    >();
    for (const it of cacheItems) {
      ...
      let p = map.get(pid);
      if (!p) {
        p = { title: it.project_title, code: it.project_code, count: 0, breached: 0, at_risk: 0 };
        map.set(pid, p);
      }
      p.count++;
        ...
        {[
          ...
          <div key={i} style={{ ...CARD, display: "flex", alignItems: "center", gap: 14, padding: "14px 16px" }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 22, fontWeight: 700, color: s.color, lineHeight: 1.2 }}>
                {s.value}
              </div>
            </div>
          </div>
          ...
          >
            ...
            <div style={{ position: "relative", padding: "14px 16px", display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 10, color: "#94a3b8" }}>
                  {b.count} pending · {b.breached} breached · {b.projects.size} project{b.projects.size !== 1 ? "s" : ""}
                </div>
              </div>
              <RagBadge rag={heat} />
            </div>
...
function SignalRow({ signal }: { signal: RiskSignal }) {
  return (
    <div style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "6px 0", borderBottom: "1px solid rgba(226,232,240,0.4)" }}>
      ...
      {signal.triggered && (
        <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 10, fontWeight: 700, color: "#f43f5e", flexShrink: 0 }}>
          +{signal.score}
        </div>
      )}
    </div>
      ...
      <div style={{ padding: "14px 16px 14px 20px" }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10, marginBottom: 10 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
              {item.project_code && (
                <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 9, fontWeight: 700, color: "#4338ca", background: "rgba(238,242,255,0.8)", border: "1px solid rgba(199,210,254,0.6)", borderRadius: 4, padding: "1px 5px" }}>
                  {item.project_code}
                </span>
              )}
        ...
        <AnimatePresence>
          {expanded && (
            <m.div
              ...
              style={{ overflow: "hidden" }}
            >
              <div style={{ paddingTop: 8 }}>{item.signals.map((s) => <SignalRow key={s.key} signal={s} />)}</div>
            </m.div>
          )}
        </AnimatePresence>
      </div>
...
// ✅ AtRiskTab passes ?active_only=true
function AtRiskTab() {
  const [data, setData] = useState<{
    items: ProjectRisk[];
    summary: { total: number; high: number; medium: number; low: number; active_only?: boolean };
  } | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/executive/projects/at-risk?active_only=true", { credentials: "include", cache: "no-store" })
      .then((r) => r.json())
      .then((j) => {
        if (j?.ok) setData({ items: j.items ?? [], summary: j.summary ?? { total: 0, high: 0, medium: 0, low: 0 } });
      })
      .catch(() => {})
    ...
    <div>
        ...
        {(["ALL", "HIGH", "MEDIUM", "LOW"] as const).map((f) => (
          <button
            ...
            style={{
              padding: "5px 14px",
              borderRadius: 20,
              border: `1px solid ${filter === f ? "#6366f1" : "rgba(226,232,240,0.8)"}`,
              background: filter === f ? "rgba(238,242,255,0.9)" : "rgba(255,255,255,0.7)",
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(340px,1fr))", gap: 14 }}>
          {filtered.map((item, i) => (
            <RiskCard key={item.project_id} item={item} idx={i} />
          ))}
        </div>
      )}
    </div>
...
function SectionCard({
  ...
  return (
    <div style={{ ...CARD, padding: "18px 20px", marginBottom: 16 }}>
      ...
      >
        <span style={{ fontSize: 11, fontWeight: 800, color: "#0f172a", textTransform: "uppercase", letterSpacing: "0.12em" }}>
          {title}
        </span>
        {count !== undefined && (
          <span style={{ background: "#6366f1", color: "white", borderRadius: 20, padding: "1px 8px", fontSize: 10, fontWeight: 700 }}>
            {count}
          </span>
        )}
      </div>
...
function DigestTab() {
  ...
  // ✅ Normalise sections across API variants
  ...
  const decisions =
    sections?.decisions ?? { total: 0, approved: 0, rejected: 0, approval_rate: null, recent: [] };
          ...
          {([7, 14, 30, 60] as const).map((d) => (
            <button
              ...
              style={{
                padding: "5px 14px",
                borderRadius: 20,
                border: `1px solid ${days === d ? "#6366f1" : "rgba(226,232,240,0.8)"}`,
                background: days === d ? "rgba(238,242,255,0.9)" : "rgba(255,255,255,0.7)",
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button
            ...
            style={{
              ...
              fontSize: 11,
              fontWeight: 700,
              cursor: digest ? "pointer" : "not-allowed",
        ...
        <>
          <div
            style={{
              ...
              padding: "16px 20px",
              marginBottom: 16,
              background: "linear-gradient(135deg,rgba(99,102,241,0.06),rgba(238,242,255,0.8))",
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
              <div>
                <div style={{ fontSize: 16, fontWeight: 800, color: "#0f172a", marginBottom: 3 }}>
                  Portfolio Governance Digest
                </div>
                <div style={{ fontSize: 11, color: "#64748b" }}>
                  Last {days} days · Generated {fmtDate(digest.generated_at)}
                </div>
                <div style={{ fontSize: 11, color: "#64748b", marginTop: 3 }}>
                  Projects: <b>{activeProjects}</b> active {totalProjects ? <span>of <b>{totalProjects}</b> total</span> : null}
                </div>
              </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginBottom: 16 }}>
            {[
              { label: "Pending", value: statPending, color: "#6366f1" },
              { label: "Breached", value: statBreached, color: "#e11d48" },
              { label: "Decisions", value: statDecisions, color: "#0f172a" },
              { label: "New Projects", value: statNewProjects, color: "#6366f1" },
            ].map((s, i) => (
              <div key={i} style={{ ...CARD, padding: "12px 14px", textAlign: "center" }}>
                <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 24, fontWeight: 700, color: s.color, lineHeight: 1 }}>
                  {s.value}
                </div>
                <div style={{ fontSize: 9, color: "#94a3b8", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.1em", marginTop: 4 }}>
                  {s.label}
                </div>
              </div>
            ))}
          ...
          <SectionCard title="Pending Approvals (incl. Breaches)" count={Number(pendingApprovals?.total ?? 0)}>
            {Number(pendingApprovals?.total ?? 0) === 0 ? (
              <div style={{ display: "flex", alignItems: "center", gap: 8, color: "#10b981", fontSize: 12, fontWeight: 600 }}>
                <CheckCircle2 size={16} /> No pending approvals in this window
              </div>
            ) : (
          ...
          <SectionCard title="Approval Decisions" count={Number(decisions?.total ?? 0)}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 14 }}>
              {[
                { label: "Approved", value: decisions?.approved ?? 0, color: "#10b981" },
                { label: "Rejected", value: decisions?.rejected ?? 0, color: (decisions?.rejected ?? 0) > 0 ? "#f43f5e" : "#94a3b8" },
                {
                  label: "Approval Rate",
                  value: decisions?.approval_rate != null ? `${decisions.approval_rate}%` : "N/A",
                  color: decisions?.approval_rate != null && decisions.approval_rate >= 70 ? "#10b981" : "#f43f5e",
                },
              ].map((s, i) => (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {pmPerf.slice(0, 12).map((pm: any, i: number) => (
                  ...
                  >
                    ...
                    >
                      {initials(ss(pm?.full_name))}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 12, fontWeight: 700, color: "#0f172a" }}>{ss(pm?.full_name)}</div>
                      <div style={{ fontSize: 10, color: "#94a3b8" }}>
                        {pm?.projects_managed} project{pm?.projects_managed !== 1 ? "s" : ""}
                      </div>
                    </div>
                    <div style={{ display: "flex", gap: 12, fontSize: 11 }}>
                      <span style={{ color: "#10b981", fontWeight: 700 }}>✅ {pm?.approved}</span>
                      <span style={{ color: (pm?.rejected ?? 0) > 0 ? "#f43f5e" : "#94a3b8", fontWeight: 700 }}>
                        ❌ {pm?.rejected}
                      </span>
                      {(pm?.overdue ?? 0) > 0 && <span style={{ color: "#e11d48", fontWeight: 700 }}>⚠ {pm?.overdue} overdue</span>}
                      {pm?.approval_rate != null && (
                        <span style={{ color: pm?.approval_rate >= 70 ? "#10b981" : "#f43f5e", fontWeight: 700 }}>
                          {pm.approval_rate}%
                        </span>
                      )}
                    </div>
                ...
                {atRisk.slice(0, 12).map((p: any, i: number) => (
                  <div
                    ...
                    style={{
                      ...
                      padding: "8px 10px",
                      borderRadius: 8,
                      background: p.risk_level === "HIGH" ? "rgba(255,241,242,0.6)" : "rgba(255,251,235,0.6)",
                      border: `1px solid ${
                        p.risk_level === "HIGH" ? "rgba(253,164,175,0.3)" : "rgba(252,211,77,0.3)"
                      }`,
                    }}
                  >
                    <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 13, fontWeight: 800, color: ragColor(p.risk_level), minWidth: 32 }}>
                      {p.risk_score}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 10, color: "#64748b" }}>
                        {p.overdue_steps > 0 ? `${p.overdue_steps} overdue` : ""}
                        {p.days_since_activity != null ? ` · ${p.days_since_activity}d inactive` : ""}
                      </div>
                    </div>
...
export default function ControlCentrePage() {
  ...
  useEffect(() => {
    fetch("/api/executive/projects", { credentials: "include", cache: "no-store" })
      .then((r) => r.json())
      .then((j) => {
        if (j?.ok) setAllProjects(j.items ?? []);
      })
      .catch(() => {});
  }, []);
    ...
    <LazyMotion features={domAnimation}>
      ...
      <div
        style={{
          minHeight: "100vh",
          background:
            "linear-gradient(135deg,rgba(238,242,255,0.6) 0%,rgba(255,255,255,0.8) 50%,rgba(240,253,250,0.5) 100%)",
          padding: "32px 24px",
        }}
      >
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div>
                <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.2em", color: "#6366f1", marginBottom: 2 }}>
                  Executive Dashboard
                </div>
                ...
                <div>
                  ...
                  {pmLoading ? (
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(320px,1fr))", gap: 16 }}>
                      {[1, 2, 3].map((i) => (
                        <div key={i} style={{ height: 220, borderRadius: 16, background: "rgba(241,245,249,0.7)", animation: "pulse 2s ease-in-out infinite" }} />
                      ))}
                    </div>
                  ) : pmData.length === 0 ? (
