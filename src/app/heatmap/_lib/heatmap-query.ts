...
export async function fetchHeatmapData(filters: HeatmapFilters): Promise<HeatmapData> {
  ...
  // Fetch soft allocations for pipeline projects as fallback demand
  // (used when no role_requirements rows exist — the common case)
  const pipelineProjectIds = (pipelineProjects ?? []).map((p: any) => String(p.id));
  const pipelineAllocMap   = new Map<string, Map<string, number>>();

  if (pipelineProjectIds.length > 0) {
    const { data: pipelineAllocs } = await supabase
      .from("allocations")
      .select("project_id, week_start_date, days_allocated")
      .in("project_id", pipelineProjectIds)
      .gte("week_start_date", dateFrom)
      .lte("week_start_date", dateTo);

    for (const alloc of pipelineAllocs ?? []) {
      const projId = String(alloc.project_id);
      const week   = toMonday(String(alloc.week_start_date));
      const days   = parseFloat(String(alloc.days_allocated));
      if (!pipelineAllocMap.has(projId)) pipelineAllocMap.set(projId, new Map());
      const existing = pipelineAllocMap.get(projId)!.get(week) ?? 0;
      pipelineAllocMap.get(projId)!.set(week, existing + days);
    }
  }
  ...
  const pipelineGaps: PipelineGapRow[] = (pipelineProjects ?? []).map((proj: any) => {
    ...
    const allocsByWeek = pipelineAllocMap.get(String(proj.id)) ?? new Map<string, number>();

    const cells = periods.map(period => {
      let demandDays = 0;

      // Primary demand source: role_requirements
      ...
      // Fallback: use soft allocations as demand when no role_requirements exist
      if (demandDays === 0) {
        for (const [weekKey, days] of allocsByWeek) {
          if (weekKey >= period.startDate && weekKey <= period.endDate) {
            demandDays += days;
          }
        }
