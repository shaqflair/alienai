...
function PipelineCell({
  ...
  return (
    ...
    >
      {hasDemand ? (hasGap ? `-${cell!.gapDays}d` : "✓") : "--"}
    </div>
  );
}
      ...
      {periods.map((p) => (
        ...
        >
          ...
          <div
            style={{
              fontSize: "11px",
              fontWeight: p.isCurrentPeriod ? 800 : 500,
...
function PersonHeatmapRow({
  ...
  // peakTier retained for future styling if needed
  ...
  return (
    <div style={{ borderBottom: "1px solid #f1f5f9" }}>
      ...
      >
        ...
        >
          {/* FIX: arrow must be child content, not `}}{'>'}` */}
          <span
            style={{
              fontSize: "12px",
              color: "#94a3b8",
      ...
      {expanded && (
        <div style={{ paddingLeft: "222px", paddingBottom: "4px" }}>
          ...
          <div style={{ padding: "4px 0 6px" }}>
            <a
              href={`/allocations/new?person_id=${person.personId}&return_to=/heatmap`}
              style={{
                fontSize: "11px",
                color: "#00b8db",
                fontWeight: 600,
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: "11px", color: "#94a3b8" }}>
            {pipelineGaps.length} project{pipelineGaps.length > 1 ? "s" : ""} • Dashed cells show demand vs available capacity
          </div>
        </div>
        <span
          style={{
...
export default function HeatmapClient({
  ...
  const togglePerson = (id: string) =>
    setFilters((f) => ({
      ...f,
      personIds: f.personIds.includes(id) ? f.personIds.filter((p) => p !== id) : [...f.personIds, id],
    }));

  const clearFilters = () => setFilters((f) => ({ ...f, departments: [], statuses: [], personIds: [] }));

  const activeFilterCount = filters.departments.length + filters.statuses.length + filters.personIds.length;
  ...
  const avgUtil = data.people.length
    ? Math.round(data.people.reduce((s, p) => s + p.avgUtilisationPct, 0) / data.people.length)
    : 0;
          ...
          >
            <div>
              ...
              <p style={{ fontSize: "13px", color: "#94a3b8", margin: 0 }}>
                {data.dateFrom} {"→"} {data.dateTo} • {data.people.length} people •{" "}
          ...
          >
            {[
              { l: "People", v: data.people.length, c: "#0f172a" },
              {
                l: "Avg util",
                v: `${avgUtil}%`,
                c: avgUtil > 90 ? "#ef4444" : avgUtil > 70 ? "#f59e0b" : "#10b981",
              },
