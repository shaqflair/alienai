...
// Width of the left person/project info column
...
function EditModal({cell,people,projects,onClose,onSaved,onSaveError}:{cell:CS;people:PersonOption[];projects:PO[];onClose:()=>void;onSaved:(a:string,b:string,c:string,d:string,e:number,f:string)=>void;onSaveError?:(msg:string)=>void;}){
  ...
  function save(){
    const fd=new FormData();fd.set("person_id",pid);fd.set("project_id",prid);fd.set("start_date",sd);fd.set("end_date",ed);fd.set("days_per_week",String(dpw));fd.set("allocation_type",at);fd.set("return_to","/heatmap");
    onSaved(pid,prid,sd,ed,dpw,at);
    startT(async()=>{try{await updateAllocation(fd);}catch(e:any){onSaveError?.(e.message||"Save failed — please try again");}});
  }
  function del(){
    const fd=new FormData();fd.set("person_id",cell.personId);fd.set("project_id",cell.projectId);fd.set("return_to","/heatmap");
    onSaved(cell.personId,cell.projectId,"","",0,"");
    startT(async()=>{try{await deleteAllocationDirect(fd);}catch(e:any){onSaveError?.(e.message||"Delete failed — please try again");}});
  }
    ...
    <div onClick={e=>{if(e.target===e.currentTarget)onClose();}} style={{position:"fixed",inset:0,zIndex:2000,background:"rgba(15,23,42,0.5)",backdropFilter:"blur(3px)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",fontFamily:"'DM Sans',sans-serif"}}>
      <div style={{background:"white",borderRadius:"16px",border:`1.5px solid ${ac}30`,width:"100%",maxWidth:"440px",boxShadow:`0 20px 60px rgba(0,0,0,0.15),0 0 0 4px ${ac}10`}}>
        <div style={{padding:"16px 20px 12px",borderBottom:"1px solid #f1f5f9",background:`linear-gradient(135deg,${ac}08 0%,transparent 60%)`,borderRadius:"16px 16px 0 0",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          ...
          <button onClick={onClose} style={{background:"none",border:"none",color:"#94a3b8",cursor:"pointer",fontSize:"18px",padding:"2px 6px"}}>×</button>
        </div>
        ...
        <div style={{padding:"12px 20px 16px",borderTop:"1px solid #f1f5f9",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <div style={{display:"flex",gap:"8px"}}>
            ...
            <button onClick={save} disabled={!pid||!prid||!sd||!ed} style={{padding:"8px 18px",borderRadius:"8px",border:"none",background:!pid||!prid?"#e2e8f0":"#00b8db",color:!pid||!prid?"#94a3b8":"white",fontSize:"12px",fontWeight:800,cursor:!pid||!prid?"not-allowed":"pointer"}}>Save changes</button>
          </div>
        </div>
      </div>
    </div>
...
/* ── Person Row ──────────────────────────────────────────────────────── */
function PersonRow({person,periods,cw,expanded,onToggle,onCell}:{
  person:PersonRow;periods:PeriodHeader[];cw:number;expanded:boolean;
  onToggle:()=>void;onCell:(s:CS)=>void;
}){
  return(
    <div style={{borderBottom:"1px solid #f1f5f9"}}>
      ...
      <div style={{display:"flex",alignItems:"center",padding:"6px 0",cursor:"pointer",background:expanded?"rgba(0,184,219,0.02)":"transparent"}} onClick={onToggle}>
        ...
        <div style={{display:"flex",gap:"2px"}}>
          {periods.map(p=>{const c=person.summaryCells.find(c=>c.periodKey===p.key)??null;return<HCell key={p.key} cell={c} cw={cw} cur={p.isCurrentPeriod}/>;})
          }
        </div>
      </div>
      ...
      {expanded&&(
        <div style={{paddingBottom:"4px"}}>
          {/* Column headers for project sub-rows */}
          <div style={{
            display:"flex",alignItems:"center",
            paddingLeft:`${PERSON_COL}px`,
            paddingBottom:"2px",
          }}>
            <div style={{
              position:"relative",
              width:`${PERSON_COL}px`,minWidth:`${PERSON_COL}px`,
              marginLeft:`-${PERSON_COL}px`,
              display:"grid",
              ...
              <div/>{/* spacer for colour bar */}
              <div style={{display:"grid",gridTemplateColumns:"68px 1fr 80px",gap:"6px"}}>
                <div style={{fontSize:"9px",fontWeight:800,color:"#cbd5e1",textTransform:"uppercase",letterSpacing:"0.06em"}}>Code</div>
                <div style={{fontSize:"9px",fontWeight:800,color:"#cbd5e1",textTransform:"uppercase",letterSpacing:"0.06em"}}>Project</div>
                <div style={{fontSize:"9px",fontWeight:800,color:"#cbd5e1",textTransform:"uppercase",letterSpacing:"0.06em"}}>Role</div>
              </div>
            </div>
          </div>

          {person.projects.length===0
            ?<div style={{paddingLeft:`${PERSON_COL}px`,padding:"8px 0 8px",paddingLeft:`${PERSON_COL + 22}px` as any,fontSize:"12px",color:"#94a3b8",fontStyle:"italic"}}>No allocations in this period</div>
            :person.projects.map(proj=>(
              <div key={proj.projectId} style={{display:"flex",alignItems:"center",padding:"3px 0",gap:"2px"}}>
                {/* Left info: colour bar + code + name + role */}
                <div style={{
                  width:`${PERSON_COL}px`,minWidth:`${PERSON_COL}px`,flexShrink:0,
                  display:"grid",gridTemplateColumns:"16px 1fr",gap:"6px",
                  paddingRight:"12px",paddingLeft:"22px",
                }}>
                  {/* Colour bar */}
                  <div style={{width:"3px",height:"100%",minHeight:"30px",borderRadius:"2px",background:proj.colour,alignSelf:"stretch",justifySelf:"center"}}/>
                  {/* Code / Name / Role */}
                  <div style={{display:"grid",gridTemplateColumns:"68px 1fr 80px",gap:"6px",alignItems:"center",minWidth:0}}>
                    {/* Code */}
                    <div style={{
                      fontSize:"10px",fontWeight:700,fontFamily:"'DM Mono',monospace",
                      color:proj.colour,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",
                    }} title={proj.projectCode||"—"}>
                    ...
                    <div style={{
                      ...
                      overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",
                      fontStyle:proj.roleOnProject?"normal":"italic",
                    }} title={proj.roleOnProject||"—"}>
                ...
                <div style={{display:"flex",gap:"2px"}}>
                  {periods.map(p=>{
                    ...
                    return(
                      <div key={p.key}
                        title={`${c.daysAllocated}d — click to edit`}
                        onClick={()=>onCell({personId:person.personId,personName:person.fullName,projectId:proj.projectId,projectTitle:proj.projectTitle,projectCode:proj.projectCode,colour:proj.colour,periodKey:p.key,startDate:p.startDate,endDate:p.endDate,daysAllocated:c.daysAllocated,capacityDays:c.capacityDays})}
                        style={{width:cw-2,minWidth:cw-2,height:"26px",borderRadius:"4px",flexShrink:0,background:`${proj.colour}15`,border:`1px solid ${proj.colour}30`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"10px",fontWeight:700,fontFamily:"'DM Mono',monospace",color:proj.colour,cursor:"pointer",transition:"all 0.1s"}}
                        onMouseEnter={e=>{const d=e.currentTarget as HTMLDivElement;d.style.background=`${proj.colour}30`;d.style.transform="scale(1.05)";}}
                        onMouseLeave={e=>{const d=e.currentTarget as HTMLDivElement;d.style.background=`${proj.colour}15`;d.style.transform="scale(1)";}}
                      >
                        {c.daysAllocated}d
                      </div>
                    );
          ...
          <div style={{padding:"4px 0 6px",paddingLeft:`${PERSON_COL + 22}px`}}>
...
function PipeSection({gaps,periods,cw}:{gaps:PipelineGapRow[];periods:PeriodHeader[];cw:number}){
  ...
  return(<div style={{marginTop:"16px",border:"1.5px dashed #c4b5fd",borderRadius:"12px",overflow:"hidden"}}>
    <div style={{padding:"10px 16px",background:"rgba(124,58,237,0.04)",display:"flex",alignItems:"center",gap:"10px",cursor:"pointer"}} onClick={()=>setOpen(o=>!o)}>
      ...
      <span style={{fontSize:"14px",color:"#94a3b8",transform:open?"rotate(180deg)":"rotate(0deg)",transition:"transform 0.2s",display:"inline-block"}}>▾</span>
    </div>
...
export default function HeatmapClient({
  ...
  // Search term for quick-filtering people by name / project
  ...
  // Filter people by search term (name, department, job title, or any project name/code)
  const visiblePeople = search.trim()
    ? data.people.filter(p=>{
        ...
        if(p.jobTitle?.toLowerCase().includes(q))return true;
        if(p.projects.some(pr=>
    ...
    <div className="hmr"><div className="hmi">
      ...
      <div style={{display:"flex",alignItems:"center",gap:"12px",flexWrap:"wrap",marginBottom:"16px"}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",gap:"8px"}}>
          <button type="button" onClick={()=>setExp(new Set(data.people.map(p=>p.personId)))} style={{padding:"6px 12px",borderRadius:"7px",border:"1.5px solid #e2e8f0",background:"white",color:"#64748b",fontSize:"11px",fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Expand all</button>
      ...
      <div className="hmc" style={{opacity:loading?0.7:1,transition:"opacity 0.2s"}}>
        ...
        {/* Sticky header: column labels + search bar */}
        <div style={{padding:"12px 16px 8px",borderBottom:"1px solid #f1f5f9",position:"sticky",top:0,background:"white",zIndex:10}}>
          {/* Column label row */}
          <div style={{display:"flex",alignItems:"flex-end",marginBottom:"8px"}}>
            {/* Left column headers: PERSON + sub-columns for project rows */}
            <div style={{width:PERSON_COL,minWidth:PERSON_COL,flexShrink:0}}>
              ...
              {/* Sub-column labels hint — shown below the PERSON label */}
              <div style={{
                display:"grid",
                gridTemplateColumns:"16px 68px 1fr 80px",
                gap:"6px",
                paddingLeft:"22px",
                paddingRight:"12px",
              }}>
                <div/>
                <div style={{fontSize:"9px",fontWeight:700,color:"#e2e8f0",textTransform:"uppercase",letterSpacing:"0.06em"}}>Code</div>
                <div style={{fontSize:"9px",fontWeight:700,color:"#e2e8f0",textTransform:"uppercase",letterSpacing:"0.06em"}}>Project name</div>
                <div style={{fontSize:"9px",fontWeight:700,color:"#e2e8f0",textTransform:"uppercase",letterSpacing:"0.06em"}}>Role</div>
              </div>
            </div>
          ...
          <div style={{position:"relative"}}>
            <input
              ...
              placeholder="Search by name, project, code, role or department…"
              style={{
            ...
            {search&&(
              <button type="button" onClick={()=>setSearch("")} style={{position:"absolute",right:"8px",top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:"#94a3b8",cursor:"pointer",fontSize:"14px",lineHeight:1,padding:"2px 4px"}}>×</button>
            )}
          </div>
          {search&&<div style={{marginTop:"4px",fontSize:"11px",color:"#94a3b8"}}>{visiblePeople.length} of {data.people.length} people match</div>}
        </div>
          ...
          {visiblePeople.length===0
            ...
            :visiblePeople.map(p=>(
              <PersonRow
                ...
                cw={cw}
                expanded={exp.has(p.personId)}
                onToggle={()=>tE(p.personId)}
