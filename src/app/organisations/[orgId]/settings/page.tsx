...
function tabBtn(active: boolean) {
  return `px-3 py-1.5 text-sm rounded ${active ? "bg-gray-100 font-semibold" : "bg-white hover:bg-gray-50"}`;
}
...
export default async function OrgSettingsPage({
  ...
  const tab: "settings" | "approvals" | "cover" | "ratecards" =
    tabRaw === "approvals"  ? "approvals"  :
    tabRaw === "cover"      ? "cover"      :
    tabRaw === "ratecards"  ? "ratecards"  :
    "settings";
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6 text-gray-900">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-600">
            ...
            <span className="ml-2 text-xs text-gray-500">&bull; Members: {memberCount ?? "—"}</span>
            <span className="ml-2 text-xs text-gray-500">&bull; Platform admin: {platformAdmin ? "Yes" : "No"}</span>
          </p>
        </div>

        <div className="flex gap-2">
          <Link href={`/organisations/${organisationId}/members`} className="rounded-md border px-3 py-1.5 text-sm hover:bg-gray-50">
            Members
          </Link>
      ...
      <div className="rounded-xl border bg-white overflow-hidden">
        ...
        <div className="border-b p-2 flex gap-1 flex-wrap">
          <Link href={`/organisations/${organisationId}/settings?tab=settings`} className={tabBtn(tab === "settings")}>
            General
          </Link>
          <Link href={`/organisations/${organisationId}/settings?tab=approvals`} className={tabBtn(tab === "approvals")}>
            Approvals
          </Link>
          {/* NEW: Holiday Cover tab */}
          <Link href={`/organisations/${organisationId}/settings?tab=cover`} className={tabBtn(tab === "cover")}>
            Holiday Cover
          </Link>
          <Link href={`/organisations/${organisationId}/settings?tab=ratecards`} className={tabBtn(tab === "ratecards")}>
            Rate Cards
            {isOrgAdmin && (
        ...
        <div className="p-4">
          ...
          {/* ── Holiday Cover tab (NEW) ── */}
          {tab === "cover" && (
            <div className="space-y-4">
              <div>
                <h2 className="text-base font-semibold text-gray-900">Holiday Cover</h2>
                <p className="mt-1 text-sm text-gray-500">
                  ...
                  the original approver. All decisions are audit-logged with the delegation reference.
                </p>
              </div>
              {/* Pass orgId directly — HolidayCoverPanel resolves members via orgId param */}
              <HolidayCoverPanel
                    ...
                    {transferCandidates.length === 0 ? (
                      ...
                      <form action={transferOwnershipAction} className="flex flex-wrap items-end gap-2">
                        <div className="space-y-1">
                          ...
                          <select name="new_owner_user_id" className="min-w-[260px] rounded-md border px-3 py-2 text-sm bg-white" required>
                ...
                {isOrgAdmin ? (
                  <form action={renameAction} className="flex flex-wrap gap-2">
                    <input
                      ...
                      required
                    />
                    <button className="rounded-md bg-black text-white px-4 py-2 text-sm" type="submit">
                      Save
                    </button>
                  </form>
                ) : (
                ...
                {isOrgAdmin ? (
                  <form action={deleteAction} className="space-y-3">
                    ...
                    <input
                      ...
                      required
                    />
                    <button className="rounded-md border border-red-300 px-4 py-2 text-sm hover:bg-red-50 text-red-700" type="submit">
                      Delete organisation
                    </button>
                  </form>
                ) : (
